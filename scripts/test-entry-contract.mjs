...
const checks = [
  [middleware.includes("redirectWithAuth('/login'"), 'protected routes redirect signed-out users to /login through the auth-cookie preserving redirect boundary'],
  ...
  [teacherSignup.includes("p_role: 'teacher'"), 'teacher signup claims its initial role through the server-owned role boundary'],
  [teacherSignup.includes("get_my_onboarding_state"), 'teacher signup continues through the canonical onboarding resolver'],
  [parentSignup.includes("p_role: 'parent'"), 'parent signup claims its initial role through the server-owned role boundary'],
  [parentSignup.includes("get_my_onboarding_state"), 'parent signup continues through the canonical onboarding resolver'],
  ...
  [roleLogin.includes("get_my_role") && roleLogin.includes("get_my_onboarding_state"), 'focused sign in resolves canonical database role and onboarding instead of trusting the selected URL'],
  [!roleLogin.includes("actualRole !== expectedRole"), 'role URL is presentation intent only and cannot override or reject the canonical database role'],
  [roleLogin.includes("student: { label: 'Learner', email: false }"), 'learner sign in uses admission number and PIN'],
  [roleLogin.includes("parent: { label: 'Parent', email: true }"), 'parent sign in uses the focused parent path'],
