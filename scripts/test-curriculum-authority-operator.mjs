...
const checks = [
  ...
  ["view-only Drive evidence fallback", fn.includes("fetchGoogleDriveViewerEvidence") && fn.includes("MAX_VIEWER_PAGES") && fn.includes("artifact_viewer_page_not_png")],
  ["viewer capture provenance is explicit", fn.includes('captureKind: "google_drive_viewer_page_bundle_v1"') && fn.includes("source_byte_identical") && fn.includes("viewer_page_count")],
  ["viewer pages retained incrementally", fn.includes('contentType: "image/png"') && fn.includes("pageSha256") && fn.includes("aggregate_page_bytes")],
