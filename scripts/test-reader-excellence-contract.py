...
def main():
 ...
 for v in ["const{publicationId}=await params","<ReadingAnalyticsTracker/>","<ReaderListenContinuity publicationId={publicationId}/>","<ReaderContinuityCoordinator/>","<ReaderStudyInteractions/>","<ReaderTermExplainer/>","<ReaderAnnotationManager/>","<ReaderPurchaseBar publicationId={publicationId}/>","<ReaderAssessmentLauncher/>"]: require(layout,v,"canonical reader capability")
 forbid(layout,"<ReaderLearningLauncher/>","single learning surface")
 for v in ["Contents","What you’ll learn","ReaderLearningSheet","ReaderNarrationMiniPlayer","data-reader-block-id","record_reading_progress","vibe:reader-chapter"]: require(page,v,"flagship reader")
