...
# Create disposable identities without invoking application onboarding triggers. This
# is a local reconstructed database only; no production data is touched.
"${PSQL[@]}" <<SQL
...
# 20 simultaneous identical teacher provisioning requests must resolve to one
# canonical learner and one receipt.
pids=()
for i in $(seq 1 20); do
  run_as "$TEACHER_ID" "select public.teacher_add_student('Teacher Race Learner','T3-RACE-T-001',null,'$SCHOOL_ID');" "$work/teacher.$i" &
  pids+=("$!")
done
...
[[ "$teacher_count" == "1" && "$teacher_receipts" == "1" && "$teacher_ids" == "1" ]] || {
  echo "teacher provisioning race failed: students=$teacher_count receipts=$teacher_receipts ids=$teacher_ids" >&2; exit 1;
}

# 20 simultaneous identical admin provisioning requests must also collapse to one
# canonical learner and one receipt.
pids=()
for i in $(seq 1 20); do
  run_as "$ADMIN_ID" "select public.admin_add_student('Admin Race Learner','T3-RACE-A-001','','',null,'$SCHOOL_ID');" "$work/admin.$i" &
  pids+=("$!")
done
for p in "${pids[@]}"; do wait "$p"; done
...
[[ "$admin_count" == "1" && "$admin_receipts" == "1" && "$admin_ids" == "1" ]] || {
  echo "admin provisioning race failed: students=$admin_count receipts=$admin_receipts ids=$admin_ids" >&2; exit 1;
}

# Task 1 reconciliation made direct Parent learner manufacture invalid. Twenty
# simultaneous stale-client attempts must all fail and create zero learners.
parent_success=0
pids=()
for i in $(seq 1 20); do
  (run_as "$PARENT_ID" "select public.create_child_for_parent('Parent Race Child','2014-01-01',null);" "$work/parent.$i" && echo ok >"$work/parent.$i.ok" || true) &
  pids+=("$!")
done
for p in "${pids[@]}"; do wait "$p"; done
...
[[ "$parent_success" == "0" && "$parent_children" == "0" ]] || {
  echo "parent direct-create denial race failed: successes=$parent_success learners=$parent_children" >&2; exit 1;
}

# Create one canonical learner for enrollment/claim races.
"${PSQL[@]}" <<SQL
...
# Twenty simultaneous current-enrollment writes may yield one winner and nineteen
# unique-constraint denials, but never more than one current row.
pids=()
for i in $(seq 1 20); do
  cls="$CLASS_A"; [[ $((i % 2)) -eq 0 ]] && cls="$CLASS_B"
  (psql "$DB_URL" -X -qAt -v ON_ERROR_STOP=1 -c "insert into public.student_classes(student_id,class_id,school_id,is_current) values('$CLAIM_STUDENT','$cls','$SCHOOL_ID',true);" >"$work/enroll.$i" 2>"$work/enroll.$i.err" || true) &
  pids+=("$!")
done
for p in "${pids[@]}"; do wait "$p"; done
...
# Repeated pending join requests must structurally collapse to one active request.
pids=()
for i in $(seq 1 20); do
  (psql "$DB_URL" -X -qAt -v ON_ERROR_STOP=1 -c "insert into public.class_join_requests(student_id,class_id,parent_id,status) values('$CLAIM_STUDENT','$CLASS_A','$PARENT_ID','pending');" >"$work/join.$i" 2>"$work/join.$i.err" || true) &
  pids+=("$!")
done
for p in "${pids[@]}"; do wait "$p"; done
...
# Two authenticated learner accounts race to claim the same canonical learner.
# The row lock/claim state may return denial as data or as an error, but the final
# identity must have exactly one claimant and one profile binding.
...
[[ "$claimants" == "1" && "$bound" == "1" ]] || {
  echo "student claim race failed: claimants=$claimants bound=$bound" >&2; exit 1;
}
