...
begin
  ...
  if not coalesce(v_rls, false) then
    raise exception 'canonical version RLS disabled';
  end if;
  ...
  where schemaname = 'public'
    and tablename = 'learning_resource_versions'
    and indexname in (
      'learning_resource_versions_one_certified_uidx',
      'learning_resource_versions_one_inflight_uidx'
    );
  if v_count <> 2 then
    raise exception 'canonical version uniqueness indexes missing';
  end if;

  select pg_get_functiondef('public.cla_get_certified_learning_resource(text)'::regprocedure)
    into v_def;
  ...
  -- Create two isolated platform roots. These are rolled back at the end.
  insert into public.learning_resources (
    ...
    'teacher_note',
    'CLA verification root A',
    'active',
    'public',
    'platform',
    'cla:test:r3.2:root-a',
    'lesson_plan',
    'teach',
    1,
    'en'
  ) returning id into v_root_a;
  ...
  insert into public.learning_resources (
    ...
    'teacher_note',
    'CLA verification root B',
    'active',
    'public',
    'platform',
    'cla:test:r3.2:root-b',
    'lesson_plan',
    'teach',
    1,
    'en'
  ) returning id into v_root_b;
  ...
  insert into public.learning_resource_versions (
    ...
    1,
    'certified',
    '{"body":"version one"}'::jsonb,
    repeat('a', 64),
    '{"origin":"test"}'::jsonb,
    'cleared',
    'test-policy-v1',
    '{"semantic":"passed","rights":"passed"}'::jsonb,
    now(),
    now()
  ) returning id into v_v1;

  -- A second currently certified version must fail while v1 remains certified.
  v_failed := false;
  begin
    insert into public.learning_resource_versions (
      resource_id,
  ...
  if not v_failed then
    raise exception 'two certified versions coexisted for one root';
  end if;

  -- Cross-root lineage must fail.
  insert into public.learning_resource_versions (
    resource_id,
    version,
    lifecycle_status,
  ...
  if not v_failed then
    raise exception 'cross-root version lineage was accepted';
  end if;

  -- Certified payload mutation must fail.
  v_failed := false;
  begin
  ...
  if not v_failed then
    raise exception 'certified version payload mutated';
  end if;

  -- Valid retirement preserves the payload/evidence and frees certification slot.
  update public.learning_resource_versions
  set lifecycle_status = 'retired', retired_at = now()
  where id = v_v1;

  -- Retired rows are immutable.
  v_failed := false;
  begin
    update public.learning_resource_versions
    set payload = '{"body":"tampered after retirement"}'::jsonb
    where id = v_v1;
  exception when insufficient_privilege then
    v_failed := true;
  end;
  if not v_failed then
    raise exception 'retired version mutated';
  end if;
  ...
  -- Candidate must never appear through certified lookup.
  if exists (
    select 1
    from public.cla_get_certified_learning_resource('cla:test:r3.2:root-b')
  ) then
    raise exception 'candidate leaked through certified lookup';
  end if;

  -- Current certified successor should be returned for root A.
  if not exists (
    select 1
    from public.cla_get_certified_learning_resource('cla:test:r3.2:root-a') x
    where x.resource_version_id = v_v2
      and x.version = 2
  ) then
    raise exception 'certified successor lookup failed';
  end if;
end $$;
rollback;
