...
"""
    ...
    if not report or (
        report["reconciliation_status"] != "RECONCILED"
    ):
...
"""
    ...
    if not report or len(report["local_only"]) != 1:
        raise AssertionError(
            "repository-only version was not detected"
        )

    print("PASS: repository-only migration detected")
...
"""
    ...
    if not report or len(report["remote_only"]) != 1:
        raise AssertionError(
            "production-only version was not detected"
        )

    print("PASS: production-only migration detected")
...
"""
    ...
    if not report or (
        len(report["duplicate_remote_versions"]) != 1
    ):
        raise AssertionError(
            "duplicate production version was not detected"
        )

    print("PASS: duplicate production version detected")
    ...
    if result.returncode == 0:
        raise AssertionError(
            "wrong production project unexpectedly passed"
        )
