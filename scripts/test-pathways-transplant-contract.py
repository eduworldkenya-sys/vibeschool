...
migration=text('supabase/migrations/20260816150500_pathways_truth_and_acquisition.sql'); continuation=text('lib/auth/continuation.ts'); callback=text('app/auth/callback/route.ts'); login=text('app/login/[role]/page.tsx'); quick=text('app/pathways/check/page.tsx'); continue_page=text('app/pathways/continue/page.tsx'); schools=text('app/pathways/schools/page.tsx')
checks={
...
if failed: raise SystemExit('Pathways contract failed: '+'; '.join(failed))
