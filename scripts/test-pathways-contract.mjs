...
// Existing canonical identities are dependencies, never duplicated.
...
// Truth and provenance are mandatory.
...
// RLS and function privilege boundaries.
...
// Replay/idempotency lineage may not silently diverge.
requireText(domain,'input_fingerprint','student decisions need input fingerprint')
requireText(domain,'idempotency_replay_mismatch','student mismatched replay must fail closed')
requireText(family,'idempotency_replay_mismatch','parent mismatched replay must fail closed')

// Anonymous-first/minor-safe UX.
...
// School-search semantics must distinguish unknown from negative.
...
// Continuation is allowlisted and onboarding-gated.
...
console.log('PASS Pathways authority, safety, continuation and discoverability contract')
