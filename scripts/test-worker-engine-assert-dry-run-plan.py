...
def expect_parse_failure(contents: str, text: str) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "expected.txt"
        path.write_text(contents, encoding="utf-8")
        try:
            module.parse_versions(path)
        except ValueError as exc:
            assert text in str(exc), (text, str(exc))
        else:
            raise AssertionError(f"expected parse failure containing {text!r}")
...
def main() -> None:
    ...
    assert "migration_mention_count=2" in summary
    assert "duplicate_planned_versions=[]" in summary

    duplicate = good + "Applying migration 20260812191500_worker_engine_we_l1_authority_lifecycle.sql...\n"
    expect_failure(expected, duplicate, "ambiguous duplicate migration mentions")
    ...
    expect_failure(expected, "unparseable output without migration filenames\n", "not all approved migrations")

    expect_parse_failure("", "only 14-digit migration versions")
    expect_parse_failure("20260812191500\nnot-a-version\n", "only 14-digit migration versions")
