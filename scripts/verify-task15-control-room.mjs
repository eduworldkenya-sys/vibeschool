...
for(const fn of [
 'hq_workforce_owner_control_snapshot','hq_workforce_owner_start_controlled_operations','hq_workforce_owner_stop_operations',
 'hq_workforce_owner_set_global_stop','hq_workforce_owner_configure_global_envelope','hq_workforce_owner_control_authority',
 'hq_workforce_owner_reset_breaker','hq_workforce_owner_authority_catalog','hq_workforce_owner_issue_authority_draft'
]) requireText(sql,`function public.${fn}`,`missing ${fn}`)

for(const marker of [
 'perform public.hq_assert_owner()','control_room_stale_runtime_state','hq_workforce_trip_execution_breaker(',
 "'global','global','owner_global_stop'",'hq_workforce_owner_transition_capability_authority(',"g.id,'suspend'",
 'task15_migration_must_not_activate_runtime','control_room_policy_change_requires_runtime_off',
 'control_room_breaker_reset_requires_runtime_off','control_room_global_breaker_release_via_global_stop_only',
 'control_room_authority_activation_global_stop_active','control_room_authority_draft_requires_runtime_off',
 'control_room_foundational_capability_grant_missing','control_room_worker_budget_missing',
 'control_room_authority_nonterminal_grant_already_exists','public.hq_workforce_issue_capability_authority_draft(',
 'v_verification:=jsonb_build_object(',"reason_code='owner_global_stop'",'owner_global_stop_breakers_reset',
 'authority_suspended',"'authority_reactivated',false"
...
for(const fn of ['hq_workforce_owner_start_controlled_operations','hq_workforce_owner_stop_operations','hq_workforce_owner_set_global_stop','hq_workforce_owner_configure_global_envelope','hq_workforce_owner_control_authority','hq_workforce_owner_reset_breaker','hq_workforce_owner_authority_catalog','hq_workforce_owner_issue_authority_draft']) requireText(sql,`revoke all on function public.${fn}`,`grant hardening missing: ${fn}`)
requireText(sql,'from public,anon,service_role','service-role consequential-control denial missing')

for(const text of [
 'Control Room','Safety summary','Authority readiness','Foundational grants','Eligible packages','Create temporary authority draft',
 'Safety envelope','Adjust permitted controls','Start Controlled Operations','Activation review','Stop Operations','Global Stop',
 'Temporary authority','Activate authority','Circuit breakers','Review breaker reset','Recent executions','Resource envelope','Owner audit trail',
 'hq_workforce_owner_control_snapshot','hq_workforce_owner_authority_catalog','hq_workforce_owner_issue_authority_draft',
 'hq_workforce_owner_start_controlled_operations','hq_workforce_owner_stop_operations','hq_workforce_owner_set_global_stop',
 'hq_workforce_owner_configure_global_envelope','hq_workforce_owner_control_authority','hq_workforce_owner_reset_breaker'
]) requireText(ui,text,`UI contract missing: ${text}`)

requireText(ui,'Type ${token} to confirm','deliberate typed confirmation missing')
requireText(ui,'CREATE AUTHORITY DRAFT','authority draft deliberate confirmation missing')
...
requireText(ui,'Control Room will not manufacture those prerequisites','foundational authority boundary not explained')
...
console.log('- authority readiness fails closed instead of manufacturing foundational grants')
