...
const checks = [
  ...
  ["Today explains Unknown is not zero or healthy", today.includes("Unknown means evidence is unavailable, stale or insufficient")],
  ["stale evidence cannot certify fresh health", today.includes('const unknown=(s:SourceState<unknown>)=>s.status!=="live"') && today.includes('const workerSafe=workforce.status==="live"')],
  ["failed refresh demotes retained evidence to cached", today.includes('return current.data?{...current,status:"cached"')],
