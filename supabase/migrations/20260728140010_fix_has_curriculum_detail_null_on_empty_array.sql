-- READ-004 fix — has_curriculum_detail returned NULL instead of FALSE.
--
-- array_length() on a non-null empty array ('{}') returns NULL in
-- Postgres, not 0. That NULL propagated through the OR chain
-- (FALSE OR FALSE OR NULL = NULL), so a chapter with cs.id/cu.id both
-- null AND learning_outcomes = '{}' (the actual live shape, not literal
-- NULL) reported has_curriculum_detail: null instead of false. Caught by
-- smoke test against the live unlinked chapter before any client wiring.
-- Only this one expression changes.
