...
declare
 v_uid uuid:=auth.uid(); v_school uuid; v_role text; v_status text; v_anonymized boolean; v_subdomain text; v_suffix int:=0;
...
begin
 ...
 select c.canonical_school_id into v_school from public.school_identity_candidates c join public.schools s on s.id=c.canonical_school_id and s.deleted_at is null and s.status in ('pending','active')
 where c.directory_school_id=p_directory_id and c.status in ('matched','new') and c.canonical_school_id is not null and c.reviewed_by is not null and c.reviewed_at is not null
 ...
 if v_school is null then select s.id into v_school from public.schools s where s.deleted_at is null and s.directory_source='schools_directory' and s.directory_source_ref=p_directory_id::text limit 1; end if;
 if v_school is null then
   v_subdomain:=left(regexp_replace(lower(coalesce(nullif(trim(d.name),''),'school')),'[^a-z0-9]+','-','g'),40);
   v_subdomain:=trim(both '-' from v_subdomain);
   if v_subdomain='' then v_subdomain:='school'; end if;
   v_subdomain:=v_subdomain||'-'||left(replace(p_directory_id::text,'-',''),8);
   while exists(select 1 from public.schools where subdomain=v_subdomain) loop v_suffix:=v_suffix+1; v_subdomain:=left(v_subdomain,54)||'-'||v_suffix::text; end loop;
   ...
   values(d.name,v_subdomain,'Africa/Nairobi','KE','pending',v_uid,true,d.county,d.sub_county,d.latitude,d.longitude,d.knec_code,d.type,'schools_directory',d.id::text,case when d.is_verified then now() else null end) returning id into v_school;
 end if;
 insert into public.school_identity_candidates(directory_school_id,canonical_school_id,status,confidence,match_reason)
 values(p_directory_id,v_school,'pending',case when d.is_verified then .95 else .70 end,'Teacher-selected directory identity; isolated canonical school created pending operator reconciliation')
 ...
 update public.profiles set school_id=v_school where id=v_uid; if not found then raise exception 'profile_update_failed'; end if;
 ...
 if not exists(select 1 from public.school_members where school_id=v_school and profile_id=v_uid and role='teacher') then raise exception 'school_connection_verification_failed'; end if;
 return v_school;
end $$;
