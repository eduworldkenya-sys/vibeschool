-- Keep autonomous workforce Library writes on the same version/approval integrity contract as owner writes.
...
begin
  ...
  insert into public.hq_artifacts(
    ...
    btrim(p_artifact_key),btrim(p_title),btrim(p_artifact_type),v_worker.department_key,
    ...
    'draft','not_required',coalesce(p_metadata,'{}'::jsonb) || jsonb_build_object(
      'origin','hq_workforce','worker_key',v_worker.worker_key,'lane_key',v_run.lane_key,'run_id',v_run.id
    )
  )
  ...
  -- A new autonomous version is always draft/unapproved until HQ reviews that exact version.
