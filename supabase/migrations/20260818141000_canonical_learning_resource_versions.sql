...
-- Additive only; production promotion is separate.
-- authorization-test: public.learning_resource_versions scripts/sql/canonical_learning_resource_versions_verify.sql
...
alter table public.learning_resources
  ...
  add constraint learning_resources_asset_kind_check check (
    asset_kind is null or asset_kind = any (array[
      'lesson_plan','teacher_notes','learner_notes','homework','quiz','exercise',
      'revision','worksheet','assessment','worked_example','project','practical',
      'remedial','enrichment','marking_scheme','rubric','content_block'
    ]::text[])
  ),
  drop constraint if exists learning_resources_purpose_check,
  add constraint learning_resources_purpose_check check (
    purpose is null or purpose = any (array[
      'teach','practise','assess','revise','remediate','enrich','reference'
    ]::text[])
  ),
...
create index if not exists learning_resources_asset_lookup_idx
  on public.learning_resources (identity_key_version, asset_kind, purpose, language_code)
...
create table if not exists public.learning_resource_versions (
  ...
  previous_version_id uuid references public.learning_resource_versions(id) on delete restrict,
  lifecycle_status text not null default 'candidate' check (
    lifecycle_status = any (array['candidate','verified','certified','retired','rejected']::text[])
  ),
  payload_format text not null default 'json' check (btrim(payload_format) <> ''),
  payload jsonb not null,
  content_sha256 text not null check (content_sha256 ~ '^[0-9a-f]{64}$'),
  provenance jsonb not null default '{}'::jsonb check (jsonb_typeof(provenance) = 'object'),
  rights_status text not null default 'pending' check (
    rights_status = any (array['pending','cleared','not_applicable','blocked']::text[])
  ),
  ...
  constraint learning_resource_versions_resource_version_unique unique (resource_id, version),
  constraint learning_resource_versions_previous_not_self
    check (previous_version_id is null or previous_version_id <> id),
  constraint learning_resource_versions_certified_contract check (
    lifecycle_status <> 'certified' or (
      certified_at is not null
      and certification_policy_version is not null
      and btrim(certification_policy_version) <> ''
      and rights_status = any (array['cleared','not_applicable']::text[])
...
create unique index if not exists learning_resource_versions_one_inflight_uidx
  on public.learning_resource_versions(resource_id)
  where lifecycle_status = any (array['candidate','verified']::text[]);
...
begin
  if new.previous_version_id is null then
    if new.version <> 1 then
      raise exception using errcode='23514', message='CLA_VERSION_LINEAGE_INVALID',
        detail='Only version 1 may omit previous_version_id.';
    end if;
    return new;
  end if;

  select v.resource_id, v.version into parent_resource_id, parent_version
  from public.learning_resource_versions v where v.id = new.previous_version_id;

  if parent_resource_id is null then
    raise exception using errcode='23503', message='CLA_PREVIOUS_VERSION_NOT_FOUND';
  end if;
  if parent_resource_id <> new.resource_id then
    raise exception using errcode='23514', message='CLA_VERSION_LINEAGE_CROSS_RESOURCE';
  end if;
  if parent_version <> new.version - 1 then
    raise exception using errcode='23514', message='CLA_VERSION_LINEAGE_NOT_SEQUENTIAL';
  end if;
  return new;
end;
$$;
...
drop trigger if exists cla_validate_resource_version_lineage on public.learning_resource_versions;
create trigger cla_validate_resource_version_lineage
before insert or update of resource_id, version, previous_version_id
on public.learning_resource_versions for each row
...
drop trigger if exists cla_guard_resource_version_mutation on public.learning_resource_versions;
create trigger cla_guard_resource_version_mutation
before update or delete on public.learning_resource_versions
for each row execute function public.cla_guard_resource_version_mutation();
...
drop policy if exists learning_resource_versions_read_visible_parent on public.learning_resource_versions;
create policy learning_resource_versions_read_visible_parent
  on public.learning_resource_versions for select to authenticated
...
returns trigger language plpgsql set search_path = '' as $$
declare pinned_resource_id uuid;
begin
  if new.resource_version_id is null then return new; end if;
  select v.resource_id into pinned_resource_id
  from public.learning_resource_versions v where v.id = new.resource_version_id;
  if pinned_resource_id is null then
    raise exception using errcode='23503', message='CLA_RESOURCE_VERSION_PIN_NOT_FOUND';
  end if;
...
drop trigger if exists cla_validate_teaching_resource_version_pin on public.teaching_resource_links;
create trigger cla_validate_teaching_resource_version_pin
before insert or update of resource_id, resource_version_id
on public.teaching_resource_links for each row
execute function public.cla_validate_teaching_resource_version_pin();

create or replace function public.cla_get_certified_learning_resource(p_family_key text)
returns table (
  resource_id uuid,
...
language sql stable security invoker set search_path = '' as $$
  select r.id, v.id, v.version, r.canonical_key, r.asset_kind, r.purpose,
         v.payload_format, v.payload, v.content_sha256,
         v.certification_policy_version, v.certified_at
  from public.learning_resources r
  join public.learning_resource_versions v
    on v.resource_id = r.id and v.lifecycle_status = 'certified'
...
comment on table public.learning_resource_versions is
  'Immutable exact versions of reusable canonical learning resources. Only certified rows are selected by canonical lookup.';
comment on column public.learning_resources.asset_kind is
  'Educational artifact kind; separate from source_type provenance/origin.';
comment on column public.learning_resources.identity_key_version is
  'Version of the deterministic canonical family-key algorithm.';
comment on column public.teaching_resource_links.resource_version_id is
  'Exact canonical resource version used by this teaching occurrence/link.';
