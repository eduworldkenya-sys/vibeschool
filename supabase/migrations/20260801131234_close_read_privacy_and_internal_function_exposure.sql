...
do $policy$
begin
  -- These are privacy hardenings for legacy/pre-tracked product tables. A
  -- blank rebuild must not invent an absent product object just so the
  -- hardening migration can run; apply each policy only when its complete
  -- dependency set exists.
  if to_regclass('public.homework_answers') is not null
     ...
     and to_regclass('public.homework') is not null
