...
declare
  v_def text;
  v_old text := E'  if v_uid <> coalesce(v_class.teacher_id,v_uid) and not public.is_school_admin(v_class.school_id) then return jsonb_build_object(''ok'',false,''reason'',''not_authorized''); end if;';
