-- READ-004 refinement (review feedback) — dedupe learning_outcomes,
-- add curriculum authority/verified_by.
--
-- 1. Removed the top-level chapter.learning_outcomes key. It duplicated
--    curriculum.learning_outcomes (same underlying c.learning_outcomes
--    column, same fallback logic) with no client code reading the
--    top-level copy — curriculum.learning_outcomes is now the single
--    authoritative field, as requested.
--
-- 2. Added curriculum.authority, derived from alignment_status rather
--    than stored redundantly:
--      verified          -> 'official'
--      creator_claimed,
--      pending_review     -> 'publisher'
--      unclaimed, rejected -> null (no authority claim exists to report)
--    A 'community' tier was proposed but is deliberately not added here —
--    there is no community-contribution data model in this schema yet,
--    and inventing a value with nothing backing it would violate the
--    same honesty principle alignment_status itself exists to protect.
--    Add it in the same fix that actually builds community contributions.
--
-- 3. Added curriculum.verified_by (raw profiles.id, matches
--    vibe_chapters.verified_by) so a future "Verified by <name>" UI can
--    resolve it without another schema change.
...
begin
  ...
  pricing_type := coalesce(publication_row.pricing->>'type', 'free');

  free_chapter_count :=
    case
      when jsonb_typeof(publication_row.pricing->'freeChapters') = 'number'
      then greatest(0, (publication_row.pricing->>'freeChapters')::integer)
      else 0
    end;
