...
create table public.hq_workforce_competency_capabilities (
 id uuid primary key default gen_random_uuid(), competency_key text not null, skill_key text not null,
 version integer not null default 1 check(version>0), min_skill_version integer not null default 1 check(min_skill_version>0),
 required boolean not null default true, priority integer not null default 100 check(priority>=0),
 status text not null default 'draft' check(status in ('draft','approved','superseded','revoked')),
 approved_at timestamptz, created_at timestamptz not null default clock_timestamp(),
 unique(competency_key,skill_key,version), check(status<>'approved' or approved_at is not null)
);
...
create or replace function public.hq_workforce_resolve_capability_for_competency(p_competency_key text,p_scope_type text,p_jurisdiction text default 'global')
...
language sql security definer set search_path=public,pg_temp stable as $$
 ...
 where b.competency_key=p_competency_key and b.status='approved'
   ...
   and exists(select 1 from public.hq_workforce_skill_resources sr join public.hq_workforce_resources r on r.id=sr.resource_id
     where sr.skill_manifest_id=m.id and r.enabled and r.shadow_capable and r.health_status in ('healthy','degraded')
       and r.required_autonomy=0 and r.risk_class<=2 and (p_scope_type=any(r.allowed_scope_types) or 'global'=any(r.allowed_scope_types))
...
begin
 ...
 foreach comp in array o.required_competencies loop
  ...
  select rw.worker_key,rw.fit_score into v_worker,v_fit from public.hq_workforce_rank_workers_by_competency(array[comp],o.scope_type,o.jurisdiction,100) rw
  join public.hq_workforce_workers w on w.worker_key=rw.worker_key and w.status='active'
  where public.hq_workforce_current_lifecycle_state(rw.worker_key)='active' order by rw.fit_score desc,rw.worker_key limit 1;
  select * into cap from public.hq_workforce_resolve_capability_for_competency(comp,o.scope_type,o.jurisdiction);
  if v_worker is null or not found then v_missing:=array_append(v_missing,comp||case when v_worker is null then ':worker' else ':capability' end); continue; end if;
  ...
  v_max_risk:=greatest(v_max_risk,cap.risk_class); v_conf:=least(v_conf,coalesce(v_fit,0));
 ...
 if cardinality(v_workers)>1 then
   foreach v_worker in array v_workers loop
     if v_worker<>v_primary then insert into public.hq_workforce_collaborations(trace_id,plan_id,from_worker_key,to_worker_key,collaboration_type,requested_competencies,authority_snapshot,status)
       values(o.trace_id,v_plan,v_primary,v_worker,'consult',o.required_competencies,jsonb_build_object('autonomy',0,'risk',v_max_risk,'authority_transfer',false),'proposed'); v_collabs:=v_collabs+1; end if;
   end loop;
 end if;
