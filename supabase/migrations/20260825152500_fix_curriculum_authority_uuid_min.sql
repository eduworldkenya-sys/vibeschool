...
begin
  ...
  loop
    ...
    insert into public.curriculum_authority_hierarchy_bindings(
      ...
      v_source.grade,r.strand,r.sub_strand,v_action,v_artifact.source_url,v_snapshot.snapshot_sha256,
      v_artifact.content_sha256,v_owner,jsonb_build_object('pacing_authority',false,'term',null,'week',null,'source_version',v_artifact.source_version)
    );
...
comment on function public.curriculum_authority_bind_hierarchy(uuid) is
'Owner-gated exact, unpaced curriculum hierarchy binding with deterministic UUID selection.';
