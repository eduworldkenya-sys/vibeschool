...
begin
  ...
  -- Lock the canonical resource row so concurrent publication/sync events can
  -- never allocate the same version number twice.
  select *
  into v_resource
...
-- Publication/content changes keep the reusable snapshot cache synchronized.
...
-- Read only versions that are genuinely reusable. Certified versions follow
-- the existing visibility function. Unverified shared candidates must be exact
-- immutable snapshots of a currently published chapter resource.
...
-- Preserve the original owner/private-candidate rule, while adding one narrow
-- global case: an exact immutable snapshot of the current published chapter.
...
-- Existing published chapters become immediately reusable. This is idempotent:
-- the materializer reuses an existing snapshot with the same content hash.
do $$
declare
  v_chapter_id uuid;
begin
