...
-- Production repair: pgcrypto is intentionally installed in the extensions schema.
-- Keep the mission-start function's hardened search_path and qualify digest explicitly.
-- NON-ACTIVATING: no runtime, scheduler, publishing, payment, or authority state changes.
...
begin
  ...
  foreach v_worker_key in array array[
    ...
    exception when others then
      v_ready:=false;
      v_findings:=v_findings||jsonb_build_array(jsonb_build_object('worker_key',v_worker_key,'code',sqlerrm));
    end;
  end loop;
  ...
  insert into public.chemistry_worker_missions(
    mission_key,publication_id,state,worker_versions,curriculum_scope,runtime_posture,readiness_findings,started_by
  )
  values(
    v_key,
    p_publication_id,
    ...
    jsonb_build_object(
      ...
      'grade',p.cbc_grade,
      'publication_id',p.id,
      'all_artifact_types',true,
      'blast_radius_scan',true
    ),
    jsonb_build_object(
  ...
  on conflict(mission_key) do update set
    ...
    state=case
      when chemistry_worker_missions.state in ('COMPLETED','RUNNING','WAITING_HUMAN_REVIEW') then chemistry_worker_missions.state
      else excluded.state
    end,
  ...
  select
    ...
    case when v_ready
      then 'Create a fresh immutable certified-author draft from locked curriculum and evidence.'
...
'Creates or refreshes a bounded Grade 10 Chemistry shadow mission while runtime and scheduler remain off and Global Stop remains on. Uses schema-qualified pgcrypto hashing.';
