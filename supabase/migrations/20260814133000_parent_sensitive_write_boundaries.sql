...
-- Sensitive parent writes must derive parent identity from auth.uid().
...
begin
  ...
  if not exists (
    ...
    where parent_id = caller and student_id = p_student_id and coalesce(access_level, 'full') <> 'none'
  ) then raise exception 'Child access not authorized'; end if;
...
set search_path = public, pg_temp
as $$
declare caller uuid := auth.uid(); changed boolean;
begin
  if caller is null then raise exception 'Authentication required'; end if;
...
begin
  if caller is null then raise exception 'Authentication required'; end if;
  update public.health_vaccinations hv
     set deleted_at = now()
