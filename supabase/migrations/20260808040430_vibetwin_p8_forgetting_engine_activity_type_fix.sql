...
begin
  ...
  loop
    ...
    if not exists (
      ...
      values(v_student_id,v_candidate.due_date,v_candidate.subject,v_candidate.topic,v_activity_type,15,case v_candidate.lane_priority when 1 then 1 when 2 then 1 else 2 end,v_candidate.reason,'/student/vibelearn/revision','planned',jsonb_build_object('source','twin_forgetting_engine','lane',v_candidate.lane,'evidence_key',v_candidate.evidence_key,'outcome_id',v_candidate.outcome_id,'forgetting_risk',round(v_candidate.forgetting_risk,3),'days_since_evidence',v_candidate.age_days,'identity_bridge',case when v_candidate.lane in ('scheduled_retest','mistake_recovery') then 'canonical_plus_profile_history' else 'canonical_mastery' end));
      v_count:=v_count+1;
    end if;
  end loop;
  return v_count;
