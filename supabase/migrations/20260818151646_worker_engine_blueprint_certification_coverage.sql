...
begin
  ...
  select * into v_contract from public.hq_workforce_creation_contracts
   where id=p_creation_contract_id and worker_key=p_worker_key and status in ('issued','consumed')
     and (expires_at is null or expires_at>clock_timestamp());
  if not found then raise exception 'valid_creation_contract_required_for_certification'; end if;

  select * into v_blueprint from public.hq_workforce_blueprints where id=v_contract.blueprint_id and status='approved';
  if not found then raise exception 'approved_blueprint_required_for_certification'; end if;
  if jsonb_typeof(v_blueprint.required_capabilities)<>'array' or jsonb_array_length(v_blueprint.required_capabilities)=0 then
    raise exception 'blueprint_required_capabilities_missing';
  end if;

  select coalesce(max(issued_at),'-infinity'::timestamptz) into v_since
  from public.hq_workforce_certifications where worker_key=p_worker_key;

  select count(*) into v_passed
  from public.hq_workforce_shadow_runs sr
  where sr.worker_key=p_worker_key and sr.passed and not sr.side_effects_applied
    and sr.verifier_key=p_verifier_key and sr.execution_method='server_shadow_executor_v2'
  ...
  select array_agg(req.capability_key order by req.capability_key) into v_missing
  from (select jsonb_array_elements_text(v_blueprint.required_capabilities) capability_key) req
  where not exists (
    select 1 from public.hq_workforce_shadow_runs sr
    join public.hq_workforce_tool_contracts tc on tc.id=sr.tool_contract_id and tc.status='approved'
    where sr.worker_key=p_worker_key and sr.passed and not sr.side_effects_applied
      and sr.verifier_key=p_verifier_key and sr.execution_method='server_shadow_executor_v2'
      and sr.executed_at>v_since and tc.required_capability_key=req.capability_key
  );
...
create or replace function public.hq_workforce_authorize_model_call(
  p_worker_key text,p_task_id uuid,p_reason_code text,p_failure_evidence jsonb,p_model_key text,p_token_budget bigint
)
returns uuid
language plpgsql
security definer
set search_path=public,pg_temp
as $$
declare v_id uuid; v_budget uuid; t public.hq_workforce_task_contracts%rowtype; w public.hq_workforce_workers%rowtype;
begin
  ...
  insert into public.hq_workforce_model_invocations(
    worker_key,task_id,reason_code,deterministic_attempted,deterministic_failure_evidence,model_key,token_budget,status,budget_id
  ) values(p_worker_key,p_task_id,p_reason_code,true,p_failure_evidence,p_model_key,p_token_budget,'authorized',v_budget)
  returning id into v_id;
  return v_id;
end $$;
