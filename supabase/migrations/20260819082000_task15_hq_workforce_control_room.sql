...
-- NON-ACTIVATING. This migration installs owner controls and read models only.
-- It deliberately leaves runtime OFF, autonomy L0, risk R0 and Global Stop unchanged.

create table if not exists public.hq_workforce_owner_control_events (
  id bigint generated always as identity primary key,
  action_key text not null check (action_key in ('start_controlled_operations','stop_operations','global_stop_activate','global_stop_release','runtime_policy_change','authority_change','decision_review','breaker_reset')),
  ...
  select jsonb_build_object(
    ...
    'global_policy',(
      select to_jsonb(p) from (
        select id,policy_key,enabled,max_autonomy_level,max_risk_class,max_concurrency,max_executions_per_minute,reason,updated_at
    ...
    'authority',coalesce((
      ...
      from (
        select id,grant_key,capability_key,capability_version,permitted_worker_key,operation,resource_type,scope_type,scope_ref,
               autonomy_level,risk_class,max_operations_per_cycle,max_records_per_operation,max_concurrency,max_executions_per_minute,
               status,issued_at,certified_at,activated_at,expires_at,revoked_at,lifecycle_reason
    ...
    'executions',coalesce((
      ...
      from (
        select id,task_key,worker_key,capability_key,operation,resource_type,scope_type,status,attempt_count,last_error,
               autonomous_authority_grant_id,created_at,started_at,completed_at
        from public.hq_workforce_task_contracts
        order by created_at desc limit lim
      ) t
    ),'[]'::jsonb),
    'budgets',coalesce((
      select jsonb_agg(to_jsonb(b) order by b.window_started_at desc)
      from (
        select id,worker_key,budget_key,limit_amount,reserved_amount,consumed_amount,status,window_started_at,window_ends_at
        from public.hq_workforce_execution_budgets
        order by window_started_at desc limit lim
      ) b
    ),'[]'::jsonb),
    ...
    'counts',jsonb_build_object(
      'active_authority',(select count(*) from public.hq_workforce_capability_authority_grants where status='active' and coalesce(activated_at,issued_at)<=clock_timestamp() and expires_at>clock_timestamp()),
      'expiring_authority',(select count(*) from public.hq_workforce_capability_authority_grants where status='active' and expires_at>clock_timestamp() and expires_at<=clock_timestamp()+interval '60 minutes'),
...
begin
  ...
  if not found then raise exception 'runtime_contract_missing'; end if;
  v_previous:=jsonb_build_object('runtime_execution_enabled',ec.runtime_execution_enabled,'runtime_autonomy_level',ec.runtime_autonomy_level,'runtime_max_risk',ec.runtime_max_risk,'updated_at',ec.updated_at);

  if ec.runtime_execution_enabled and ec.runtime_autonomy_level=p_autonomy_level and ec.runtime_max_risk=p_max_risk then
    insert into public.hq_workforce_owner_control_events(action_key,actor_id,previous_state,requested_state,result_state,outcome,reason)
    values('start_controlled_operations',v_uid,v_previous,jsonb_build_object('runtime_execution_enabled',true,'runtime_autonomy_level',p_autonomy_level,'runtime_max_risk',p_max_risk),v_previous,'idempotent',btrim(p_reason));
  ...
  insert into public.hq_workforce_owner_control_events(action_key,actor_id,previous_state,requested_state,result_state,outcome,reason)
  values('start_controlled_operations',v_uid,v_previous,jsonb_build_object('runtime_execution_enabled',true,'runtime_autonomy_level',p_autonomy_level,'runtime_max_risk',p_max_risk),v_result,'succeeded',btrim(p_reason));
  return v_result;
end $$;
...
begin
  ...
  for g in select id from public.hq_workforce_capability_authority_grants where status='active' order by id for update loop
    perform public.hq_workforce_owner_transition_capability_authority(g.id,'suspend',p_reason,jsonb_build_array(jsonb_build_object('source','hq_control_room','action','stop_operations')));
  end loop;
  insert into public.hq_workforce_owner_control_events(action_key,actor_id,previous_state,requested_state,result_state,outcome,reason)
  values('stop_operations',v_uid,v_previous,jsonb_build_object('runtime_execution_enabled',false,'runtime_autonomy_level',0,'runtime_max_risk',0),v_result,'succeeded',btrim(p_reason));
  return v_result;
end $$;
...
begin
  ...
  select * into ec from public.hq_workforce_engine_contract where singleton=true for update;
  if not found then raise exception 'runtime_contract_missing'; end if;
  v_previous:=jsonb_build_object('shadow_global_stop',ec.shadow_global_stop,'runtime_execution_enabled',ec.runtime_execution_enabled,'runtime_autonomy_level',ec.runtime_autonomy_level,'runtime_max_risk',ec.runtime_max_risk);

  if coalesce(p_active,false) then
    update public.hq_workforce_engine_contract set runtime_execution_enabled=false,runtime_autonomy_level=0,runtime_max_risk=0,heartbeat_enabled=false,factory_enabled=false,shadow_enabled=false,shadow_scheduler_enabled=false,shadow_global_stop=true,updated_at=clock_timestamp() where singleton=true;
    v_breaker:=public.hq_workforce_trip_execution_breaker('global','global','owner_global_stop','hq-owner:'||v_uid::text,jsonb_build_object('reason',btrim(p_reason)));
    for g in select id from public.hq_workforce_capability_authority_grants where status='active' order by id for update loop
      perform public.hq_workforce_owner_transition_capability_authority(g.id,'suspend',p_reason,jsonb_build_array(jsonb_build_object('source','hq_control_room','action','global_stop')));
    end loop;
    insert into public.hq_workforce_owner_control_events(action_key,actor_id,previous_state,requested_state,result_state,outcome,reason)
    values('global_stop_activate',v_uid,v_previous,jsonb_build_object('global_stop',true),jsonb_build_object('global_stop',true,'runtime_execution_enabled',false,'breaker_id',v_breaker),'succeeded',btrim(p_reason));
    return jsonb_build_object('global_stop',true,'runtime_execution_enabled',false,'breaker_id',v_breaker);
  end if;

  update public.hq_workforce_engine_contract set shadow_global_stop=false,updated_at=clock_timestamp() where singleton=true;
  for b in select id from public.hq_workforce_execution_breakers where scope_type='global' and scope_ref='global' and status='tripped' order by created_at for update loop
    perform public.hq_workforce_reset_execution_breaker(b.id,'hq-owner:'||v_uid::text,p_reason,jsonb_build_object('source','hq_control_room','authority_effect','none'));
  end loop;
  insert into public.hq_workforce_owner_control_events(action_key,actor_id,previous_state,requested_state,result_state,outcome,reason)
  values('global_stop_release',v_uid,v_previous,jsonb_build_object('global_stop',false),jsonb_build_object('global_stop',false,'runtime_execution_enabled',false,'authority_reactivated',false),'succeeded',btrim(p_reason));
...
-- Installation cannot activate the engine or grant authority.
do $$
