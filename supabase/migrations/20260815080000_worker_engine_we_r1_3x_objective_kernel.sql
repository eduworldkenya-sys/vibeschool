...
-- Compatibility bridge: existing HQ work items may be attached to an objective without changing their current lifecycle.
...
-- Append-only objective history. Objective rows may evolve through governed lifecycle; history records why.
...
-- Service-only creation primitive. Detection/planning callers must provide provenance; no implicit authority is granted.
...
begin
  ...
  insert into public.hq_workforce_objectives(
    objective_key,parent_objective_id,source_type,source_ref,desired_outcome,scope_type,scope_ref,
    constraints,success_criteria,evidence_requirements,priority,risk_class,sla_due_at,provenance
  ) values(
    btrim(p_objective_key),p_parent_objective_id,btrim(p_source_type),nullif(btrim(coalesce(p_source_ref,'')),''),btrim(p_desired_outcome),
    coalesce(nullif(btrim(p_scope_type),''),'platform_internal'),p_scope_ref,p_constraints,p_success_criteria,p_evidence_requirements,
    p_priority,p_risk_class,p_sla_due_at,p_provenance
...
create or replace function public.hq_workforce_link_objective_work_item(
  p_objective_id uuid,
  p_work_item_id uuid,
  p_relationship text default 'source'
) returns void
language plpgsql security definer set search_path=public,pg_temp as $$
begin
  ...
  insert into public.hq_workforce_objective_work_items(objective_id,work_item_id,relationship)
  values(p_objective_id,p_work_item_id,p_relationship)
...
-- Strict transition function. Approval does not execute production action.
...
begin
  ...
  v_event := case p_to_status
    when 'context_pending' then 'context_requested'
    when 'planning' then 'planning_started'
    when 'shadow_ready' then 'shadow_ready'
    when 'awaiting_review' then 'review_requested'
  ...
  update public.hq_workforce_objectives set
    status=p_to_status,
...
-- Migration-time fail-closed assertion: X1 may not activate any runtime.
do $$
...
begin
  ...
  if coalesce(ec.heartbeat_enabled,false) or coalesce(ec.factory_enabled,false) or coalesce(ec.runtime_execution_enabled,false)
    ...
    raise exception 'WE-R1.3X X1 violated fail-closed runtime boundary';
  end if;
end $$;
