...
create table if not exists public.pathway_sources (
  ...
  source_kind text not null check (source_kind in ('official_portal','official_document','professional_body','institution_verified','editorial_guidance')),
...
create table if not exists public.pathway_source_observations (
  ...
  entity_kind text not null check (entity_kind in ('pathway','track','subject_combination','career','school_offering')),
  observed_payload jsonb not null,
  observed_at timestamptz not null,
  content_checksum text not null,
  resolution_state text not null default 'unresolved' check (resolution_state in ('unresolved','resolved','disputed','retired')),
  resolved_entity_id uuid,
  created_at timestamptz not null default now(),
  unique(source_id, external_key, content_checksum)
);
...
create table if not exists public.pathways (
  ...
  source_id uuid not null references public.pathway_sources(id),
  verification_state text not null default 'unverified' check (verification_state in ('verified','unverified','disputed','stale','retired')),
...
create table if not exists public.pathway_combination_subject_claims (
  ...
  subject_code text,
  subject_order smallint not null check(subject_order > 0),
  source_id uuid not null references public.pathway_sources(id),
  resolved_subject_id uuid references public.subjects(id),
  resolution_state text not null default 'unresolved' check (resolution_state in ('unresolved','resolved','ambiguous','disputed')),
  created_at timestamptz not null default now(),
  primary key(combination_id, subject_order)
);
...
create table if not exists public.pathway_career_links (
  ...
  pathway_id uuid not null references public.pathways(id) on delete cascade,
  track_id uuid references public.pathway_tracks(id),
  relationship_type text not null check (relationship_type in ('official','professional_requirement','guidance')),
...
create table if not exists public.pathway_profile_passports (
  ...
  pathway_id uuid not null references public.pathways(id),
  source_decision_id uuid not null references public.pathway_profile_decisions(id),
  rule_version text not null,
  evidence_snapshot jsonb not null default '{}'::jsonb,
