...
alter table public.vibe_reading_sessions
...
where r.student_id is null
  and s.profile_id = r.viewer_id
  and s.deleted_at is null;

alter table public.vibe_reading_sessions
  drop constraint if exists vibe_reading_sessions_student_id_fkey;
alter table public.vibe_reading_sessions
  add constraint vibe_reading_sessions_student_id_fkey
  foreign key (student_id) references public.students(id) on delete set null;
create index if not exists vibe_reading_sessions_student_time_idx
  on public.vibe_reading_sessions(student_id, started_at desc)
  where student_id is not null;

alter table public.content_learning_events
  add column if not exists account_user_id uuid;

-- Preserve every historical account UUID before changing student_id semantics.
update public.content_learning_events
set account_user_id = student_id
where account_user_id is null;

alter table public.content_learning_events
  drop constraint if exists content_learning_events_student_id_fkey;

-- Canonicalize only deterministic active student mappings. Unmapped historical
-- account events remain attributable through account_user_id and are excluded
-- from learner intelligence until a canonical learner can be proven.
update public.content_learning_events e
set student_id = s.id
from public.students s
where s.profile_id = e.account_user_id
  and s.deleted_at is null;

update public.content_learning_events e
set student_id = null
where not exists (
  select 1 from public.students s where s.id = e.student_id and s.deleted_at is null
);

alter table public.content_learning_events
  alter column student_id drop not null;
alter table public.content_learning_events
  add constraint content_learning_events_student_id_fkey
  foreign key (student_id) references public.students(id) on delete set null;
alter table public.content_learning_events
  drop constraint if exists content_learning_events_account_user_id_fkey;
alter table public.content_learning_events
  add constraint content_learning_events_account_user_id_fkey
...
create index if not exists content_learning_events_account_time_idx
  on public.content_learning_events(account_user_id, occurred_at desc)
  where account_user_id is not null;

-- Authenticated clients no longer insert arbitrary learning-event rows. The
-- RPC below owns identity resolution and lineage checks.
...
create policy content_learning_events_account_read
on public.content_learning_events
...
begin
  ...
  insert into public.content_learning_events(
    student_id,account_user_id,publication_id,chapter_id,content_block_id,
    outcome_id,event_type,duration_ms,metadata
  ) values(
    v_student_id,v_account_user_id,p_publication_id,p_chapter_id,p_content_block_id,
    p_outcome_id,p_event_type,p_duration_ms,coalesce(p_metadata,'{}'::jsonb)
...
-- Reading sessions remain account-scoped for entitlement/session ownership,
-- while student_id is captured separately whenever the reader is a learner.
...
begin
  ...
  insert into public.vibe_reading_sessions(
    ...
    v_viewer,v_student_id,v_publication_id,p_chapter_id,p_client_session_id,
  ...
  return jsonb_build_object('ok',true,'session_id',v_session.id,
    'active_seconds',v_session.active_seconds,'max_progress_percent',v_session.max_progress_percent,
    'completed',v_session.completed_at is not null);
end
$$;

-- Project reading evidence only for proven canonical learners. Account-only
-- readers retain session analytics but cannot contaminate learner intelligence.
...
begin
  ...
  select outcome_id into v_outcome
  from public.chapter_learning_outcome_links
...
-- Content intelligence must consume learner-domain evidence only.
...
begin
 ...
 with assessment_health as (
  select b.publication_id,b.chapter_id,b.id block_id,count(r.id)::int evidence_count,
    avg(case when coalesce(r.max_score,0)>0 then coalesce(r.final_score,r.teacher_score,r.auto_score,0)/r.max_score else null end) avg_ratio
  ...
  select coalesce(a.publication_id,e.publication_id) publication_id,coalesce(a.chapter_id,e.chapter_id) chapter_id,
    ...
    case when coalesce(e.total_events,0)>0 then e.weak_events::numeric/e.total_events else null end weak_ratio
...
begin
  ...
  if exists(
    select 1 from public.content_learning_events e
    where e.student_id is not null and not exists(select 1 from public.students s where s.id=e.student_id)
  ) then raise exception 'content_learning_events_noncanonical_student_remaining'; end if;
  if exists(
    select 1 from public.vibe_reading_sessions r
    where r.student_id is not null and not exists(select 1 from public.students s where s.id=r.student_id)
  ) then raise exception 'reading_sessions_noncanonical_student_remaining'; end if;
  if has_table_privilege('authenticated','public.content_learning_events','INSERT') then
    raise exception 'content_learning_events_direct_insert_still_granted';
  end if;
end
$$;
