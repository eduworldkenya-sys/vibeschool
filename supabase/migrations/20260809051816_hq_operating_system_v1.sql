...
begin
  ...
  case p_event_type
    when 'user.signup' then
      v_category := 'growth';
      v_title := 'New signup';
      v_body := coalesce(p_metadata->>'role', 'User') || case when nullif(p_metadata->>'name','') is not null then ' · ' || (p_metadata->>'name') else '' end;
    ...
    when 'lesson_plan.created' then
      v_category := 'teaching';
      v_title := 'Lesson plan created';
      v_body := coalesce(p_metadata->>'title', 'New lesson plan');
      v_route := '/hq?view=lesson-plans';
    when 'lesson_plan.published' then
      v_category := 'teaching';
      v_severity := 'success';
      v_title := 'Lesson plan published';
      v_body := coalesce(p_metadata->>'title', 'Lesson plan published');
      v_route := '/hq?view=lesson-plans';
    ...
    when 'homework.created' then
      v_category := 'teaching';
      v_title := 'Homework assigned';
      v_body := coalesce(p_metadata->>'title', 'New homework');
      v_route := '/hq?view=homework';
    when 'homework.submitted' then
      v_category := 'learning';
      v_should_notify := false;
    when 'publication.created' then
      v_category := 'content';
      v_title := 'Publication draft created';
      v_body := coalesce(p_metadata->>'title', 'New publication');
...
create or replace function public.hq_profile_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    perform public.hq_emit_event(
      'user.signup', new.id, coalesce(new.role,'user'), new.school_id,
      'profile', new.id,
      jsonb_build_object('role', coalesce(new.role,'user'), 'name', coalesce(new.full_name,''))
    );
  end if;
  return new;
end;
...
create or replace function public.hq_school_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    perform public.hq_emit_event('school.created', new.created_by, 'admin', new.id, 'school', new.id,
      jsonb_build_object('name', coalesce(new.name,''), 'county', coalesce(new.county,''), 'status', new.status::text));
  end if;
  return new;
end;
$$;
...
create or replace function public.hq_lesson_plan_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    v_event := 'lesson_plan.created';
  elsif old.published_at is null and new.published_at is not null then
    v_event := 'lesson_plan.published';
  elsif old.taught_date is null and new.taught_date is not null then
    v_event := 'lesson_plan.completed';
  else
    return new;
  end if;

  perform public.hq_emit_event(v_event, new.teacher_id, 'teacher', new.school_id, 'lesson_plan', new.id,
    jsonb_build_object('title', coalesce(new.title,''), 'status', coalesce(new.status,''), 'term', new.term, 'generated_by', coalesce(new.generated_by,'')));
  return new;
end;
$$;
...
create or replace function public.hq_homework_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.hq_emit_event('homework.created', new.teacher_id, 'teacher', new.school_id, 'homework', new.id,
    jsonb_build_object('title', coalesce(new.title,''), 'subject', coalesce(new.subject,''), 'type', coalesce(new.type,''), 'due_date', new.due_date));
  return new;
end;
$$;
...
create or replace function public.hq_homework_submission_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  perform public.hq_emit_event('homework.submitted', new.student_id, 'student', v_school_id, 'homework_submission', new.id,
    jsonb_build_object('homework_id', new.homework_id, 'status', coalesce(new.status,'')));
  return new;
end;
$$;
...
create or replace function public.hq_publication_event_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    v_event := 'publication.created';
  elsif coalesce(old.status,'') <> 'published' and new.status = 'published' then
    v_event := 'publication.published';
  else
    return new;
  end if;
  perform public.hq_emit_event(v_event, new.author_id, 'author', null, 'publication', new.id,
    jsonb_build_object('title', coalesce(new.title,''), 'format', coalesce(new.format,''), 'status', coalesce(new.status,''), 'cbc_subject', coalesce(new.cbc_subject,''), 'cbc_grade', coalesce(new.cbc_grade,'')));
  return new;
end;
$$;
...
create or replace function public.hq_get_snapshot()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select jsonb_build_object(
    ...
    'users', jsonb_build_object(
      'total', (select count(*) from public.profiles where coalesce(is_anonymized,false) = false),
      ...
      'learners', (select count(*) from public.student_profiles)
    ),
    'schools', jsonb_build_object(
      'total', (select count(*) from public.schools where deleted_at is null),
      'active', (select count(*) from public.schools where deleted_at is null and status::text = 'active'),
      'today', (select count(*) from public.schools where deleted_at is null and created_at >= date_trunc('day', now()))
    ),
    'teaching', jsonb_build_object(
      'lesson_plans_today', (select count(*) from public.lesson_plans where created_at >= date_trunc('day', now())),
      'lesson_plans_7d', (select count(*) from public.lesson_plans where created_at >= now() - interval '7 days'),
      'lessons_taught_today', (select count(*) from public.lesson_plans where taught_date = current_date),
      'homework_today', (select count(*) from public.homework where created_at >= date_trunc('day', now())),
      'submissions_today', (select count(*) from public.homework_submissions where submitted_at >= date_trunc('day', now())),
      'unreviewed_submissions', (select count(*) from public.homework_submissions where reviewed_at is null)
    ),
    'content', jsonb_build_object(
      'publications_total', (select count(*) from public.vibe_publications),
      'publications_live', (select count(*) from public.vibe_publications where status = 'published'),
      'publications_draft', (select count(*) from public.vibe_publications where status = 'draft'),
      'reads_total', (select coalesce(sum(total_reads),0) from public.vibe_publications)
    ),
    'events', jsonb_build_object(
      'today', (select count(*) from public.platform_events where occurred_at >= date_trunc('day', now())),
      'last_hour', (select count(*) from public.platform_events where occurred_at >= now() - interval '1 hour')
    ),
    'notifications', jsonb_build_object(
      'unread', (select count(*) from public.hq_notifications where status = 'unread'),
      'critical', (select count(*) from public.hq_notifications where status <> 'resolved' and severity = 'critical')
    ),
    'incidents', jsonb_build_object(
      'open', (select count(*) from public.hq_incidents where status <> 'resolved')
    )
  ) into v_result;
...
begin
  perform public.hq_assert_owner();
  return query
  select n.id, n.category, n.severity, n.title, n.body, n.route, n.status, n.metadata, n.created_at
...
create or replace function public.hq_mark_notification_read(p_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
...
create or replace function public.hq_resolve_notification(p_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
