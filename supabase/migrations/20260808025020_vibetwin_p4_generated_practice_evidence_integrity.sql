...
as $function$
declare
  v_profile_id uuid:=auth.uid(); v_student_id uuid; v_q public.student_generated_practice_questions%rowtype; v_correct boolean; v_event_id uuid; v_mistake_id uuid; v_mastery_after numeric; v_effective_after numeric; v_forgetting_after numeric; v_brain jsonb; v_item jsonb; v_misconception jsonb:='{}'::jsonb; v_intervention jsonb:='{}'::jsonb; v_intervention_type text; v_intervention_key text;
begin
  ...
  perform public.twin_record_verified_practice_effect(v_profile_id,v_q.outcome_id,v_intervention_type,v_intervention_key,v_correct,p_response_ms,jsonb_build_object('source','adaptive_generated_practice','generated_question_id',v_q.id,'difficulty',v_q.difficulty,'misconception',v_misconception,'evidence_authority','behavioral_only'));
