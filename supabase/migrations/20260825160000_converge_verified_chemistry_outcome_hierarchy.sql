...
begin
  ...
  select (ci.payload->>'curriculum_authority_snapshot_id')::uuid into v_snapshot_id
  from public.curriculum_imports ci where ci.id=v_import_id;
  ...
  if (select count(*) from public.curriculum_authority_hierarchy_bindings h where h.snapshot_id=v_snapshot_id)<>7 then raise exception 'EXACT_SEVEN_HIERARCHY_BINDINGS_REQUIRED'; end if;
  if (select count(*) from public.curriculum_authority_reconciliation r where r.snapshot_id=v_snapshot_id)<>32
     or (select count(*) from public.curriculum_authority_reconciliation r where r.snapshot_id=v_snapshot_id and r.classification='missing_outcome')<>32 then raise exception 'EXPECTED_32_MISSING_OUTCOMES_REQUIRED'; end if;
  if (select count(*) from public.curriculum_learning_outcomes clo where clo.source_import_id=v_import_id and clo.source_type='official' and clo.status='verified' and clo.outcome_code like 'CHEM-G10-%' and clo.sub_strand_id is null)<>32 then raise exception 'VERIFIED_32_OUTCOME_COHORT_REQUIRED'; end if;
  ...
  if exists(
    ...
    left join public.curriculum_learning_outcomes clo
      on clo.source_import_id=v_import_id and clo.source_type='official' and clo.status='verified'
    ...
    left join public.curriculum_authority_hierarchy_bindings h on h.snapshot_id=v_snapshot_id and h.sub_strand_id=r.target_sub_strand_id
    where r.snapshot_id=v_snapshot_id and (r.target_sub_strand_id is null or clo.id is null or h.sub_strand_id is null)
  ...
  update public.curriculum_authority_snapshots set status='sealed',reconciled_at=null,updated_at=clock_timestamp() where id=v_snapshot_id;
end $$;
