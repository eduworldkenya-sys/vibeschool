...
begin
  ...
  if not found then
    raise exception 'PATHWAY_OBSERVATION_NOT_FOUND';
  end if;

  if v_obs.entity_kind <> 'school_offering' then
    raise exception 'PATHWAY_OBSERVATION_WRONG_KIND';
  end if;

  if v_obs.resolution_state <> 'resolved' or v_obs.resolved_entity_id is null then
    raise exception 'PATHWAY_OBSERVATION_NOT_RESOLVED';
  end if;

  v_school_id := v_obs.resolved_entity_id;

  if not exists (select 1 from public.schools where id = v_school_id) then
    raise exception 'PATHWAY_OBSERVATION_RESOLVED_ENTITY_NOT_SCHOOL';
  end if;
  ...
  if v_pathway_name is null then
    raise exception 'PATHWAY_OBSERVATION_MISSING_PATHWAY';
  end if;

  select id into v_pathway_id
  from public.pathways
  where lower(name) = lower(v_pathway_name)
  ...
  if v_pathway_id is null then
    raise exception 'PATHWAY_NOT_VERIFIED_OR_NOT_FOUND';
  end if;

  if v_track_name is not null then
    select id into v_track_id
    from public.pathway_tracks
    where pathway_id = v_pathway_id
      and lower(name) = lower(v_track_name)
      and verification_state = 'verified'
      and status = 'published'
    order by created_at
    limit 1;
    ...
    if v_track_id is null then
      raise exception 'PATHWAY_TRACK_NOT_VERIFIED_OR_NOT_FOUND';
    end if;
  end if;

  if v_combination_name is not null then
    select id into v_combination_id
    from public.pathway_subject_combinations
    where pathway_id = v_pathway_id
      and (v_track_id is null or track_id = v_track_id)
      and lower(display_name) = lower(v_combination_name)
      and verification_state = 'verified'
      and status = 'published'
    order by created_at
    limit 1;

    if v_combination_id is null then
      raise exception 'PATHWAY_COMBINATION_NOT_VERIFIED_OR_NOT_FOUND';
    end if;
  end if;

  insert into public.pathway_school_offerings(
    school_id,pathway_id,track_id,combination_id,source_id,source_observation_id,
    verification_state,observed_at,verified_at,effective_from,effective_to
  ) values (
    v_school_id,v_pathway_id,v_track_id,v_combination_id,v_obs.source_id,v_obs.id,
    'verified',v_obs.observed_at,now(),null,null
  )
  ...
  do update set
    ...
    verified_at = coalesce(public.pathway_school_offerings.verified_at, now()),
    updated_at = now()
