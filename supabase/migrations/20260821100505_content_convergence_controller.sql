...
create table if not exists public.content_convergence_runs (
  ...
  state text not null check (state in (
    'DRAFT','AUTHORED','MEASURING','MEASURED','CRITIC_REVIEW','REPAIR_REQUIRED',
    'REPAIRING','REPAIRED','REVERIFYING','CONVERGED','ESCALATED',
    'RELEASE_CANDIDATE','RELEASE_APPROVED','PUBLISHED','REJECTED','SUPERSEDED'
  )),
...
create table if not exists public.content_convergence_findings (
  ...
  state text not null check (state in (
    'OPEN','REPAIR_REQUESTED','REPAIR_ATTEMPTED','REVERIFYING',
    'VERIFIED_RESOLVED','STILL_PRESENT','REGRESSED','SUPERSEDED','ESCALATED'
  )),
...
begin
  ...
  if not exists (select 1 from public.vibe_publications where id=p_publication_id) then
  ...
  if p_mode <> 'shadow' and exists (
    select 1 from public.hq_workforce_engine_contract
    where singleton=true and (shadow_global_stop is true or runtime_execution_enabled is not true)
  ) then raise exception 'WORKER_ENGINE_GLOBAL_STOP_OR_RUNTIME_OFF'; end if;

  insert into public.content_convergence_versions(
    publication_id,version_number,content_hash,content_snapshot,worker_key,worker_execution_id,
    provenance,curriculum_identity,evaluation_lineage
  ) values (
    p_publication_id,1,p_content_hash,coalesce(p_content_snapshot,'{}'::jsonb),p_worker_key,p_worker_execution_id,
    coalesce(p_provenance,'{}'::jsonb),coalesce(p_curriculum_identity,'{}'::jsonb),'[]'::jsonb
  )
  on conflict (publication_id,content_hash) do update set content_hash=excluded.content_hash
  returning id into v_version;

  insert into public.content_convergence_runs(
    publication_id,current_version_id,state,idempotency_key,mode
  ) values (p_publication_id,v_version,'AUTHORED',p_idempotency_key,p_mode)
  returning id into v_run;

  insert into public.content_convergence_events(
    run_id,from_state,to_state,actor,execution_id,artifact_version_id,artifact_hash,reason,evidence
  ) values (
    v_run,'DRAFT','AUTHORED',p_worker_key,p_worker_execution_id,v_version,p_content_hash,
    'convergence run initialized',jsonb_build_object('mode',p_mode)
  );
  return v_run;
end $$;

create or replace function public.content_convergence_transition(
  p_run_id uuid,
...
) returns public.content_convergence_runs
language plpgsql
security definer
set search_path = ''
as $$
...
begin
  select * into v_run
  from public.content_convergence_runs
  where id = p_run_id
  for update;
  ...
  if p_to_state in ('REPAIRING','RELEASE_CANDIDATE') and exists (
    select 1 from public.hq_workforce_engine_contract
    where singleton=true
      and (shadow_global_stop is true or runtime_execution_enabled is not true)
  ) then
    raise exception 'WORKER_ENGINE_GLOBAL_STOP_OR_RUNTIME_OFF';
  end if;
  ...
  v_allowed := case
    ...
    when p_expected_state='RELEASE_CANDIDATE' and p_to_state in ('ESCALATED','REJECTED','SUPERSEDED') then true
    -- RELEASE_APPROVED/PUBLISHED are observations owned by the existing human publication authority, never by P5.
    else false end;
  if not v_allowed then raise exception 'ILLEGAL_CONVERGENCE_TRANSITION:%->%', p_expected_state, p_to_state; end if;

  update public.content_convergence_runs
  set state=p_to_state, decision_reason=p_reason, updated_at=now()
  where id=p_run_id
  ...
  insert into public.content_convergence_events(
    ...
    p_run_id,p_expected_state,p_to_state,p_actor,p_execution_id,p_expected_version_id,p_expected_hash,p_reason,coalesce(p_evidence,'{}'::jsonb)
  );

  return v_run;
end $$;

create or replace function public.content_convergence_create_repair_version(
  p_run_id uuid,
  ...
  p_worker_key text,
  p_worker_execution_id text,
  p_repair_reasons jsonb,
  p_findings_addressed jsonb,
  p_findings_preserved jsonb,
  p_provenance jsonb,
  p_curriculum_identity jsonb
) returns uuid
language plpgsql
...
declare
  v_run public.content_convergence_runs;
  v_parent public.content_convergence_versions;
  v_new_id uuid;
begin
  ...
  if v_parent.curriculum_identity is distinct from coalesce(p_curriculum_identity,'{}'::jsonb) then
    raise exception 'CURRICULUM_IDENTITY_MUTATION_BLOCKED';
  end if;
  if coalesce(p_content_hash,'') = p_parent_hash then raise exception 'NO_MEANINGFUL_VERSION_CHANGE'; end if;

  insert into public.content_convergence_versions(
    publication_id,parent_version_id,version_number,content_hash,content_snapshot,
    worker_key,worker_execution_id,repair_reasons,findings_addressed,findings_preserved,
    provenance,curriculum_identity,evaluation_lineage
  ) values (
    v_run.publication_id,p_parent_version_id,v_parent.version_number+1,p_content_hash,coalesce(p_content_snapshot,'{}'::jsonb),
    p_worker_key,p_worker_execution_id,coalesce(p_repair_reasons,'[]'::jsonb),coalesce(p_findings_addressed,'[]'::jsonb),
    coalesce(p_findings_preserved,'[]'::jsonb),coalesce(p_provenance,'{}'::jsonb),coalesce(p_curriculum_identity,'{}'::jsonb),'[]'::jsonb
  ) returning id into v_new_id;

  update public.content_convergence_runs
  set current_version_id=v_new_id, attempt_count=attempt_count+1, state='REPAIRED',
  ...
  insert into public.content_convergence_events(
    ...
    p_run_id,'REPAIRING','REPAIRED',p_worker_key,p_worker_execution_id,v_new_id,p_content_hash,'immutable repair version created',
    jsonb_build_object('parent_version_id',p_parent_version_id,'parent_hash',p_parent_hash)
  );

  return v_new_id;
end $$;

create or replace function public.content_convergence_record_evaluation(
  p_run_id uuid,
  p_version_id uuid,
  p_stage text,
  p_worker_execution_id text,
...
begin
  ...
  insert into public.content_convergence_evaluations(
    run_id,version_id,stage,worker_execution_id,content_hash,disposition,quality_score,
    dimensions,findings,safety_status,assessment_status,provenance_status
  ) values (
    p_run_id,p_version_id,p_stage,p_worker_execution_id,p_content_hash,p_disposition,p_quality_score,
    coalesce(p_dimensions,'{}'::jsonb),coalesce(p_findings,'[]'::jsonb),coalesce(p_safety_status,'UNKNOWN'),
    coalesce(p_assessment_status,'UNKNOWN'),coalesce(p_provenance_status,'UNKNOWN')
  )
  on conflict (run_id,version_id,stage,worker_execution_id) do update
...
declare
  v_run public.content_convergence_runs;
  v_version public.content_convergence_versions;
  v_p2 public.content_convergence_evaluations;
  v_p3 public.content_convergence_evaluations;
  v_critical integer;
  v_severe_regression boolean;
  v_decision text;
  v_reason text;
begin
  select * into v_run from public.content_convergence_runs where id=p_run_id for update;
  ...
  select * into v_p2 from public.content_convergence_evaluations
   where run_id=p_run_id and version_id=v_version.id and stage='P2'
   order by created_at desc limit 1;
  select * into v_p3 from public.content_convergence_evaluations
   where run_id=p_run_id and version_id=v_version.id and stage='P3'
   order by created_at desc limit 1;

  select count(*) into v_critical from public.content_convergence_findings
   where run_id=p_run_id and severity='CRITICAL' and state not in ('VERIFIED_RESOLVED','SUPERSEDED');
  select coalesce(bool_or(severe_regression),false) into v_severe_regression
   from public.content_convergence_deltas where run_id=p_run_id and to_version_id=v_version.id;

  if v_run.state <> 'CONVERGED' then
    v_decision := 'NOT_READY'; v_reason := 'run has not converged';
  elsif v_p2.id is null or v_p3.id is null then
    v_decision := 'NOT_READY'; v_reason := 'fresh P2 and P3 evidence is required for exact version';
  elsif v_p2.content_hash <> v_version.content_hash or v_p3.content_hash <> v_version.content_hash then
    v_decision := 'NOT_READY'; v_reason := 'evaluation hash does not match release version';
  elsif v_p2.disposition <> 'PASS' or v_p3.disposition <> 'PASS' then
    v_decision := 'NOT_READY'; v_reason := 'latest independent evaluation is not PASS';
  elsif v_critical > 0 then
    v_decision := 'NOT_READY'; v_reason := 'unresolved critical finding';
  elsif v_severe_regression then
    v_decision := 'NOT_READY'; v_reason := 'zero-tolerance regression detected';
  elsif upper(v_p2.safety_status) <> 'PASS' or upper(v_p3.safety_status) <> 'PASS' then
    v_decision := 'NOT_READY'; v_reason := 'safety status is not PASS';
  elsif upper(v_p2.assessment_status) <> 'PASS' or upper(v_p3.assessment_status) <> 'PASS' then
    v_decision := 'NOT_READY'; v_reason := 'assessment integrity is not PASS';
  elsif upper(v_p2.provenance_status) <> 'PASS' or upper(v_p3.provenance_status) <> 'PASS' then
    v_decision := 'HUMAN_REVIEW_REQUIRED'; v_reason := 'provenance requires human authority';
  elsif exists (
    select 1 from public.hq_workforce_engine_contract
    where singleton=true and (shadow_global_stop is true or runtime_execution_enabled is not true)
  ) then
    v_decision := 'NOT_READY'; v_reason := 'Worker Engine Global Stop or runtime-off blocks release-candidate generation';
  else
    v_decision := 'RELEASE_CANDIDATE'; v_reason := 'converged exact version has fresh independent evidence';
  end if;

  insert into public.content_convergence_release_decisions(run_id,version_id,content_hash,decision,reason,evidence_packet)
  values (
    p_run_id,v_version.id,v_version.content_hash,v_decision,v_reason,
    jsonb_build_object(
      'publication_id',v_run.publication_id,'version_id',v_version.id,'content_hash',v_version.content_hash,
      'p2_evaluation_id',v_p2.id,'p3_evaluation_id',v_p3.id,'attempt_count',v_run.attempt_count,
      'critical_open',v_critical,'severe_regression',v_severe_regression,
      'human_publication_approval_required',true
    )
  )
  on conflict (run_id,version_id,content_hash) do update
    set decision=excluded.decision, reason=excluded.reason, evidence_packet=excluded.evidence_packet, created_at=now();

  return v_decision;
end $$;

create or replace function public.content_convergence_recover_expired_leases()
returns integer
language plpgsql
security definer
set search_path = ''
...
begin
  with expired as (
    update public.content_convergence_runs
      set state = case when state in ('REPAIRING','REVERIFYING','MEASURING') then 'ESCALATED' else state end,
          failure_code='LEASE_EXPIRED',
          decision_reason='execution lease expired; explicit recovery required',
          next_action='inspect evidence and resume from current immutable version',
          lease_owner=null, lease_expires_at=null, updated_at=now()
    where lease_expires_at < now()
      and state in ('REPAIRING','REVERIFYING','MEASURING')
    returning id
  ) select count(*) into v_count from expired;
  return v_count;
end $$;

revoke all on function public.content_convergence_start(uuid,text,jsonb,text,text,jsonb,jsonb,text,text) from public, anon, authenticated;
...
grant execute on function public.content_convergence_start(uuid,text,jsonb,text,text,jsonb,jsonb,text,text) to service_role;
...
create index if not exists idx_content_convergence_runs_publication_state
  on public.content_convergence_runs(publication_id,state,updated_at desc);
create index if not exists idx_content_convergence_findings_open
  on public.content_convergence_findings(run_id,state,severity);
create index if not exists idx_content_convergence_eval_latest
  on public.content_convergence_evaluations(run_id,version_id,stage,created_at desc);
create index if not exists idx_content_convergence_events_run
  on public.content_convergence_events(run_id,created_at,id);
