-- Worker Engine WE-L7: governed Worker Factory V2.
-- Demand evidence -> quantified diagnosis -> creation contract -> SHADOW.
-- The factory never certifies or activates the worker it creates.
-- access: service-only public.hq_workforce_demand_evidence
-- authorization-test: public.hq_workforce_demand_evidence anon/authenticated denied; service_role only.
-- access: service-only public.hq_workforce_factory_runs
