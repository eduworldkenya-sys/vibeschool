...
create unique index if not exists content_blocks_chapter_sequence_uidx
  on public.content_blocks(chapter_id, sequence);
create unique index if not exists content_blocks_chapter_legacy_id_uidx
  on public.content_blocks(chapter_id, legacy_block_id)
  where legacy_block_id is not null and btrim(legacy_block_id) <> '';
...
using (status = 'published' and exists (
  ...
  where p.id = content_blocks.publication_id and p.status = 'published'
));
create policy content_blocks_author_read on public.content_blocks for select to authenticated
using (exists (
  select 1 from public.vibe_publications p
  where p.id = content_blocks.publication_id and p.author_id = (select auth.uid())
));
create policy content_blocks_author_insert on public.content_blocks for insert to authenticated
with check (exists (
  select 1 from public.vibe_publications p
  where p.id = content_blocks.publication_id and p.author_id = (select auth.uid())
));
create policy content_blocks_author_update on public.content_blocks for update to authenticated
using (exists (
  select 1 from public.vibe_publications p
  where p.id = content_blocks.publication_id and p.author_id = (select auth.uid())
))
...
create policy content_blocks_author_delete on public.content_blocks for delete to authenticated
using (exists (
  select 1 from public.vibe_publications p
  where p.id = content_blocks.publication_id and p.author_id = (select auth.uid())
));
...
as $$
  select nullif(btrim(concat_ws(' ',
    ...
    case when jsonb_typeof(p_payload->'items') = 'array'
      then (select string_agg(case when jsonb_typeof(x) = 'string' then trim(both '"' from x::text) else coalesce(x->>'text', x::text) end, ' ')
...
begin
  ...
  if chapter_publication <> new.publication_id then
    raise exception 'Content block publication % does not match chapter publication %', new.publication_id, chapter_publication;
  end if;
  new.updated_at := now();
...
security definer
set search_path = public, pg_temp
as $$ begin perform public.ce_reconcile_chapter_content_blocks(new.id); return new; end; $$;
revoke all on function public.ce_reconcile_chapter_blocks_trigger() from public, anon, authenticated;
grant execute on function public.ce_reconcile_chapter_blocks_trigger() to service_role;

create or replace function public.ce_sync_content_block_status_from_chapter()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  if old.status is distinct from new.status then
    ...
    set status = case when new.status='published' then 'published' else new.status end,
        updated_at=now()
...
$$;
revoke all on function public.ce_sync_content_block_status_from_chapter() from public, anon, authenticated;
grant execute on function public.ce_sync_content_block_status_from_chapter() to service_role;

drop trigger if exists ce_validate_content_block on public.content_blocks;
create trigger ce_validate_content_block before insert or update on public.content_blocks
for each row execute function public.ce_validate_content_block();
drop trigger if exists ce_reconcile_chapter_blocks on public.vibe_chapters;
create trigger ce_reconcile_chapter_blocks after insert or update of blocks on public.vibe_chapters
for each row execute function public.ce_reconcile_chapter_blocks_trigger();
drop trigger if exists ce_sync_content_block_status_from_chapter on public.vibe_chapters;
create trigger ce_sync_content_block_status_from_chapter after update of status on public.vibe_chapters
for each row execute function public.ce_sync_content_block_status_from_chapter();

do $$
declare chapter_record record;
begin
  for chapter_record in select id from public.vibe_chapters loop
    perform public.ce_reconcile_chapter_content_blocks(chapter_record.id);
  end loop;
end;
$$;

insert into public.content_engine_authorities(domain, authoritative_table, authority_role, derived_tables, notes)
values ('content_block','public.content_blocks','Normalized first-class chapter content blocks',array['public.vibe_chapters.blocks'],'During frontend compatibility, vibe_chapters.blocks remains writable and is automatically reconciled into content_blocks. A later API-contract phase can make content_blocks the sole write authority.')
...
comment on table public.content_blocks is
  'CE-003 normalized educational content blocks. Preserves current vibe_chapters.blocks JSON while making each explanation, activity, question, media item, note and future semantic block addressable.';
