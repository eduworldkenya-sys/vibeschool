...
-- qualification introduced immediately before this migration. The historical
-- function referenced hq_workforce_workers.version, but that table has no version
-- column. The canonical certified worker version is hq_workforce_worker_assurance.worker_version,
-- which is also what content_convergence_assert_certified_worker binds into stage attempts.
-- NON-ACTIVATING: no runtime, scheduler, publishing, payments, autonomy or authority.

-- Converge only the currently certified Content Author from stale draft to the
-- non-activating restricted lifecycle state so it is eligible for specialization
-- qualification. No permissions are added.
do $$
declare
...
begin
  ...
  where worker_key='content-factory-r2-canary-01' for update;
  ...
  if w.status='draft' then
    ...
    ) then raise exception 'CHEMISTRY_AUTHOR_PERMISSION_BOUNDARY_INVALID'; end if;
...
begin
  ...
  if not found or w.status not in ('restricted','active') then
...
'Owner-gated Grade 10 Chemistry Author/Quality specialization qualification using current professional assurance worker_version as canonical execution identity. Non-activating and authority-neutral.';
