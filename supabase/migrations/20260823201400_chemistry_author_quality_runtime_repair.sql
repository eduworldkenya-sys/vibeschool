...
-- Runtime repair for the owner-gated Chemistry Author/Quality specialization
-- qualification. The historical function referenced hq_workforce_workers.version,
-- but that table has no version column. Canonical execution identity comes from
-- hq_workforce_worker_assurance.worker_version.
--
-- This migration is schema-only and reconstructs cleanly without production worker
-- rows. The certified Author's stale draft -> restricted convergence happens only
-- inside the owner-gated qualification invocation after all safety checks pass.
-- NON-ACTIVATING: no runtime, scheduler, publishing, payments, autonomy or authority.
...
begin
  ...
  if not found then raise exception 'CHEMISTRY_AUTHOR_QUALITY_WORKER_NOT_AVAILABLE'; end if;
  ...
  -- Production-only lifecycle convergence occurs at invocation time, never during
  -- clean schema reconstruction. It changes no permission or approval boundary.
  if p_worker_key='content-factory-r2-canary-01' and w.status='draft' then
    update public.hq_workforce_workers
    set status='restricted',updated_at=clock_timestamp()
    where id=w.id and status='draft';
    w.status:='restricted';
  end if;
  if w.status not in ('restricted','active') then
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    ...
    jsonb_build_object(
      ...
      'worker_version',a.worker_version,'worker_identity_source','hq_workforce_worker_assurance',
      'professional_expires_at',a.expires_at,'specialization_key','chemistry.grade10',
      'specialization_version',s.specialization_version,'stage',v_stage,
      'required_capabilities',to_jsonb(r.required_capabilities),'held_capabilities',to_jsonb(s.capabilities),
      'independent_verifier',p_verifier_key,'authority_changed',false));
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    ...
    jsonb_build_object(
      ...
      'stage_admission_gate','chemistry_claim_stage','candidate_denied',true,'expired_denied',true,
      'revoked_denied',true,'capability_mismatch_denied',true,'professional_expiry_denied',true,
      'invalid_worker_version_denied',true,'qualification_contract_version',2,'authority_changed',false));
  v_evidence_ids:=array_append(v_evidence_ids,v_evidence_id);

  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    p_worker_key,'global_stop',p_verifier_key,'chemistry-author-quality-global-stop-v2',true,
    jsonb_build_object(
      'runtime_execution_enabled',ec.runtime_execution_enabled,'heartbeat_enabled',ec.heartbeat_enabled,
      'factory_enabled',ec.factory_enabled,'runtime_autonomy_level',ec.runtime_autonomy_level,
      'runtime_max_risk',ec.runtime_max_risk,'shadow_enabled',ec.shadow_enabled,
      'shadow_scheduler_enabled',ec.shadow_scheduler_enabled,'shadow_global_stop',ec.shadow_global_stop,
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    ...
    jsonb_build_object(
      ...
      'qualification_only',true,'provider_call_executed',false,'side_effects_applied',false,
      'authority_changed',false));
  v_evidence_ids:=array_append(v_evidence_ids,v_evidence_id);

  update public.hq_workforce_worker_specializations
  set qualification_state='qualified',qualified_at=clock_timestamp(),qualified_until=v_until,
      evidence=coalesce(evidence,'{}'::jsonb)||jsonb_build_object(
        'qualification_program','chemistry-author-quality-p0-runtime-repair',
        'qualification_contract_version',2,'professional_certification_state',a.certification_state,
        'professional_qualification_state',a.qualification_state,'worker_version',a.worker_version,
        'worker_identity_source','hq_workforce_worker_assurance','stage',v_stage,
        'required_capabilities',to_jsonb(r.required_capabilities),
        'qualification_evidence_ids',to_jsonb(v_evidence_ids),'independent_verifier',p_verifier_key,
        'authority_changed',false),updated_at=clock_timestamp()
  where worker_key=p_worker_key
    and specialization_key='chemistry.grade10'
    and specialization_version=1;
  ...
  return jsonb_build_object(
    'worker_key',p_worker_key,'worker_version',a.worker_version,'worker_identity_source','hq_workforce_worker_assurance',
    'stage',v_stage,'professional_certified',true,'specialization_qualified',true,
    'specialization_key','chemistry.grade10','specialization_version',1,'qualified_until',v_until,
    'qualification_evidence_ids',to_jsonb(v_evidence_ids),'independent_verifier',p_verifier_key,
    'provider_call_executed',false,'authority_changed',false);
end $$;
...
'Owner-gated Grade 10 Chemistry Author/Quality specialization qualification using current professional assurance worker_version as canonical execution identity. The certified Author may converge draft to restricted only inside this guarded invocation. Non-activating and authority-neutral.';
