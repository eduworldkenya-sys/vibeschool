-- WE-L10: close duplicate-capacity creation path.
-- A capable certified/active worker is authoritative evidence for reuse/rebalance.
...
begin
 ...
 select coalesce(l.department_key,g.lane_key) into v_department from public.hq_workforce_lanes l where l.lane_key=g.lane_key; v_department:=coalesce(v_department,g.lane_key);
 ...
 v_metrics:=jsonb_build_object(
   ...
   'rebalance_capacity',v_skilled or coalesce((g.metrics_snapshot->>'rebalance_capacity')::boolean,false),
   ...
   'telemetry_source','gap_signal_authoritative_v2'
 );
 return v_metrics;
end $$;
...
begin
 ...
 for g in select gs.* from public.hq_workforce_gap_signals gs where gs.status in ('candidate','accepted') and not exists(select 1 from public.hq_workforce_demand_evidence de where de.gap_id=gs.id) order by case gs.severity when 'critical' then 1 when 'high' then 2 when 'medium' then 3 else 4 end,gs.detected_at for update skip locked limit p_limit loop
   ...
   if v_live>=t.max_live_workers then v_metrics:=v_metrics||jsonb_build_object('existing_worker_available',true,'existing_worker_has_skill',true,'rebalance_capacity',true,'capacity_ceiling_reached',true); end if;
   ...
   v_processed:=v_processed+1; if coalesce((v_result->>'worker_created')::boolean,false) then v_created:=v_created+1; else v_rejected:=v_rejected+1; end if;
 end loop;
...
revoke all on function public.hq_workforce_authoritative_demand_metrics(uuid,uuid),public.hq_workforce_autonomous_factory_heartbeat(integer) from public,anon,authenticated;
grant execute on function public.hq_workforce_authoritative_demand_metrics(uuid,uuid),public.hq_workforce_autonomous_factory_heartbeat(integer) to service_role;
