-- Supabase installs pgcrypto functions in the extensions schema. Pathways RPCs
-- intentionally keep a narrow SECURITY DEFINER search_path, so cryptographic
-- calls must be schema-qualified rather than widening search_path.
...
begin
  ...
  where slug = lower(trim(p_pathway_slug)) and status = 'published'
  limit 1;
  ...
  from public.student_pathway_decisions
...
begin
  ...
  if found then
    if existing.input_fingerprint<>fingerprint or existing.pathway_id<>chosen.id then raise exception 'idempotency_replay_mismatch'; end if;
  ...
  insert into public.parent_pathway_drafts(parent_profile_id,pathway_id,evidence_snapshot,rule_version,input_fingerprint,idempotency_key)
  values (caller,chosen.id,jsonb_build_object(
    'evidence_class','family_pathways_draft','answers',coalesce(p_answers,'{}'::jsonb),
    'scores',coalesce(p_scores,'{}'::jsonb),'notice','Parent-owned planning draft; not a learner Pathway Passport.'
...
begin
  ...
  v_hash := encode(extensions.digest(convert_to(jsonb_build_object(
    'kind',p_observation_kind,'external_record_id',trim(p_external_record_id),
    'external_parent_id',nullif(trim(coalesce(p_external_parent_id,'')),''),
    'label',trim(p_observed_label),'payload',coalesce(p_observed_payload,'{}'::jsonb),
