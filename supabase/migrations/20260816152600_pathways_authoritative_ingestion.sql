...
-- Raw authoritative observations are immutable evidence inputs; verified product
-- claims are promoted only through explicit platform-owner authority.
...
begin
  ...
  if not exists(
    select 1 from public.pathway_sources
    where id=p_source_id and status='active'
      and source_kind in ('official_portal','official_document','professional_body','institution_verified')
  ) then raise exception 'authoritative_source_required'; end if;

  insert into public.pathway_source_observations(
    source_id,external_key,entity_kind,observed_payload,observed_at,content_checksum,resolution_state
  ) values (
    p_source_id,trim(p_external_key),p_entity_kind,coalesce(p_payload,'{}'::jsonb),coalesce(p_observed_at,now()),trim(p_content_checksum),'unresolved'
  )
...
begin
  ...
  if obs.resolution_state in ('disputed','retired') then raise exception 'observation_not_promotable'; end if;
  ...
  if p_track_id is not null and not exists(
    select 1 from public.pathway_tracks where id=p_track_id and pathway_id=p_pathway_id and status='published' and verification_state='verified'
  ) then raise exception 'verified_track_required'; end if;
  if p_combination_id is not null and not exists(
    select 1 from public.pathway_subject_combinations where id=p_combination_id and pathway_id=p_pathway_id and status='published' and verification_state='verified'
  ) then raise exception 'verified_combination_required'; end if;
  ...
  insert into public.pathway_school_offerings(
    school_id,pathway_id,track_id,combination_id,source_id,verification_state,
    observed_at,verified_at,effective_from,effective_to
  ) values (
    p_school_id,p_pathway_id,p_track_id,p_combination_id,obs.source_id,'verified',
    obs.observed_at,now(),p_effective_from,p_effective_to
  )
  on conflict (
    school_id,
    pathway_id,
  ...
  update public.pathway_source_observations
  set resolution_state='resolved',resolved_entity_id=offering_id
  where id=obs.id;
...
-- Evidence rows for the three official high-level pathway seed claims.
insert into public.pathway_source_observations(
  source_id,external_key,entity_kind,observed_payload,observed_at,content_checksum,resolution_state,resolved_entity_id
) values
