-- School Identity P0 coverage runtime repair.
--
-- Production execution exposed a PL/pgSQL name collision: the RETURNS TABLE
-- output variable `county` conflicted with unqualified CTE/USING references.
-- Keep the owner-only contract and metrics unchanged, but use `county_key`
-- internally everywhere so PL/pgSQL can never resolve an output variable by
-- accident. No data mutation or authority widening occurs here.

create or replace function public.hq_school_identity_coverage_by_county()
returns table(
  county text,
  discovery_records bigint,
  discovery_with_knec bigint,
  canonical_active bigint,
  canonical_with_strong_id bigint,
  authoritative_observations bigint,
  authoritative_matched bigint,
  authoritative_new_candidates bigint,
  authoritative_review bigint,
  canonical_to_discovery_ratio numeric,
  authoritative_resolution_ratio numeric
)
language plpgsql
security definer
set search_path = public, extensions, pg_temp
as $$
begin
  if auth.uid() is null or not coalesce(public.is_platform_owner(), false) then
    raise exception 'owner_authorization_required';
  end if;

  return query
  with discovery as (
    select
      coalesce(upper(nullif(trim(d.county),'')), 'UNKNOWN') as county_key,
      count(*)::bigint as records,
      count(*) filter (where nullif(trim(d.knec_code),'') is not null)::bigint as with_knec
    from public.schools_directory d
    group by coalesce(upper(nullif(trim(d.county),'')), 'UNKNOWN')
  ),
  canonical as (
    select
      coalesce(upper(nullif(trim(s.county),'')), 'UNKNOWN') as county_key,
      count(*) filter (where s.deleted_at is null and s.status = 'active')::bigint as active,
      count(*) filter (
        where s.deleted_at is null and s.status = 'active'
          and (
            nullif(trim(s.knec_code),'') is not null
            or nullif(trim(s.nemis_code),'') is not null
            or nullif(trim(s.moe_registration_no),'') is not null
            or nullif(trim(s.tsc_code),'') is not null
          )
      )::bigint as with_strong_id
    from public.schools s
    group by coalesce(upper(nullif(trim(s.county),'')), 'UNKNOWN')
  ),
  authority as (
    select
      coalesce(upper(nullif(trim(o.raw_record->>'county'),'')), 'UNKNOWN') as county_key,
      count(*)::bigint as observations,
      count(*) filter (where r.classification = 'matched')::bigint as matched,
      count(*) filter (where r.classification = 'new_candidate')::bigint as new_candidates,
      count(*) filter (where r.classification = 'review')::bigint as review
    from public.school_directory_source_observations o
    join public.school_directory_source_registry sr
      on sr.source_name = o.source_name
     and sr.authority_tier = 0
     and sr.canonical_use
     and sr.active
     and sr.verification_mode = 'authoritative'
    left join public.school_authoritative_reconciliation r
      on r.source_observation_id = o.id
    group by coalesce(upper(nullif(trim(o.raw_record->>'county'),'')), 'UNKNOWN')
  ),
  county_keys as (
    select d0.county_key from discovery d0
    union
    select c0.county_key from canonical c0
    union
    select a0.county_key from authority a0
  )
  select
    ck.county_key::text as county,
    coalesce(d.records,0)::bigint as discovery_records,
    coalesce(d.with_knec,0)::bigint as discovery_with_knec,
    coalesce(k.active,0)::bigint as canonical_active,
    coalesce(k.with_strong_id,0)::bigint as canonical_with_strong_id,
    coalesce(a.observations,0)::bigint as authoritative_observations,
    coalesce(a.matched,0)::bigint as authoritative_matched,
    coalesce(a.new_candidates,0)::bigint as authoritative_new_candidates,
    coalesce(a.review,0)::bigint as authoritative_review,
    case when coalesce(d.records,0) = 0 then null
      else round(coalesce(k.active,0)::numeric / d.records::numeric, 6)
    end as canonical_to_discovery_ratio,
    case when coalesce(a.observations,0) = 0 then null
      else round((coalesce(a.matched,0) + coalesce(a.new_candidates,0) + coalesce(a.review,0))::numeric / a.observations::numeric, 6)
    end as authoritative_resolution_ratio
  from county_keys ck
  left join discovery d on d.county_key = ck.county_key
  left join canonical k on k.county_key = ck.county_key
  left join authority a on a.county_key = ck.county_key
  order by
    case when ck.county_key = 'UNKNOWN' then 1 else 0 end,
    ck.county_key;
end;
$$;

revoke all on function public.hq_school_identity_coverage_by_county() from public, anon;
grant execute on function public.hq_school_identity_coverage_by_county() to authenticated;

comment on function public.hq_school_identity_coverage_by_county() is
'Owner-only School Identity coverage projection. Internal geography uses county_key to prevent PL/pgSQL output-variable ambiguity. Missing county stays UNKNOWN; directory ratios remain diagnostics only.';
