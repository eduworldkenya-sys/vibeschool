-- PostgreSQL now() is transaction-stable; use wall-clock timestamps for certification evidence ordering.
...
begin
 ...
 insert into public.hq_workforce_shadow_runs(worker_key,tool_contract_id,input_snapshot,expected_outcome,observed_outcome,verifier_key,passed,executed_at)
 values(p_worker_key,p_tool_contract_id,p_input,p_expected,p_observed,p_verifier_key,v_pass,clock_timestamp()) returning id into v_id;
 return v_id;
end $$;
...
begin
 ...
 values(p_worker_key,p_creation_contract_id,p_worker_key||':'||extract(epoch from v_issued)::bigint,'active',p_required,v_passed,p_verifier_key,v_issued,v_issued+p_valid_for) returning id into v_id;
 return v_id;
end $$;
