...
-- member of the same school. Account/profile UUIDs are never compared to student_id.

create or replace function public.is_live_teacher_class(
  p_school_id uuid,
  p_class_id uuid
)
returns boolean
...
create or replace function public.is_live_teacher_subject(
  p_school_id uuid,
  p_class_id uuid,
  p_subject_id uuid
)
...
-- Canonical student roster read: current enrollment + assignment + live teacher membership.
...
-- student_profiles.profile_id is an ACCOUNT identity. Resolve it to the canonical
-- learner before testing parent/teacher authorization; never compare it directly
-- with student_classes.student_id or parent_student_links.student_id.
...
using (
  ...
  or exists (
    ...
    where s.profile_id=student_profiles.profile_id
...
-- Attendance: teacher access must be live and class-bound. Student identity remains
-- attendance.student_id -> students.id and the current enrollment must agree.
...
for select to authenticated
using (
  public.is_live_teacher_class(school_id,class_id)
...
-- CBC assessment teacher boundary.
...
for select to authenticated
using (
  public.is_live_teacher_subject(school_id,class_id,subject_id)
  and public.is_teacher_of_student(student_id)
);
...
-- Exam result high-integrity boundary. Preserve exam-lock and school/class/subject
-- checks while adding live teacher membership to every teacher path.
...
with check (
  ...
  and exists (
    select 1 from public.student_classes sc
    where sc.student_id=exam_results.student_id
      and sc.school_id=exam_results.school_id
      and sc.class_id=exam_results.class_id
      and sc.is_current=true
  )
...
-- Teacher read of assessment attempts must remain tied to a live learner relationship,
-- not merely a historical assignment authored by the same account.
...
-- Homework submissions: the teacher who authored the homework must still have a
-- live class relationship and current authorization to this canonical learner.
...
using (
  ...
  and exists (
    ...
    where h.id=homework_submissions.homework_id
      ...
      and public.is_live_teacher_class(h.school_id,h.class_id)
  )
)
with check (
  public.is_teacher_of_student(student_id)
...
-- Evidence, interventions, projects and exercises all use canonical student_id.
...
using (
  ...
  and exists (
    ...
    where e.id=exercise_submissions.exercise_id
      ...
      and public.is_live_teacher_class(e.school_id,e.class_id)
  )
)
with check (
  public.is_teacher_of_student(student_id)
...
using (
  ...
  and exists (
    ...
    where p.id=project_submissions.project_id
      ...
      and public.is_live_teacher_subject(p.school_id,p.class_id,p.subject_id)
  )
)
with check (
  public.is_teacher_of_student(student_id)
...
-- Parent communication/relationship surfaces: teacher access derives from the
-- canonical linked learner and current teacher authorization.
...
with check (
  ...
  and exists (
    select 1 from public.student_classes sc
    where sc.student_id=parent_messages.student_id
      and sc.school_id=parent_messages.school_id
      and sc.is_current=true
  )
);
...
using (
  ...
  or exists (
    ...
    where psl.parent_id=parent_profiles.profile_id
      and public.is_teacher_of_student(psl.student_id)
  )
  or exists (
    select 1 from public.parent_student_links psl
    where psl.parent_id=parent_profiles.profile_id
      and public.is_school_admin(psl.school_id)
  )
);

-- Claim code and learner mastery visibility are current canonical learner relations.
...
-- The old learner_outcomes policy joined students.class_id directly and treated it
-- as enrollment truth. Canonical current student_classes is the only class truth.
...
-- Fail closed if any known student-facing policy still compares student_id to
-- student_profiles.profile_id or grants teacher learner access from teacher_classes
-- without live school membership through the canonical helpers above.
do $block$
declare
  v_bad integer;
begin
  ...
  where schemaname='public'
    ...
    and (coalesce(qual,'') ilike '%student_id = student_profiles.profile_id%'
