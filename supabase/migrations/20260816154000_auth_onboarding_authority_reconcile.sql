...
-- Auth provider/user metadata is identity/display data only; it never grants an app role.
...
-- Explicit signup may classify an otherwise unclassified account into a public,
-- self-service lane. Existing canonical role always wins and cannot be replaced.
...
-- Direct profile editing may change display/profile attributes but never authority.
-- SECURITY DEFINER governance functions execute as their owner and can perform
-- explicit authority transitions after their own checks.
...
-- School membership is derived from canonical teacher authority, never vice versa.
...
begin
  ...
  if not exists(
    ...
    where id = p_school_id and deleted_at is null and status in ('pending','active')
  ) then
...
revoke all on function public.connect_teacher_to_school(uuid) from public, anon;
grant execute on function public.connect_teacher_to_school(uuid) to authenticated;
...
revoke all on function public.connect_teacher_to_directory_school(uuid) from public, anon;
grant execute on function public.connect_teacher_to_directory_school(uuid) to authenticated;

-- Approval is an explicit authority transition. It must not silently overwrite a
-- different established account role.
...
comment on function public.guard_profile_authority_fields() is
  'Blocks direct authenticated mutation of canonical authority fields; governed SECURITY DEFINER transitions remain available.';
comment on function public.connect_teacher_to_school(uuid) is
  'Connects only an already-canonical teacher account to a canonical school.';
comment on function public.connect_teacher_to_directory_school(uuid) is
  'Connects only an already-canonical teacher account through the governed directory reconciliation path.';
