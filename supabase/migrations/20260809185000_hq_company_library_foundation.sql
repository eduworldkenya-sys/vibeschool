-- HQ Company Library foundation.
-- Establishes a single provenance-aware artifact authority for HQ work outputs.
...
create table if not exists public.hq_artifacts (
  ...
  lifecycle_state text not null default 'draft'
    check (lifecycle_state in ('draft','in_review','approved','published','archived','superseded')),
  approval_state text not null default 'not_required'
    check (approval_state in ('not_required','pending','approved','rejected')),
  confidentiality text not null default 'internal'
    check (confidentiality in ('internal','restricted','public')),
  purpose text,
...
alter table public.hq_artifacts
  drop constraint if exists hq_artifacts_current_version_id_fkey;
alter table public.hq_artifacts
  add constraint hq_artifacts_current_version_id_fkey
  foreign key (current_version_id) references public.hq_artifact_versions(id) on delete set null;
...
revoke all on public.hq_artifacts, public.hq_artifact_versions, public.hq_artifact_provenance,
  public.hq_artifact_links, public.hq_artifact_approvals from anon, authenticated;
...
create policy "hq owners read artifacts" on public.hq_artifacts for select to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));
create policy "hq owners create artifacts" on public.hq_artifacts for insert to authenticated
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));
create policy "hq owners update artifacts" on public.hq_artifacts for update to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())))
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));

create policy "hq owners read artifact versions" on public.hq_artifact_versions for select to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));
create policy "hq owners create artifact versions" on public.hq_artifact_versions for insert to authenticated
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));

create policy "hq owners read artifact provenance" on public.hq_artifact_provenance for select to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));
create policy "hq owners create artifact provenance" on public.hq_artifact_provenance for insert to authenticated
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));

create policy "hq owners manage artifact links" on public.hq_artifact_links for all to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())))
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));

create policy "hq owners manage artifact approvals" on public.hq_artifact_approvals for all to authenticated
  using (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())))
  with check (exists (select 1 from public.platform_owners po where po.profile_id = (select auth.uid())));

-- Private binary storage for HQ artifacts. This is intentionally separate from
-- public learning/publication buckets.
