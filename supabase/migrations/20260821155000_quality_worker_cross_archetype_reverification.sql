...
begin
  ...
  if a.risk_class in ('R1','R2','R3') and not exists(select 1 from public.hq_workforce_qualification_evidence qe where qe.worker_key=p_target_worker_key and qe.worker_version=a.worker_version and qe.evidence_kind='shadow' and qe.passed) then v_missing:=array_append(v_missing,'shadow'); end if;
