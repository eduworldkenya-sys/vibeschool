-- VibeSchool Task 12: Pilot telemetry, observability and product analytics foundation.
-- Reuses public.platform_events as the canonical pilot event ledger.
...
grant select on public.pilot_event_contract to authenticated;
drop policy if exists pilot_event_contract_authenticated_read on public.pilot_event_contract;
create policy pilot_event_contract_authenticated_read on public.pilot_event_contract
...
('auth.login_started','authentication','login_started','User submitted a sign-in attempt',false,array['teacher','student','parent','school_admin','global_user'],array['method'],null),
('auth.login_succeeded','authentication','authentication_success','Supabase Auth returned an authenticated user',false,array['teacher','student','parent','school_admin','global_user'],array['method'],null),
('auth.login_failed','authentication','authentication_failure','Authentication attempt failed',false,array['teacher','student','parent','school_admin','global_user'],array['method'],null),
('auth.identity_resolved','authentication','identity_resolution','Canonical application identity and role resolved',true,array['teacher','student','parent','school_admin','global_user'],array[]::text[],null),
('auth.identity_failed','authentication','identity_resolution_failure','Identity resolution failed',true,array['teacher','student','parent','school_admin','global_user'],array[]::text[],null),
('auth.onboarding_resolved','onboarding','onboarding_resolution','Onboarding state and safe destination resolved',true,array['teacher','student','parent','school_admin','global_user'],array['state'],null),
('auth.dashboard_reached','authentication','dashboard','Authorized dashboard destination reached',false,array['teacher','student','parent','school_admin','global_user'],array[]::text[],null),
...
('admin.school_operation_committed','admin','activation','Legitimate school operation committed',true,array['school_admin'],array['operation_type'],'school_admin'),
('vibelearn.content_requested','vibelearn','content_requested','Learner requested an approved learning asset',false,array['student','teacher','parent','school_admin','global_user'],array['content_type'],null),
('vibelearn.content_opened','vibelearn','content_opened','Learning asset rendered and became usable',false,array['student','teacher','parent','school_admin','global_user'],array['content_type'],null),
('vibelearn.render_failed','vibelearn','render_failure','Learning asset failed to render',false,array['student','teacher','parent','school_admin','global_user'],array['content_type'],null),
...
('security.authorization_denied','security','authorization_denied','Protected operation was denied',true,array['teacher','student','parent','school_admin','global_user'],array['resource_type'],null),
('payment.completed','payments','payment_completed','Payment completion is authoritative',true,array['parent','teacher','school_admin','global_user'],array['provider'],null),
...
begin
  ...
  if cardinality(v_contract.allowed_roles)>0 and (v_role is null or not (v_role=any(v_contract.allowed_roles))) then
  ...
  values(p_event_name,v_uid,v_role,p_school_id,coalesce(nullif(p_entity_type,''),'application'),p_entity_id,v_metadata,now(),p_idempotency_key,
...
revoke all on function public.pilot_record_event(text,text,text,uuid,uuid,text,uuid,uuid,text,text,integer,text,text,jsonb,text) from public, anon;
...
comment on table public.pilot_event_contract is 'Task 12 canonical allowlist for pilot telemetry. Metadata is schema-limited; platform_events remains canonical ledger.';
comment on function public.pilot_record_event(text,text,text,uuid,uuid,text,uuid,uuid,text,text,integer,text,text,jsonb,text) is 'Privacy-safe client telemetry ingress. Cannot emit authoritative success events.';
comment on function public.pilot_record_authoritative_event(text,uuid,text,uuid,text,uuid,text,uuid,text,text,integer,jsonb,text) is 'Service-only authoritative event ingress for committed backend operations.';
