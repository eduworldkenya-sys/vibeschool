...
alter table public.platform_events add constraint platform_events_outcome_check check (outcome is null or outcome in ('attempted','succeeded','failed','denied','cancelled')) not valid;
alter table public.platform_events drop constraint if exists platform_events_failure_class_check;
alter table public.platform_events add constraint platform_events_failure_class_check check (failure_class is null or failure_class in ('authentication','authorization','identity','database','rpc','network','content','validation','application','external_integration','unknown')) not valid;
alter table public.platform_events drop constraint if exists platform_events_network_class_check;
alter table public.platform_events add constraint platform_events_network_class_check check (network_class is null or network_class in ('online','slow','timeout','offline','unknown')) not valid;
alter table public.platform_events drop constraint if exists platform_events_latency_check;
alter table public.platform_events add constraint platform_events_latency_check check (latency_ms is null or latency_ms between 0 and 3600000) not valid;

create index if not exists platform_events_journey_time_idx on public.platform_events (journey, occurred_at desc) where journey is not null;
create index if not exists platform_events_outcome_time_idx on public.platform_events (outcome, occurred_at desc) where outcome is not null;
create index if not exists platform_events_correlation_idx on public.platform_events (correlation_id, occurred_at) where correlation_id is not null;
create index if not exists platform_events_failure_idx on public.platform_events (failure_class, error_code, occurred_at desc) where outcome in ('failed','denied');

-- access: service-only public.pilot_event_contract
-- authorization-test: public.pilot_event_contract
...
on conflict (event_name) do update set journey=excluded.journey, stage=excluded.stage, success_semantics=excluded.success_semantics, authoritative_required=excluded.authoritative_required, allowed_roles=excluded.allowed_roles, metadata_keys=excluded.metadata_keys, activation_role=excluded.activation_role, updated_at=now(), active=true;

create or replace function public.pilot_sanitize_event_metadata(p_metadata jsonb, p_allowed text[]) returns jsonb language sql immutable set search_path = public, pg_temp as $$
...
declare
  v_uid uuid := auth.uid(); v_contract public.pilot_event_contract%rowtype; v_role text; v_school_id uuid; v_id uuid; v_metadata jsonb;
begin
  ...
  insert into public.platform_events(event_type,actor_id,actor_role,school_id,entity_type,entity_id,metadata,occurred_at,idempotency_key,journey,surface,outcome,failure_class,error_code,latency_ms,correlation_id,session_id,source,authoritative,network_class,app_version)
  values(p_event_name,v_uid,v_role,v_school_id,'application',null,v_metadata,now(),p_idempotency_key,v_contract.journey,left(coalesce(p_surface,'unknown'),120),p_outcome,p_failure_class,left(p_error_code,80),p_latency_ms,p_correlation_id,p_session_id,'client',false,p_network_class,left(p_app_version,80))
...
begin
  ...
  v_metadata := public.pilot_sanitize_event_metadata(p_metadata,v_contract.metadata_keys);
  insert into public.platform_events(event_type,actor_id,actor_role,school_id,entity_type,entity_id,metadata,occurred_at,idempotency_key,journey,surface,outcome,failure_class,error_code,latency_ms,correlation_id,source,authoritative)
  values(p_event_name,p_actor_id,p_actor_role,p_school_id,coalesce(nullif(p_entity_type,''),'application'),p_entity_id,v_metadata,now(),p_idempotency_key,v_contract.journey,'backend',p_outcome,p_failure_class,left(p_error_code,80),p_latency_ms,p_correlation_id,'backend',true)
  on conflict (idempotency_key) where idempotency_key is not null do nothing returning id into v_id;
...
create or replace function public.hq_get_pilot_observability_scorecard(p_since timestamptz default now()-interval '7 days') returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare v jsonb;
begin
  if auth.uid() is null or not public.is_platform_owner() then raise exception 'HQ_OWNER_REQUIRED' using errcode='42501'; end if;
  with e as (select * from public.platform_events where occurred_at>=p_since), rollup as (
    select count(*) filter(where event_type='auth.login_started') login_attempts,
      ...
      percentile_cont(0.95) within group(order by latency_ms) filter(where latency_ms is not null) p95_latency_ms from e)
  select jsonb_build_object('since',p_since,'generated_at',now(),'entry',jsonb_build_object('login_attempts',login_attempts,'login_successes',login_successes,'login_success_rate',case when login_attempts=0 then null else round(100.0*login_successes/login_attempts,1) end,'activation',jsonb_build_object('teachers',activated_teachers,'students',activated_students,'parents',activated_parents,'admins',activated_admins),'reliability',jsonb_build_object('failures',failures,'content_failures',content_failures,'auth_identity_failures',auth_identity_failures,'p95_latency_ms',p95_latency_ms))) into v from rollup;
  return v;
end;
$$;
...
begin
  if auth.uid() is null or not public.is_platform_owner() then raise exception 'HQ_OWNER_REQUIRED' using errcode='42501'; end if;
  return query select e.id,e.occurred_at,e.journey,e.surface,e.actor_role,e.school_id,e.event_type,e.outcome,e.failure_class,e.error_code,e.correlation_id,e.latency_ms from public.platform_events e where e.occurred_at>=p_since and e.outcome in ('failed','denied') order by e.occurred_at desc limit greatest(1,least(p_limit,500));
end;
$$;
