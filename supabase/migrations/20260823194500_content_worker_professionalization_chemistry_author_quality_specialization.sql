...
-- P0 Grade 10 Chemistry specialization qualification for the existing Author and Quality workers.
-- NON-ACTIVATING: this does not enable runtime, heartbeat, factory, shadow, scheduler,
-- publishing, payments, autonomy, consequential authority, or disable Global Stop.
-- It preserves the existing professional-worker certification and adds the narrower,
-- expiring chemistry.grade10 specialization only after independent, fail-closed checks.
...
begin
  ...
  if a.worker_version is distinct from w.version then
    raise exception 'CHEMISTRY_AUTHOR_QUALITY_CERTIFICATION_VERSION_MISMATCH';
  end if;
  if exists(
  ...
  if s.qualification_state='qualified'
    ...
    return jsonb_build_object(
      'worker_key',p_worker_key,'stage',v_stage,'professional_certified',true,
      'specialization_qualified',true,'idempotent_replay',true,'authority_changed',false);
  end if;
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    p_worker_key,'independent',p_verifier_key,'chemistry-author-quality-professional-baseline-v1',true,
    jsonb_build_object(
      ...
      'worker_version',a.worker_version,'professional_expires_at',a.expires_at,
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    p_worker_key,'adversarial',p_verifier_key,'chemistry-author-quality-admission-negative-controls-v1',true,
    jsonb_build_object(
      ...
      'qualification_contract_version',1,'authority_changed',false));
  v_evidence_ids:=array_append(v_evidence_ids,v_evidence_id);

  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    p_worker_key,'global_stop',p_verifier_key,'chemistry-author-quality-global-stop-v1',true,
    jsonb_build_object(
  ...
  v_evidence_id:=public.hq_workforce_record_qualification_evidence(
    p_worker_key,'authority_separation',p_verifier_key,'chemistry-author-quality-authority-separation-v1',true,
    jsonb_build_object(
  ...
  -- Fail closed after mutation: the canonical admission assertion must now pass for
  -- exactly this worker/stage/specialization, otherwise the transaction rolls back.
...
'Owner-gated, non-activating Grade 10 Chemistry specialization qualification for the already professionally certified Author and Quality workers. Requires current professional certification, exact worker version, capability fit, independent verifier, runtime OFF and Global Stop ON; records immutable qualification evidence and grants no authority.';

-- Reconstruction-time non-activation assertion. A schema promotion must fail if it
-- encounters an unsafe Worker Engine posture rather than silently qualifying workers.
do $$
...
begin
  ...
  if coalesce(ec.runtime_execution_enabled,false)
     ...
     or coalesce(ec.shadow_scheduler_enabled,false)
     or not coalesce(ec.shadow_global_stop,true) then
    raise exception 'CHEMISTRY_AUTHOR_QUALITY_QUALIFICATION_MIGRATION_NON_ACTIVATING_BOUNDARY_VIOLATED';
  end if;
end $$;
