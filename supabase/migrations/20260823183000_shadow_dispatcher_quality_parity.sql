...
-- Restore canonical Quality fixture handling in the shared professional shadow dispatcher.
-- NON-ACTIVATING: deterministic read-only qualification support only.
...
begin
  ...
  if t.handler_key='work_item.triage_and_own' then
    ...
    return jsonb_build_object('decision',v_decision,'reason',v_reason,'handler',t.handler_key,'finding_required',v_decision='reject','side_effects_applied',false,'authority_changed',false);
...
begin
  select id into v_tool from public.hq_workforce_tool_contracts where tool_key='workforce.chemistry_critic.assess_fixture' and version=1 and status='approved';
