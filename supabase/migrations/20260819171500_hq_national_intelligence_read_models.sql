  ...
  if p_parent_type='country' then
    return coalesce((
      with school_rollup as (
        select c.id,
               c.official_name as name,
               count(sg.school_id)::bigint as school_count,
               count(sg.school_id) filter(where sg.verification_state='verified')::bigint as verified_school_count
        ...
        where (p_parent_id is null or c.country_id=p_parent_id)
          and c.status='active'
        group by c.id,c.official_name
      ), event_rollup as (
        select sg.county_id as id,
               count(distinct pe.school_id)::bigint as active_school_count
        from public.school_geography sg
        join public.platform_events pe on pe.school_id=sg.school_id
      ...
      select jsonb_agg(jsonb_build_object(
        'id',sr.id,
        'name',sr.name,
        'school_count',sr.school_count,
        'verified_school_count',sr.verified_school_count,
        'active_school_count',coalesce(er.active_school_count,0)
      ) order by sr.school_count desc,sr.name)
      from school_rollup sr
      left join event_rollup er using(id)
    ),'[]'::jsonb);
  elsif p_parent_type='county' then
    return coalesce((
      with school_rollup as (
        select sc.id,
               sc.official_name as name,
               count(sg.school_id)::bigint as school_count,
               count(sg.school_id) filter(where sg.verification_state='verified')::bigint as verified_school_count
        ...
        select sg.subcounty_id as id,
               count(distinct pe.school_id)::bigint as active_school_count
        from public.school_geography sg
        join public.platform_events pe on pe.school_id=sg.school_id
        join public.schools s on s.id=sg.school_id and s.deleted_at is null
        where sg.county_id=p_parent_id
    ...
    return coalesce((
      with school_rollup as (
        ...
        select sg.ward_id as id,
               count(distinct pe.school_id)::bigint as active_school_count
        from public.school_geography sg
        join public.platform_events pe on pe.school_id=sg.school_id
        join public.schools s on s.id=sg.school_id and s.deleted_at is null
        where sg.subcounty_id=p_parent_id
          and pe.occurred_at>=now()-(v_days||' days')::interval
        group by sg.ward_id
      )
      select jsonb_agg(jsonb_build_object(
...
begin
  ...
  return jsonb_build_object(
    'identity',jsonb_build_object(
      ...
      'aliases',coalesce((select jsonb_agg(jsonb_build_object('alias',a.alias,'verified',a.verified,'source',a.source) order by a.verified desc,a.alias) from public.school_aliases a where a.school_id=p_school_id),'[]'::jsonb)
    ),
    'geography',coalesce((
...
begin
  ...
  return (
    with scope_schools as (
      ...
      select sm.profile_id as profile_id from public.school_members sm join scope_schools ss on ss.id=sm.school_id
      union
      ...
      join scope_schools ss on ss.id=pe.school_id
    ...
    select jsonb_build_object(
      ...
      'measurement',jsonb_build_object(
        'certified_from',(select certified_from from public.product_measurement_state where singleton=true limit 1),
        'session_kernel_available',exists(select 1 from public.product_account_sessions limit 1),
      ...
      'semantics',jsonb_build_object(
        ...
        'activity','distinct platform_events.actor_id in scoped schools',
...
begin
  ...
  return coalesce((
    with scope_schools as (
      select s.id,s.name,sg.verification_state,gco.official_name as county,gsc.official_name as subcounty,gw.official_name as ward
      from public.schools s
      left join public.school_geography sg on sg.school_id=s.id
      left join public.geo_counties gco on gco.id=sg.county_id
      left join public.geo_subcounties gsc on gsc.id=sg.subcounty_id
      ...
      select ss.*,
             (select count(distinct sc.student_id) from public.student_classes sc where sc.school_id=ss.id and sc.is_current=true) as learners,
             (select count(distinct sm.profile_id) from public.school_members sm where sm.school_id=ss.id and sm.role::text='teacher') as teachers,
             (select count(distinct pe.actor_id) from public.platform_events pe where pe.school_id=ss.id and pe.actor_id is not null and pe.actor_role::text='teacher' and pe.occurred_at>=now()-(v_days||' days')::interval) as active_teachers
      from scope_schools ss
    ), signals as (
      select 'teacher_activation'::text as signal_type,
             id as school_id,
             name as school_name,
             county,subcounty,ward,
             'attention'::text as state,
             jsonb_build_object('learners',learners,'teachers',teachers,'active_teachers',active_teachers,'window_days',v_days) as evidence,
             'Inspect school onboarding and teacher activation.'::text as recommended_investigation
      from evidence
      where learners>0 and active_teachers=0
      union all
      select 'geography_gap',id,name,county,subcounty,ward,
             'unknown'::text,
             jsonb_build_object('verification_state',coalesce(verification_state,'unmapped')),
             'Inspect school identity/geography evidence; do not auto-repair ambiguous geography.'::text
      from evidence
      where verification_state is null or verification_state in ('unresolved','conflicting')
    )
    select jsonb_agg(jsonb_build_object(
    ...
    ) order by signal_type,school_name)
    from (select * from signals limit v_limit) x
  ),'[]'::jsonb);
end;
$$;
