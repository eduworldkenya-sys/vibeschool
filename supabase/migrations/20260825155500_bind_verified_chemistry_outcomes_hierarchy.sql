...
begin
  ...
  for update;
  if not found
     or v_import.status<>'verified'
     or v_import.source_type<>'official'
     or lower(v_import.subject)<>'chemistry'
     or replace(lower(v_import.grade),' ','')<>'grade10'
     or v_import.version_label<>'July 2025'
     or v_import.content_sha256 !~ '^[0-9a-f]{64}$'
  ...
  for update;
  if not found or v_snapshot.status<>'reconciled' or v_snapshot.observation_count<>32 then
    raise exception 'RECONCILED_32_OBSERVATION_SNAPSHOT_REQUIRED';
  end if;

  select * into v_source
  from public.curriculum_authority_sources
  where id=v_snapshot.source_id;
  if not found
     or v_source.source_status<>'approved'
  ...
  if coalesce(v_import.payload->>'curriculum_authority_snapshot_id','')<>p_snapshot_id::text then
    raise exception 'IMPORT_SNAPSHOT_PROVENANCE_MISMATCH';
  end if;

  if (select count(*) from public.curriculum_authority_hierarchy_bindings h
      where h.snapshot_id=p_snapshot_id)<>7 then
    raise exception 'EXACT_SEVEN_HIERARCHY_BINDINGS_REQUIRED';
  end if;

  if (select count(*) from public.curriculum_authority_reconciliation r
      where r.snapshot_id=p_snapshot_id)<>32
     or (select count(*) from public.curriculum_authority_reconciliation r
         where r.snapshot_id=p_snapshot_id and r.classification='missing_outcome')<>32
  ...
  if exists(
    ...
    left join public.curriculum_learning_outcomes clo
      on clo.source_import_id=p_import_id
  ...
  where clo.source_import_id=p_import_id
    and clo.source_type='official'
    and clo.status='verified'
    and clo.outcome_code like 'CHEM-G10-%'
  ...
  where r.snapshot_id=p_snapshot_id
    ...
    and clo.source_import_id=p_import_id
    and clo.source_type='official'
    and clo.status='verified'
    and clo.sub_strand_id is null
    and public.curriculum_authority_normalize_text(clo.outcome_code)=public.curriculum_authority_normalize_text(o.outcome_code)
  ...
  delete from public.curriculum_authority_reconciliation
...
comment on function public.hq_bind_verified_grade10_chemistry_outcomes_hierarchy(uuid,uuid) is
'Owner-gated repair that binds exactly 32 already-verified Grade 10 Chemistry KICD outcomes to their exact source-bound hierarchy, then requires fresh reconciliation.';
