...
create unique index if not exists hq_marketing_campaigns_campaign_code_uq
  on public.hq_marketing_campaigns(campaign_code)
  where campaign_code is not null;

alter table public.hq_marketing_events
  add column if not exists value_kes numeric,
...
create table if not exists public.hq_growth_attributions (
  ...
  returned_day2_at timestamptz,
  first_payment_at timestamptz,
  revenue_kes numeric not null default 0 check (revenue_kes >= 0),
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.hq_growth_school_pipeline (
  ...
  teacher_champion_profile_id uuid references public.profiles(id) on delete set null,
  territory text,
  first_student_at timestamptz,
  first_payment_at timestamptz,
  institutional_proposal_sent_at timestamptz,
  institutional_contract_signed_at timestamptz,
  notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (school_id)
...
create policy hq_growth_agents_owner_only on public.hq_growth_agents for all to authenticated
  using (coalesce(public.is_platform_owner(), false))
  with check (coalesce(public.is_platform_owner(), false));

drop policy if exists hq_growth_creators_owner_only on public.hq_growth_creators;
create policy hq_growth_creators_owner_only on public.hq_growth_creators for all to authenticated
  using (coalesce(public.is_platform_owner(), false))
  with check (coalesce(public.is_platform_owner(), false));

drop policy if exists hq_growth_attributions_owner_only on public.hq_growth_attributions;
create policy hq_growth_attributions_owner_only on public.hq_growth_attributions for all to authenticated
  using (coalesce(public.is_platform_owner(), false))
  with check (coalesce(public.is_platform_owner(), false));

drop policy if exists hq_growth_school_pipeline_owner_only on public.hq_growth_school_pipeline;
create policy hq_growth_school_pipeline_owner_only on public.hq_growth_school_pipeline for all to authenticated
  using (coalesce(public.is_platform_owner(), false))
  with check (coalesce(public.is_platform_owner(), false));
...
begin
  ...
  return jsonb_build_object(
    ...
    'sources', coalesce((
      select jsonb_agg(to_jsonb(x) order by x.paid desc, x.signups desc)
      from (
        select source_key as source,
               ...
               count(*) filter (where returned_day2_at is not null) as returned_day2,
               count(*) filter (where first_payment_at is not null) as paid,
               coalesce(sum(revenue_kes),0) as revenue_kes
        from public.hq_growth_attributions
        where first_touch_at >= v_since
        group by source_key
      ) x
    ), '[]'::jsonb),
    'campaigns', coalesce((
      ...
      from (
        select c.id,c.name,c.campaign_code,c.campaign_type,c.channel,c.source_key,c.territory,c.status,c.budget,c.currency,c.created_at,
               count(a.id) filter (where a.first_touch_at >= v_since) as touches,
               count(a.id) filter (where a.signup_at >= v_since) as signups,
               count(a.id) filter (where a.first_payment_at >= v_since) as paid,
               coalesce(sum(a.revenue_kes) filter (where a.first_payment_at >= v_since),0) as revenue_kes
        from public.hq_marketing_campaigns c
        left join public.hq_growth_attributions a on a.campaign_id=c.id
        group by c.id
    ...
    'agents', coalesce((
      ...
      from (
        select g.id,g.agent_code,g.display_name,g.territory,g.schools_assigned,g.commission_rate,g.status,
               count(a.id) filter (where a.signup_at >= v_since) as signups,
               count(a.id) filter (where a.activated_at >= v_since) as activated,
               count(a.id) filter (where a.first_payment_at >= v_since) as paid,
               coalesce(sum(a.revenue_kes) filter (where a.first_payment_at >= v_since),0) as revenue_kes
        from public.hq_growth_agents g
        left join public.hq_growth_attributions a on a.agent_id=g.id
        group by g.id
      ) x
    ), '[]'::jsonb),
    'creators', coalesce((
      ...
      from (
        select g.id,g.creator_code,g.display_name,g.platform,g.handle,g.commission_rate,g.status,
               ...
               count(a.id) filter (where a.first_payment_at >= v_since) as paid,
               coalesce(sum(a.revenue_kes) filter (where a.first_payment_at >= v_since),0) as revenue_kes
        from public.hq_growth_creators g
        left join public.hq_growth_attributions a on a.creator_id=g.id
        group by g.id
      ) x
    ), '[]'::jsonb),
    'schools', coalesce((
      ...
      from (
        ...
        from public.hq_growth_school_pipeline p
        join public.schools s on s.id=p.school_id
        left join public.hq_growth_agents a on a.id=p.assigned_agent_id
...
comment on function public.hq_growth_command_overview(integer) is
  'Owner-authorized Growth & Distribution Command read model. Deterministic evidence only; no automated outreach, payments, commissions, publishing, or worker-runtime activation.';
