-- VLR-ASSESS-001B: grounded practice authority
...
create index if not exists assessment_items_source_block_id_idx
  on public.assessment_items(source_block_id)
  where source_block_id is not null;

alter table public.student_practice_attempts
  add column if not exists source_publication_id uuid null references public.vibe_publications(id) on delete set null,
  add column if not exists source_chapter_id uuid null references public.vibe_chapters(id) on delete set null,
  add column if not exists source_block_id uuid null references public.content_blocks(id) on delete set null,
  add column if not exists outcome_id uuid null references public.curriculum_learning_outcomes(id) on delete set null;

create index if not exists student_practice_attempts_source_block_idx
  on public.student_practice_attempts(student_id, source_block_id, attempted_at desc)
  where source_block_id is not null;
create index if not exists student_practice_attempts_outcome_idx
  on public.student_practice_attempts(student_id, outcome_id, attempted_at desc)
...
create index if not exists student_mistake_notebook_source_block_idx
  on public.student_mistake_notebook(student_id, source_block_id, status)
  where source_block_id is not null;
...
declare v_profile_id uuid:=auth.uid(); v_student_id uuid; v_block public.content_blocks%rowtype; v_pub public.vibe_publications%rowtype; v_chapter public.vibe_chapters%rowtype; v_outcome_id uuid; v_attempt_id uuid; v_evidence_id uuid; v_subject text; v_topic text;
begin
 if v_profile_id is null then raise exception 'Authentication required'; end if;
