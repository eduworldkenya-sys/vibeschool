...
-- Task 19 ordering recovery: a provider callback may reach VibeSchool before the
-- initiating request has finished persisting CheckoutRequestID. Attaching the
-- provider request must replay that already-durable callback in the same DB transaction.
  ...
  if v_event_id is not null then
    ...
    if coalesce((v_processed->>'success')::boolean,false) is not true then
      ...
      return jsonb_build_object(
        'success',false,'error','early_callback_replay_failed','state','reconciliation_required',
        'callback_result',v_processed
      );
    end if;
    return jsonb_build_object('success',true,'state',case when coalesce(v_processed->>'state','')='settled' then 'settled' else 'awaiting_customer' end,'early_callback_replayed',true,'callback_result',v_processed);
  end if;
