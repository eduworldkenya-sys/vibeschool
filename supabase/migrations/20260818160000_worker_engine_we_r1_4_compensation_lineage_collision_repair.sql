...
-- Fail closed on an unexpected pre-existing shape rather than masking another lineage.
do $shape$
declare
  v_type text;
begin
  ...
  select data_type into v_type from information_schema.columns
   where table_schema='public' and table_name='hq_workforce_execution_intents'
     and column_name='authoritative_before_state';
  if v_type is not null and v_type <> 'jsonb' then
    raise exception 'R1.4 compensation lineage repair unexpected authoritative_before_state type: %',v_type;
  end if;

  select data_type into v_type from information_schema.columns
   where table_schema='public' and table_name='hq_workforce_execution_intents'
     and column_name='expected_after_state';
  if v_type is not null and v_type <> 'jsonb' then
    raise exception 'R1.4 compensation lineage repair unexpected expected_after_state type: %',v_type;
  end if;
end
$shape$;
...
-- If an object with this name existed before the repair, require the canonical columns.
do $comp_shape$
declare
  v_missing text;
begin
  ...
  from unnest(array[
    'id','intent_id','task_id','authority_grant_id','plan_step_id','capability_key',
    'capability_version','requested_by','reason','before_state','expected_current_state',
    'observed_current_state','outcome','evidence','created_at'
  ]) c
  where not exists (
    select 1 from information_schema.columns
     where table_schema='public' and table_name='hq_workforce_execution_compensations'
       and column_name=c
  );
  if v_missing is not null then
    raise exception 'R1.4 compensation lineage repair unexpected compensation table shape; missing: %',v_missing;
  end if;
end
$comp_shape$;
...
-- The later 20260815130200 exact-state hardening is already the canonical RPC/trigger
-- implementation. Never replace it with the older 151301 function bodies here.
do $canonical$
begin
  if to_regprocedure('public.hq_workforce_capture_execution_authoritative_after_state()') is null then
    raise exception 'R1.4 compensation lineage repair missing canonical exact-state capture function';
  end if;
  if to_regprocedure('public.hq_workforce_compensate_consequential_execution(uuid,text,text)') is null then
    raise exception 'R1.4 compensation lineage repair missing canonical compensation RPC';
  end if;
  if not exists (
    ...
    where n.nspname='public' and c.relname='hq_work_items'
      and t.tgname='trg_hq_workforce_capture_execution_authoritative_after_state'
      and not t.tgisinternal
  ) then
    raise exception 'R1.4 compensation lineage repair missing canonical exact-state capture trigger';
  end if;
end
$canonical$;
...
declare ec public.hq_workforce_engine_contract%rowtype;
begin
  if to_regclass('public.hq_workforce_execution_compensations') is null then
    raise exception 'R1.4 compensation lineage repair table postcondition failed';
  end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='hq_workforce_execution_intents' and column_name='authoritative_before_state' and data_type='jsonb')
     or not exists (select 1 from information_schema.columns where table_schema='public' and table_name='hq_workforce_execution_intents' and column_name='expected_after_state' and data_type='jsonb') then
    raise exception 'R1.4 compensation lineage repair state-column postcondition failed';
  end if;
  if not exists (select 1 from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname='hq_workforce_execution_compensations' and t.tgname='trg_hq_workforce_execution_compensation_immutable' and not t.tgisinternal) then
    raise exception 'R1.4 compensation lineage repair immutable-trigger postcondition failed';
  end if;
  ...
  if coalesce(ec.heartbeat_enabled,false)
     ...
     or coalesce(ec.shadow_scheduler_enabled,false)
     or not coalesce(ec.shadow_global_stop,true) then
    raise exception 'R1.4 compensation lineage repair changed runtime safety state';
  end if;
end
$postflight$;
