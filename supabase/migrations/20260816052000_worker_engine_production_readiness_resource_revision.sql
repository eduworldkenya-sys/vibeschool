-- Worker Engine production-readiness hardening: monotonic resource revision / ABA closure.
-- NON-ACTIVATING. No runtime, authority, heartbeat, Factory, Shadow, or autonomy state changes.
-- access: service-only Worker Engine control functions; hq_work_items keeps its existing access model.
-- authorization-test: public/anon/authenticated remain unable to execute consequential control functions.

alter table public.hq_work_items
  add column if not exists resource_revision bigint not null default 0 check (resource_revision >= 0);

create or replace function public.hq_workforce_bump_work_item_revision()
returns trigger
language plpgsql
security definer
...
as $$
begin
  -- Revision is database-owned. Every UPDATE advances it, including human/product/legacy updates,
  -- so a value that changes A->B->A can never recreate an old consequential precondition.
  new.resource_revision := old.resource_revision + 1;
  return new;
end $$;

drop trigger if exists trg_hq_workforce_bump_work_item_revision on public.hq_work_items;
create trigger trg_hq_workforce_bump_work_item_revision
before update on public.hq_work_items
for each row execute function public.hq_workforce_bump_work_item_revision();

revoke all on function public.hq_workforce_bump_work_item_revision() from public,anon,authenticated;
...
begin
  ...
  if tc.handler_key='work_item.triage_and_own' then
    ...
    begin
      v_expected_revision:=(v_precondition->>'resource_revision')::bigint;
    exception when others then
      raise exception 'priority_canary_revision_invalid';
    end;
  ...
  begin
    ...
    if tc.handler_key='work_item.triage_and_own' then
      ...
      update public.hq_work_items
       ...
       where id=work_item_id and resource_revision=v_expected_revision;
    ...
    result:=jsonb_build_object('handler',tc.handler_key,'work_item_id',work_item_id,'worker_key',t.worker_key,
      ...
      'resource_revision_after',case when tc.handler_key='work_item.prioritize' then v_expected_revision+1 else null end,
...
-- Verification must prove the exact post-mutation revision as well as priority and queue membership.
...
begin
  ...
  if v_is_priority_canary then
    if not (i.expected_after_state ? 'priority' and i.expected_after_state ? 'resource_revision') or jsonb_object_length(i.expected_after_state)<>2 then raise exception 'priority_canary_verification_expected_state_invalid'; end if;
    select * into wi from public.hq_work_items where id=v_work_item_id;
    v_expected:=jsonb_build_object('resource_exists',true,'queue_member',true,'priority',i.expected_after_state->>'priority','resource_revision',(i.expected_after_state->>'resource_revision')::bigint,'execution_intent_id',i.id::text,'authority_grant_id',i.authority_grant_id::text,'plan_step_id',i.plan_step_id::text);
    if not found then
      v_observed:=jsonb_build_object('resource_exists',false,'queue_member',false,'priority',null,'resource_revision',null,'execution_intent_id',i.id::text,'authority_grant_id',i.authority_grant_id::text,'plan_step_id',i.plan_step_id::text);
    else
      v_observed:=jsonb_build_object('resource_exists',true,'queue_member',exists(select 1 from public.hq_workforce_canary_queue_memberships m where m.work_item_id=wi.id and m.queue_key='worker_engine_internal'),'priority',wi.priority,'resource_revision',wi.resource_revision,'execution_intent_id',i.id::text,'authority_grant_id',i.authority_grant_id::text,'plan_step_id',i.plan_step_id::text);
    end if;
    ...
    if not found then v_expected:=jsonb_build_object('resource_exists',true,'desired_state',i.desired_state,'task_id',t.id,'intent_id',i.id); v_observed:=jsonb_build_object('resource_exists',false); v_pass:=false;
      ...
      v_expected:=jsonb_build_object('resource_exists',true,'status',i.desired_state->>'status','task_id',t.id::text,'authority_grant_id',i.authority_grant_id::text,'plan_step_id',i.plan_step_id::text,'execution_intent_id',i.id::text);
      v_observed:=jsonb_build_object('resource_exists',true,'status',wi.status,'task_id',wi.action_taken->>'task_id','authority_grant_id',wi.action_taken->>'authority_grant_id','plan_step_id',wi.action_taken->>'plan_step_id','execution_intent_id',wi.action_taken->>'execution_intent_id');
      v_pass:=v_expected=v_observed;
    end if;
  ...
  values(i.id,t.id,i.authority_grant_id,i.plan_step_id,i.capability_key,i.capability_version,btrim(p_verifier_key),v_expected,v_observed,g.verification_contract,v_pass) returning id into v_id;
...
-- Compensation compares both expected priority and expected revision. Successful compensation
-- restores priority but advances the database-owned revision again, preserving monotonic history.
...
begin
  ...
  v_is_priority_canary:=t.capability_key='internal.work_queue.prioritize' and t.capability_version=1 and t.operation='update_priority' and t.resource_type='hq_work_items' and t.scope_type='platform_internal';
  if v_is_priority_canary then
    if not (i.authoritative_before_state ? 'priority' and i.authoritative_before_state ? 'resource_revision' and i.expected_after_state ? 'priority' and i.expected_after_state ? 'resource_revision') then raise exception 'priority_canary_compensation_state_incomplete'; end if;
  elsif not (i.expected_after_state ? 'status' and i.expected_after_state ? 'action_taken' and i.expected_after_state ? 'acted_at' and i.expected_after_state ? 'updated_at') then raise exception 'compensation_authoritative_after_state_incomplete'; end if;
  select * into g from public.hq_workforce_capability_authority_grants where id=i.authority_grant_id;
  ...
  if v_is_priority_canary then v_observed:=jsonb_build_object('priority',wi.priority,'resource_revision',wi.resource_revision);
  else v_observed:=jsonb_build_object('status',wi.status,'action_taken',coalesce(wi.action_taken,'null'::jsonb),'acted_at',case when wi.acted_at is null then null else to_jsonb(wi.acted_at) end,'updated_at',case when wi.updated_at is null then null else to_jsonb(wi.updated_at) end); end if;
  if v_observed is distinct from i.expected_after_state then
    insert into public.hq_workforce_execution_compensations(intent_id,task_id,authority_grant_id,plan_step_id,capability_key,capability_version,requested_by,reason,before_state,expected_current_state,observed_current_state,outcome,evidence)
    values(i.id,t.id,i.authority_grant_id,i.plan_step_id,i.capability_key,i.capability_version,btrim(p_requested_by),btrim(p_reason),i.authoritative_before_state,i.expected_after_state,v_observed,'conflict_escalated',jsonb_build_object('cause',case when v_is_priority_canary then 'current_priority_or_revision_diverged' else 'current_state_diverged' end,'mutation_applied',false,'aba_safe',true)) returning id into v_comp_id;
    return jsonb_build_object('compensation_id',v_comp_id,'outcome','conflict_escalated','mutation_applied',false);
  end if;
  if v_is_priority_canary then
    update public.hq_work_items set priority=i.authoritative_before_state->>'priority' where id=v_work_item_id and resource_revision=(i.expected_after_state->>'resource_revision')::bigint;
    if not found then raise exception 'priority_canary_compensation_revision_compare_and_set_failed'; end if;
  else
    v_before_action:=case when i.authoritative_before_state->'action_taken'='null'::jsonb then null else i.authoritative_before_state->'action_taken' end;
    v_before_acted_at:=nullif(i.authoritative_before_state->>'acted_at','')::timestamptz;
    update public.hq_work_items set status=i.authoritative_before_state->>'status',action_taken=v_before_action,acted_at=v_before_acted_at,updated_at=clock_timestamp() where id=v_work_item_id;
  end if;
...
begin
  ...
  if not found then raise exception 'production readiness revision hardening requires engine contract'; end if;
  if coalesce(ec.heartbeat_enabled,false) or coalesce(ec.factory_enabled,false) or coalesce(ec.runtime_execution_enabled,false)
    ...
    raise exception 'resource revision hardening changed runtime boundary';
  end if;
  select count(*) into v_active from public.hq_workforce_capability_authority_grants where status='active';
  if v_active<>0 then raise exception 'resource revision hardening activated authority'; end if;
end $$;
