...
revoke all on public.auth_identity_reconciliation_findings from public, anon, authenticated;
grant select, insert, update on public.auth_identity_reconciliation_findings to service_role;
...
revoke all on public.auth_identity_reconciliation_actions from public, anon, authenticated;
grant select, insert on public.auth_identity_reconciliation_actions to service_role;
...
declare
  ...
  v_count integer;
begin
  -- Findings are evidence, not authority. No role is inferred or granted here.
  ...
  select
    'AUTH_PROFILE_MISSING:' || u.id,
    u.id,
    'AUTH_PROFILE_MISSING',
    'recoverable_partial_provisioning',
    'P1',
    'automatic_neutral',
    jsonb_build_object(
  ...
  select
    'STUDENT_DOMAIN_MISSING:' || p.id,
    p.id,
    'STUDENT_DOMAIN_MISSING',
    'orphaned_domain_relation',
    'P1',
    'manual_proof_required',
    jsonb_build_object('account_status',p.account_status::text,'profile_school_id',p.school_id),
    v_scan_started,
    null
  ...
  select
    'ADMIN_MEMBERSHIP_MISSING:' || p.id,
    p.id,
    'ADMIN_MEMBERSHIP_MISSING',
    ...
    'manual_proof_required',
    jsonb_build_object('account_status',p.account_status::text,'is_anonymized',p.is_anonymized,'profile_school_id',p.school_id),
    v_scan_started,
    null
  from public.profiles p
  where p.role='admin'
  ...
  select
    'MULTI_PROVIDER_IDENTITY:' || i.user_id,
    i.user_id,
    'MULTI_PROVIDER_IDENTITY',
    'valid_transitional_state',
    'INFO',
    'none',
    jsonb_build_object('providers',jsonb_agg(i.provider order by i.provider),'identity_count',count(*)),
    v_scan_started,
    null
  ...
  update public.auth_identity_reconciliation_findings f
  set resolved_at=v_scan_started
  where f.resolved_at is null
    and f.last_seen_at < v_scan_started;

  select count(*) into v_count
  from public.auth_identity_reconciliation_findings
  where resolved_at is null;

  return jsonb_build_object('scan_started_at',v_scan_started,'open_findings',v_count);
end;
$$;
...
begin
  ...
  values(
    'AUTH_PROFILE_MISSING:' || p_user_id,
    p_user_id,
...
-- Administrative authority must be backed by a school membership. This closes
-- the invalid state where profiles.role='admin' alone could authorize routing.
...
as $$
  select case
    when auth.uid() is null then
      jsonb_build_object('authenticated',false,'role',null,'account_status',null,'is_anonymized',false,'profile_complete',false,'authority_complete',false,'reason_code','AUTHENTICATION_REQUIRED')
    else
      coalesce((
        select jsonb_build_object(
          ...
          'authority_complete',case
            when p.role='admin' then exists(
              select 1 from public.school_members sm
              where sm.profile_id=p.id and sm.role::text in ('admin','owner')
            )
            else p.role is not null
          end,
          'reason_code',case
      ...
      ),jsonb_build_object('authenticated',true,'role',null,'account_status',null,'is_anonymized',false,'profile_complete',false,'authority_complete',false,'reason_code','PROFILE_MISSING'))
  end;
$$;
...
comment on function public.repair_missing_neutral_profile(uuid) is 'Service-only deterministic recovery for an Auth user missing a profile. Creates role=NULL and grants no authority.';
comment on function public.refresh_auth_identity_reconciliation() is 'Service-only idempotent scan that classifies identity integrity findings without mutating user authority.';
