  ...
  loop
    if not exists (
      ...
      where p.student_id=v_student_id and p.plan_date between current_date and current_date+7 and p.status='planned'
        and (p.source->>'evidence_key'=v_candidate.evidence_key or (lower(p.subject)=lower(v_candidate.subject) and lower(p.topic)=lower(v_candidate.topic) and p.activity_type='spaced_revision'))
    ) then
      ...
      values(v_student_id,v_candidate.due_date,v_candidate.subject,v_candidate.topic,'spaced_revision',15,case v_candidate.lane_priority when 1 then 1 when 2 then 1 else 2 end,v_candidate.reason,'/student/vibelearn/revision','planned',jsonb_build_object('source','twin_forgetting_engine','lane',v_candidate.lane,'evidence_key',v_candidate.evidence_key,'outcome_id',v_candidate.outcome_id,'forgetting_risk',round(v_candidate.forgetting_risk,3),'days_since_evidence',v_candidate.age_days,'identity_bridge',case when v_candidate.lane in ('scheduled_retest','mistake_recovery') then 'canonical_plus_profile_history' else 'canonical_mastery' end));
      v_count:=v_count+1;
    end if;
  end loop;
  return v_count;
