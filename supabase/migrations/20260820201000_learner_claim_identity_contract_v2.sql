...
create unique index if not exists uq_student_claim_codes_active_shared_per_student
on public.student_claim_codes(student_id)
where role='shared' and claimed=false;
...
begin
  ...
  select exists (
    ...
    where tc.class_id=v_student.class_id
      and tc.teacher_id=auth.uid()
  ) or exists (
    select 1
    from public.school_members sm
    join public.classes c on c.school_id=sm.school_id
    where sm.profile_id=auth.uid()
      and sm.role::text in ('owner','admin')
      and c.id=v_student.class_id
  ) into v_allowed;

  if not v_allowed then raise exception 'unauthorized_teacher'; end if;

  delete from public.student_claim_codes
  where student_id=p_student_id and role='shared' and claimed=false;
  ...
  return jsonb_build_object(
    'status','success','student_id',p_student_id,'student_name',v_student.name,
    'code',v_code,'expires_at',v_expires
  );
end;
$$;
...
begin
  ...
  where code=upper(trim(p_code)) and role='shared'
  limit 1;
  ...
  return jsonb_build_object(
    ...
    'guardian_linked',v_guardian_linked,
    'parent_claimed',v_row.parent_claimed_at is not null,
    'student_claimed',v_row.student_claimed_at is not null,
...
begin
  ...
  select * into v_student
  from public.students where id=v_row.student_id
  limit 1 for update;
  ...
  select c.school_id into v_school_id from public.classes c where c.id=v_student.class_id;
  if v_school_id is null then return jsonb_build_object('status','school_not_found'); end if;

  v_parent_linked := exists(
    select 1 from public.parent_student_links psl
    where psl.student_id=v_student.id and psl.school_id=v_school_id
      and coalesce(psl.access_level::text,'') <> 'none'
  );
