...
create table if not exists public.hq_cyborg_missions (
  ...
  owner_gates jsonb not null default '[]'::jsonb,
  ...
  no_progress_cycles integer not null default 0 check (no_progress_cycles >= 0),
...
create table if not exists public.hq_cyborg_mission_events (
  ...
  mission_id uuid not null references public.hq_cyborg_missions(id),
...
create table if not exists public.hq_cyborg_mission_leases (
  mission_id uuid primary key references public.hq_cyborg_missions(id),
...
revoke all on public.hq_cyborg_missions from anon, authenticated;
revoke all on public.hq_cyborg_mission_events from anon, authenticated;
revoke all on public.hq_cyborg_mission_leases from anon, authenticated;
revoke all on public.hq_cyborg_slo_events from anon, authenticated;
...
grant usage,select on sequence public.hq_cyborg_mission_events_id_seq to service_role;
grant usage,select on sequence public.hq_cyborg_slo_events_id_seq to service_role;
...
returns public.hq_cyborg_mission_leases language plpgsql security definer set search_path=public as $$
...
revoke all on function public.hq_cyborg_acquire_lease(uuid,text,text,integer) from public,anon,authenticated;
revoke all on function public.hq_cyborg_append_event(uuid,text,jsonb,text) from public,anon,authenticated;
grant execute on function public.hq_cyborg_acquire_lease(uuid,text,text,integer) to service_role;
grant execute on function public.hq_cyborg_append_event(uuid,text,jsonb,text) to service_role;
