...
-- CBC updates must preserve teacher/admin authority AND canonical current class membership.
...
-- A learner may submit only to a project assigned to their current canonical class/school.
...
-- Parent links are identity/authorization records. Parents may read or unlink their own link,
-- but only school authority may mutate a link's identity-bearing fields.
...
-- Fail closed on the three retargeting boundaries.
do $$
begin
  if not exists (
