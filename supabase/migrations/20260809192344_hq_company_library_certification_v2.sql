-- Company Library certification v2: include approval/version lineage and storage-policy coverage.
...
as $$
  ...
  storage_policy_state as (
    ...
    where schemaname='storage' and tablename='objects'
      and (
        coalesce(qual,'') like '%hq-company-library%'
        or coalesce(with_check,'') like '%hq-company-library%'
      )
  ),
  integrity as (
    select
      (select count(*)::integer
       ...
       where a.current_version_id is not null and (v.id is null or v.artifact_id<>a.id)) as orphan_current_versions,
      (select count(*)::integer
       from public.hq_artifact_approvals ap
       join public.hq_artifact_versions v on v.id=ap.version_id
       where v.artifact_id<>ap.artifact_id) as cross_artifact_approvals,
      (select count(*)::integer
       from public.hq_artifact_provenance pr
       join public.hq_artifact_versions v on v.id=pr.version_id
       where pr.version_id is not null and v.artifact_id<>pr.artifact_id) as cross_artifact_provenance,
      (select count(*)::integer
       from public.hq_artifact_approvals ap
       join public.hq_artifacts a on a.id=ap.artifact_id
       where ap.status='pending' and ap.version_id is distinct from a.current_version_id) as stale_pending_approvals,
      (select count(*)::integer
       from public.hq_artifacts a
       where a.lifecycle_state='published'
         and (a.approval_state<>'approved' or a.current_version_id is null)) as published_without_approved_current,
      (select count(*)::integer from (
         select artifact_id,version_id,count(*)
         from public.hq_artifact_approvals
         where status='pending'
         group by artifact_id,version_id
         having count(*)>1
       ) d) as duplicate_pending_approvals
  )
