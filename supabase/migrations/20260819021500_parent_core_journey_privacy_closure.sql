...
--
-- Canonical rule: a parent relationship authorizes learner data only while
-- public.is_parent_of_student(student_id) is true. That helper already binds
-- auth.uid() to parent_student_links and rejects access_level = 'none'.
--
-- This migration also closes parent visibility of unreleased assessment data
-- and removes the legacy parent -> authoritative school fee payment write path.
...
-- Keep the active-parent predicate deterministic and explicit in migration
-- lineage. The function remains callable from RLS but is not a client mutation.
...
-- Attendance: revoked links must fail closed.
...
-- Homework is class-scoped, so derive authorized classes only from active links.
drop policy if exists homework_parent_read on public.homework;
create policy homework_parent_read
on public.homework
for select
to authenticated
using (
  exists (
    ...
    join public.student_classes sc
      on sc.student_id = psl.student_id
...
-- Submission visibility already had access_level protection; express it through
-- the same canonical helper so future revocation semantics cannot drift.
drop policy if exists homework_submissions_parent_read on public.homework_submissions;
create policy homework_submissions_parent_read
on public.homework_submissions
for select
to authenticated
using ((select public.is_parent_of_student(homework_submissions.student_id)));

-- Parent-facing teacher messages require a currently active relationship.
...
-- Modern assessment engine: parents see only explicitly released gradebook rows.
drop policy if exists assessment_gradebook_parent_read on public.assessment_gradebook_entries;
create policy assessment_gradebook_parent_read
on public.assessment_gradebook_entries
for select
to authenticated
using (
  assessment_gradebook_entries.released_at is not null
  and (select public.is_parent_of_student(assessment_gradebook_entries.student_id))
);

-- Legacy exam results have no released_at field. For pilot use, exam lock is the
-- fail-closed publication boundary: teachers cannot mutate a locked exam and
-- parents cannot see rows from an exam that is still open/editable.
...
-- Legacy traditional_grades has no publication state. Parent product no longer
-- consumes it directly, so fail closed rather than exposing provisional marks.
drop policy if exists trad_grades_parent_read on public.traditional_grades;

-- Parent learning summaries already enforce published status; converge their
-- relationship predicate on the canonical helper and retire the unused legacy
-- parent_students authorization branch.
drop policy if exists parent_summaries_learner_parent_read on public.parent_learning_summaries;
create policy parent_summaries_learner_parent_read
...
-- Finance: active relationship + explicit finance permission is required for
-- all parent reads. Parents can submit evidence only through the R1 payment
-- claims boundary; they can no longer create authoritative school ledger rows.
...
-- Class-level learning resources used by Parent Learn must also derive their
-- access from an active relationship, never from an arbitrary browser class id.
drop policy if exists exercises_parent_read on public.exercises;
create policy exercises_parent_read
on public.exercises
for select
to authenticated
using (
...
create policy projects_parent_read
on public.projects
for select
to authenticated
using (
...
drop policy if exists pol_lesson_content_select_parent on public.lesson_content;
create policy pol_lesson_content_select_parent
...
using (
  exists (
    ...
    join public.student_classes sc
      on sc.class_id = lp.class_id
     and sc.is_current = true
    join public.parent_student_links psl
      on psl.student_id = sc.student_id
    where lp.id = lesson_content.lesson_plan_id
      and psl.parent_id = (select auth.uid())
      and coalesce(psl.access_level, 'full') <> 'none'
      and (lesson_content.school_id is null or sc.school_id = lesson_content.school_id)
  )
);

-- Parent access to student homework answers must stop immediately on revocation.
...
-- Family-facing badges remain a deliberate parent action, but only for an
-- active relationship. Internal audit-log rows are not a parent product surface.
drop policy if exists "parent reads child badges" on public.child_badges;
create policy "parent reads child badges"
on public.child_badges
for select
to authenticated
using ((select public.is_parent_of_student(child_badges.student_id)));

drop policy if exists child_badges_insert on public.child_badges;
create policy child_badges_insert
on public.child_badges
for insert
to authenticated
with check ((select public.is_parent_of_student(child_badges.student_id)));
