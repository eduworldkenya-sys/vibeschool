...
declare ec record;
begin
 select heartbeat_enabled,factory_enabled,runtime_execution_enabled,runtime_autonomy_level,runtime_max_risk,shadow_enabled,shadow_scheduler_enabled,shadow_global_stop,shadow_anomaly_paused into ec from public.hq_workforce_engine_contract where singleton=true;
...
do $$ declare defs text; c text; kinds text; begin
 select string_agg(pg_get_functiondef(p.oid),E'\n') into defs from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('hq_workforce_shadow_evaluate_authority','hq_workforce_shadow_review_decision','hq_workforce_run_shadow_cycle','hq_workforce_shadow_recommend_candidate');
...
do $$ declare blocked boolean:=false; begin begin perform public.hq_workforce_run_shadow_cycle('off-contract',1); exception when others then blocked:=true; end; if not blocked then raise exception 'shadow_scheduler_ran_while_off'; end if; end $$;
...
begin
 ...
 values
 ('000-we-r1-3-acceptance-worker','operations.triage',1,.99,.99,'certified',array['platform_internal','global'],array['global']),
 ('000-we-r1-3-acceptance-worker','strategy.analysis',1,.96,.97,'certified',array['platform_internal','global'],array['global']);
 ...
 insert into public.hq_workforce_competency_capabilities(competency_key,skill_key,version,min_skill_version,priority,status,approved_at) values
 ('operations.triage','we_r1_3_acceptance_triage',1,1,1000,'approved',clock_timestamp()),
 ('strategy.analysis','we_r1_3_acceptance_triage',1,1,1000,'approved',clock_timestamp());
 ...
 if jsonb_array_length(rr->'workers')<>1 then raise exception 'single_worker_full_coverage_expected:%',rr; end if;
 tr:=(rr->>'trace_id')::uuid;
 select count(*) into n from public.hq_workforce_shadow_events where trace_id=tr; if n<>7 then raise exception 'trace_event_count_expected_7_got:%',n; end if;
 ...
 if (select count(*) from public.hq_workforce_plan_steps where plan_id=(rr->>'plan_id')::uuid)<>2 then raise exception 'full_competency_composition_not_built'; end if;
 ...
 select count(*) into n from public.hq_workforce_shadow_candidates where source_work_item_id=wid; if n<>1 then raise exception 'duplicate_candidate_inserted:%',n; end if;
 select status into after_status from public.hq_work_items where id=wid; if after_status is distinct from before_status then raise exception 'shadow_mutated_source_work'; end if;
