...
begin
  ...
  where n.nspname='public'
    and p.proname in (
      ...
      'hq_workforce_certify_learning_pipeline'
    ) and has_function_privilege('service_role',p.oid,'EXECUTE');
  if v_bad is not null then raise exception 'legacy service authority remains:%',v_bad; end if;
end $$;

-- Runtime and authority state are governance-owned, not service transport writable.
do $$
begin
  if has_table_privilege('service_role','public.hq_workforce_engine_contract','INSERT')
     ...
     or has_table_privilege('service_role','public.hq_workforce_engine_contract','TRUNCATE') then
    raise exception 'service_role can mutate engine contract';
  end if;
  if has_table_privilege('service_role','public.hq_workforce_runtime_policies','INSERT')
     ...
     or has_table_privilege('service_role','public.hq_workforce_runtime_policies','TRUNCATE') then
    raise exception 'service_role can mutate runtime policy';
  end if;
  if has_table_privilege('service_role','public.hq_workforce_capability_authority_grants','UPDATE') then
    raise exception 'service_role can activate capability authority directly';
  end if;
...
-- No explicit global policy means runtime authorization itself fails closed even if a
-- caller somehow reaches the assertion function through an internal path.
do $$
declare d text;
begin
  ...
  if has_function_privilege('service_role','public.hq_workforce_assert_runtime_task_authorized_r12_internal(uuid)','EXECUTE') then
    raise exception 'service_role bypasses explicit global policy wrapper';
  end if;
end $$;
...
begin
  if has_table_privilege('service_role','public.hq_workforce_objectives','UPDATE')
    ...
    raise exception 'service_role retains direct planning approval writes';
  end if;
  if has_function_privilege('service_role','public.hq_workforce_owner_review_objective(uuid,text,text,jsonb)','EXECUTE') then
    raise exception 'service_role can impersonate objective owner review';
  end if;
  select lower(pg_get_functiondef('public.hq_workforce_owner_review_objective(uuid,text,text,jsonb)'::regprocedure)) into d;
  if position('hq_assert_owner' in d)=0 or position('auth.uid()' in d)=0 or position('approved_plan_hash' in d)=0 then
    raise exception 'objective owner/plan approval binding incomplete';
  end if;
end $$;
...
begin
  ...
  if has_function_privilege('service_role','public.hq_workforce_owner_verify_memory(uuid,text,jsonb,boolean)','EXECUTE') then
    raise exception 'service_role can impersonate owner memory verification';
  end if;
end $$;
...
begin
  if has_function_privilege('service_role','public.hq_workforce_verify_consequential_execution_r14_unbound_internal(uuid,text)','EXECUTE') then
    raise exception 'unbound verifier externally callable';
  end if;
  ...
  if has_table_privilege('service_role','public.hq_workforce_verifier_assignments','INSERT')
     or has_table_privilege('service_role','public.hq_workforce_verifier_assignments','UPDATE') then
    raise exception 'service_role can forge verifier assignment evidence';
  end if;
end $$;
...
begin
  ...
  if position('old.worker_state_revision+1' in d)=0 or position('new.updated_at:=clock_timestamp()' in d)=0 then
    raise exception 'work item version clock incomplete';
  end if;
  if not exists(select 1 from pg_trigger where tgrelid='public.hq_work_items'::regclass and tgname='trg_hq_workforce_advance_work_item_version' and not tgisinternal) then
    raise exception 'work item version trigger missing';
  end if;
end $$;

-- Breaker denial evidence is emitted after failed execution subtransaction rollback and
-- queue semantics treat explicit deny as failure.
do $$
...
begin
  ...
  if coalesce(ec.heartbeat_enabled,false) or coalesce(ec.factory_enabled,false)
    ...
    raise exception 'production closure changed fail-closed posture';
  end if;
