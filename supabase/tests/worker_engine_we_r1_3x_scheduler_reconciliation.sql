...
set shadow_enabled=true,shadow_scheduler_enabled=true,shadow_global_stop=false,shadow_anomaly_paused=false,heartbeat_enabled=false,factory_enabled=false,runtime_execution_enabled=false,runtime_autonomy_level=0,runtime_max_risk=0,shadow_max_cycles_per_hour=100,shadow_max_candidates_per_cycle=25,shadow_max_queue_depth=1000
...
do $$ declare oid uuid; mid uuid; pid uuid; sid uuid; cid uuid; begin
  ...
  insert into public.hq_workforce_plan_steps(plan_id,step_key,ordinal,purpose,actor_mode,input_contract,expected_output,verification_contract,required_autonomy,required_risk,status)
  values (pid,'analyze',1,'Analyze verified scheduler fixture','unassigned','{}'::jsonb,'{"recommendation":true}'::jsonb,'{"human_review":true}'::jsonb,0,0,'planned') returning id into sid;
...
-- Adversarial proof: create authoritative context in a valid state, bind it, then revoke it. The scheduler must revalidate every critical binding and fail closed.
do $$ declare oid uuid; good_mid uuid; bad_mid uuid; v jsonb; begin
  ...
  bad_mid:=public.hq_workforce_add_memory('test.x7.context.bad','fact','{"condition":"will_be_revoked"}'::jsonb,'{"suite":"x7","source":"adversarial"}'::jsonb,'acceptance','x7',1::numeric,'verified',true);
  perform public.hq_workforce_bind_objective_context(oid,good_mid,'required','Usable critical context.',3600::bigint);
  perform public.hq_workforce_bind_objective_context(oid,bad_mid,'policy','Policy context that will be revoked must block planning.',3600::bigint);
  update public.hq_workforce_memory_records set verification_state='revoked',updated_at=clock_timestamp() where id=bad_mid;
...
do $$ begin
  begin update public.hq_workforce_scheduler_events set outcome='tampered' where cycle_key='x7-cycle-plan'; raise exception 'X7 scheduler evidence was mutable';
  exception when others then if sqlerrm='X7 scheduler evidence was mutable' then raise; end if; if position('worker_engine_scheduler_evidence_is_append_only' in sqlerrm)=0 then raise; end if; end;
end $$;
...
do $$ declare ec public.hq_workforce_engine_contract%rowtype; begin
  ...
  if ec.heartbeat_enabled or ec.factory_enabled or ec.runtime_execution_enabled or ec.runtime_autonomy_level<>0 or ec.runtime_max_risk<>0 then raise exception 'X7 test crossed L0/R0 safety boundary'; end if;
end $$;
