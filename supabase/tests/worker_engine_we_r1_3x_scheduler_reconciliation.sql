...
-- Production migration state must remain disabled/fail-closed.
do $$ declare v jsonb; begin
  v:=public.hq_workforce_run_r1_3x_shadow_scheduler('x7-disabled-proof',10);
  if v->>'status'<>'disabled' or v->>'reason'<>'shadow_scheduler_global_stop' or coalesce((v->>'consequential_execution')::boolean,true) then
    raise exception 'X7 scheduler did not fail closed while Shadow is disabled: %',v;
  end if;
end $$;

-- Legacy scheduled heartbeat is a tombstone and cannot wake autonomous workers.
do $$ declare v jsonb; begin
  v:=public.hq_workforce_scheduled_heartbeat();
  if v->>'status'<>'retired' or v->>'mode'<>'compatibility_tombstone' or coalesce((v->>'consequential_execution')::boolean,true) then
    raise exception 'X7 scheduled heartbeat compatibility boundary invalid: %',v;
  end if;
end $$;

-- The old R1.3 shadow-cycle API must be a compatibility bridge, not an open-work-item scanner.
...
insert into public.hq_workforce_capabilities(
  ...
  'test.x7.scheduler.analysis',1,'X7 Scheduler Analysis','Prove objective-first scheduler orchestration',0,0,'certified','{"suite":"x7","kind":"scheduler"}'
);

insert into public.hq_workforce_capability_competencies(capability_id,competency_key,required,weight,minimum_proficiency)
select id,'scheduler.analysis',true,1,.6
from public.hq_workforce_capabilities where capability_key='test.x7.scheduler.analysis';

insert into public.hq_workforce_resources(
  resource_key,version,resource_kind,display_name,enabled,shadow_capable,health_status,reliability,cost_per_unit,cost_unit,
  latency_class,required_autonomy,risk_class,allowed_scope_types,jurisdictions,allowed_data_classifications,provenance
) values (
  'test.x7.deterministic.context',1,'deterministic','X7 Deterministic Context Resolver',true,true,'healthy',.99,0,'count',0,0,0,
  array['platform_internal'],array['global'],array['internal'],'{"suite":"x7","kind":"resource"}'
);
...
select c.id,r.id,'read',true,.9,100,'{}'::jsonb
from public.hq_workforce_capabilities c
join public.hq_workforce_resources r on r.resource_key='test.x7.deterministic.context'
where c.capability_key='test.x7.scheduler.analysis';

-- Deliberately unrelated department: department must never be an eligibility gate.
...
insert into public.hq_workforce_worker_competencies(
  ...
  'x7-scheduler-specialist','scheduler.analysis',1,.97,.96,60,'certified','{"suite":"x7"}',array['platform_internal'],array['global']
);

-- Temporarily enable only Shadow scheduling inside this transaction; heartbeat, Factory, execution and autonomy remain OFF/L0/R0.
update public.hq_workforce_engine_contract
set shadow_enabled=true,shadow_scheduler_enabled=true,shadow_global_stop=false,shadow_anomaly_paused=false,
    heartbeat_enabled=false,factory_enabled=false,runtime_execution_enabled=false,runtime_autonomy_level=0,runtime_max_risk=0,
    shadow_max_cycles_per_hour=100,shadow_max_candidates_per_cycle=25,shadow_max_queue_depth=1000
where singleton=true;

-- Create a first-class objective. First scheduler pass may only acknowledge it and request governed context.
do $$ declare oid uuid; v jsonb; begin
  ...
  if (select status from public.hq_workforce_objectives where id=oid)<>'context_pending' then
    raise exception 'X7 detected objective did not advance only to governed context stage';
  end if;
  if exists(select 1 from public.hq_workforce_scheduler_events where cycle_key='x7-cycle-detect' and objective_id=oid and stage='planning') then
    raise exception 'X7 scheduler skipped context boundary';
  end if;
end $$;

-- Bind verified governed context, then prepare a candidate plan outside scheduler policy.
do $$ declare oid uuid; mid uuid; pid uuid; sid uuid; cid uuid; begin
  ...
  insert into public.hq_workforce_plan_steps(
    plan_id,step_key,ordinal,purpose,actor_mode,input_contract,expected_output,verification_contract,required_autonomy,required_risk,status
  ) values (
    pid,'analyze',1,'Analyze verified scheduler fixture','unassigned','{}'::jsonb,'{"recommendation":true}'::jsonb,'{"human_review":true}'::jsonb,0,0,'planned'
  ) returning id into sid;
  select id into cid from public.hq_workforce_capabilities where capability_key='test.x7.scheduler.analysis';
  insert into public.hq_workforce_plan_step_capabilities(plan_step_id,capability_id,role,minimum_coverage)
...
do $$ declare v jsonb; oid uuid; begin
  ...
  if not exists(
    ...
    join public.hq_workforce_plan_steps ps on ps.id=psr.plan_step_id
    join public.hq_workforce_plans p on p.id=ps.plan_id
    where p.objective_id=oid and psr.required
  ) then raise exception 'X7 did not route through canonical resource resolution'; end if;
  if not exists(
    select 1 from public.hq_workforce_routing_events re
    where re.objective_id=oid and re.routing_mode='single_worker' and re.selected_workers @> array['x7-scheduler-specialist']::text[]
      and re.rationale->>'department_is_hard_gate'='false'
  ) then raise exception 'X7 did not route through competency graph independent of department'; end if;
...
-- Scheduler evidence must be append-only.
do $$ begin
  begin
    update public.hq_workforce_scheduler_events set outcome='tampered' where cycle_key='x7-cycle-plan';
...
-- Product roles have no direct scheduler table/function authority.
do $$ begin
  if has_table_privilege('anon','public.hq_workforce_scheduler_events','SELECT') or has_table_privilege('authenticated','public.hq_workforce_scheduler_events','SELECT') then
    raise exception 'X7 scheduler evidence leaked to product roles';
  end if;
  if has_function_privilege('anon','public.hq_workforce_run_r1_3x_shadow_scheduler(text,integer)','EXECUTE')
     or has_function_privilege('authenticated','public.hq_workforce_run_r1_3x_shadow_scheduler(text,integer)','EXECUTE') then
    raise exception 'X7 canonical scheduler executable by product roles';
  end if;
...
-- Even while the test temporarily enables Shadow scheduling, consequential authority never moved.
do $$ declare ec public.hq_workforce_engine_contract%rowtype; begin
  ...
  if ec.heartbeat_enabled or ec.factory_enabled or ec.runtime_execution_enabled or ec.runtime_autonomy_level<>0 or ec.runtime_max_risk<>0 then
    raise exception 'X7 test crossed L0/R0 safety boundary';
  end if;
end $$;
