...
begin
  foreach r in array array['public','anon','authenticated'] loop
    if has_table_privilege(r,'public.hq_workforce_execution_breakers','SELECT')
       ...
       or has_table_privilege(r,'public.hq_workforce_execution_breakers','DELETE') then
      raise exception 'unexpected breaker privilege for %',r;
    end if;
    if has_table_privilege(r,'public.hq_workforce_execution_breaker_events','SELECT')
       ...
       or has_table_privilege(r,'public.hq_workforce_execution_breaker_events','DELETE') then
      raise exception 'unexpected breaker event privilege for %',r;
    end if;
  end loop;
  ...
  if has_table_privilege('service_role','public.hq_workforce_execution_breakers','INSERT')
     or has_table_privilege('service_role','public.hq_workforce_execution_breakers','UPDATE')
     or has_table_privilege('service_role','public.hq_workforce_execution_breakers','DELETE') then raise exception 'service_role breaker table must be read-only'; end if;
  if has_table_privilege('service_role','public.hq_workforce_execution_breaker_events','INSERT')
     or has_table_privilege('service_role','public.hq_workforce_execution_breaker_events','UPDATE')
     or has_table_privilege('service_role','public.hq_workforce_execution_breaker_events','DELETE') then raise exception 'service_role breaker event table must be read-only'; end if;
...
declare
  ...
  ec_before public.hq_workforce_engine_contract%rowtype;
  ec_after public.hq_workforce_engine_contract%rowtype;
begin
...
begin
  ...
  begin
    update public.hq_workforce_execution_breaker_events set reason_code='rewritten' where breaker_id=bid;
...
-- Canonical gateway must place breaker checks after replay handling but before budget,
-- limits and consequential mutation, and retain the legacy single entrypoint.
do $$
declare body text; p_replay integer; p_stop integer; p_budget integer; p_limits integer; p_stop2 integer; p_mut integer;
begin
  select pg_get_functiondef('public.hq_workforce_consequential_execution_gateway(uuid)'::regprocedure) into body;
  p_replay:=position('idempotent_replay' in body);
  p_stop:=position('pre_reservation' in body);
  p_budget:=position('hq_workforce_reserve_budget' in body);
  p_limits:=position('hq_workforce_reserve_capability_execution' in body);
  p_stop2:=position('pre_mutation' in body);
  p_mut:=position('update public.hq_work_items' in body);
  ...
  if not (p_replay<p_stop and p_stop<p_budget and p_budget<p_limits and p_limits<p_stop2 and p_stop2<p_mut) then
    raise exception 'gateway breaker ordering unsafe';
  end if;
...
begin
  ...
  begin
    perform public.hq_workforce_assert_execution_not_stopped(gen_random_uuid(),'pre_reservation');
  exception when others then failed:=true; end;
  -- Missing task must still fail closed; the assertion cannot silently succeed.
...
begin
  ...
  if coalesce(ec.heartbeat_enabled,false) or coalesce(ec.factory_enabled,false)
    ...
    raise exception 'R1.4.8 changed runtime safety boundary';
  end if;
