...
begin
  if to_regprocedure('public.hq_autopilot_constitution_snapshot()') is null
     or to_regprocedure('public.hq_autopilot_founder_brief()') is null then raise exception 'autopilot_read_model_missing'; end if;
  if has_function_privilege('service_role','public.hq_autopilot_constitution_snapshot()','EXECUTE')
     or has_function_privilege('service_role','public.hq_autopilot_founder_brief()','EXECUTE') then raise exception 'service_role_can_impersonate_founder_read_model'; end if;
  ...
  if position('hq_assert_owner' in d)=0 or position('verification_status=''verified''' in d)=0 then raise exception 'founder_brief_truth_contract_incomplete'; end if;
end $$;
...
begin
  ...
  if position('hq_assert_owner' in d)=0 or position('runtime_activation_active_capability_authority_required' in d)=0 then raise exception 'runtime_activation_constitution_incomplete'; end if;
end $$;
...
begin
  if to_regprocedure('public.hq_workforce_consequential_execution_gateway(uuid)') is null
     or to_regprocedure('public.hq_workforce_consequential_execution_gateway_r14_approval_bound_internal(uuid)') is null then
    raise exception 'canonical_gateway_missing';
  end if;
  select lower(pg_get_functiondef('public.hq_workforce_consequential_execution_gateway_r14_approval_bound_internal(uuid)'::regprocedure)) into d;
  if position('approved_plan_hash' in d)=0 or position('autonomous_authority_grant_id' in d)=0 then raise exception 'canonical_gateway_plan_or_authority_binding_missing'; end if;
...
begin
  if to_regprocedure('public.hq_content_authoring_evidence_packet(uuid)') is null
     ...
     or to_regprocedure('public.hq_accept_content_authoring_draft(uuid)') is null then raise exception 'content_autopilot_vertical_contract_missing'; end if;
  if has_function_privilege('service_role','public.hq_accept_content_authoring_draft(uuid)','EXECUTE') then raise exception 'content_worker_can_impersonate_owner_acceptance'; end if;
end $$;
...
begin
  ...
  if coalesce(ec.heartbeat_enabled,false) or coalesce(ec.factory_enabled,false)
     ...
     or coalesce(ec.shadow_scheduler_enabled,false) or not coalesce(ec.shadow_global_stop,true) then raise exception 'autopilot_contract_runtime_not_fail_closed'; end if;
