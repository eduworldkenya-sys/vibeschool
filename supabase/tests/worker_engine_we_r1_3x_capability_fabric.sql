...
-- Build certified and draft shadow skills without granting execution authority.
insert into public.hq_workforce_skill_manifests(skill_key,version,purpose,input_contract,scope_contract,resource_contract,data_classification,risk_class,autonomy_ceiling,preconditions,expected_outcome,verification_contract,failure_handling,compensation_contract,retry_policy,escalation_contract,certification_status,shadow_capable)
values
 ('test.quality.inspect',1,'test','{}','{}','{}',array['internal'],0,0,'{}','{}','{}','{}','{}','{}','{}','certified',true),
 ('test.quality.draft',1,'test','{}','{}','{}',array['internal'],0,0,'{}','{}','{}','{}','{}','{}','{}','draft',true);
...
s as (insert into public.hq_workforce_plan_steps(plan_id,step_key,ordinal,skill_manifest_id,worker_key,required_competencies) select p.id,'inspect',1,m.id,'test-worker-a',array['quality.analysis'] from p cross join public.hq_workforce_skill_manifests m where m.skill_key='test.quality.inspect' returning id,plan_id)
select 1;
...
-- Uncertified skill must invalidate a plan.
...
-- Skill genesis proposes only; it can never self-certify.
...
-- Memory explicitly exposes stale/contradictory state instead of silently treating it as truth.
...
-- Empirical calibration derives reliability from outcomes.
...
-- Collaboration cannot be same-worker and carries an authority snapshot rather than transferring authority.
