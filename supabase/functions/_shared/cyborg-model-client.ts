...
export function groqText(output:unknown):string{if(!output||typeof output!=='object'||Array.isArray(output))return'';const choices=Array.isArray((output as Record<string,unknown>).choices)?(output as Record<string,unknown>).choices as unknown[]:[];const first=choices[0]&&typeof choices[0]==='object'&&!Array.isArray(choices[0])?choices[0] as Record<string,unknown>:null;const message=first?.message&&typeof first.message==='object'&&!Array.isArray(first.message)?first.message as Record<string,unknown>:null;return typeof message?.content==='string'?message.content.trim():''}
export function anthropicText(output:unknown):string{if(!output||typeof output!=='object'||Array.isArray(output))return'';const content=Array.isArray((output as Record<string,unknown>).content)?(output as Record<string,unknown>).content as unknown[]:[];return content.flatMap(item=>item&&typeof item==='object'&&!Array.isArray(item)&&typeof(item as Record<string,unknown>).text==='string'?[(item as Record<string,unknown>).text as string]:[]).join('').trim()}
