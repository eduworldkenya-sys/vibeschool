...
export async function invokeCyborgEdgeModel(input:EdgeCyborgInput):Promise<EdgeCyborgResult>{
  ...
  const payload=await gatewayResponse.json().catch(()=>({})) as Record<string,unknown>;if(!gatewayResponse.ok)throw new Error(`CYBORG_GATEWAY_FAILED:${String(payload.error??gatewayResponse.status)}`)
  const lineage=payload.lineage&&typeof payload.lineage==='object'&&!Array.isArray(payload.lineage)?payload.lineage as Record<string,unknown>:null;if(!lineage||lineage.lineageVerified!==true||lineage.policyDecision!=='ALLOW'||typeof lineage.receiptHash!=='string')throw new Error('CYBORG_LINEAGE_REQUIRED')
  const {receiptHash:stored,lineageVerified:_verified,...unsigned}=lineage;if(await receiptHash(unsigned)!==stored)throw new Error('CYBORG_LINEAGE_HASH_MISMATCH')
  const envelope=payload.output&&typeof payload.output==='object'&&!Array.isArray(payload.output)?payload.output as Record<string,unknown>:null;return{missionId,missionRevision,chatId,invocationId,output:envelope?.output,lineage}
}
export function groqText(output:unknown):string{if(!output||typeof output!=='object'||Array.isArray(output))return'';const record=output as Record<string,unknown>;const choices=Array.isArray(record.choices)?record.choices as Array<unknown>:[];const first=choices[0]&&typeof choices[0]==='object'&&!Array.isArray(choices[0])?choices[0] as Record<string,unknown>:null;const message=first?.message&&typeof first.message==='object'&&!Array.isArray(first.message)?first.message as Record<string,unknown>:null;return typeof message?.content==='string'?message.content.trim():''}
export function anthropicText(output:unknown):string{if(!output||typeof output!=='object'||Array.isArray(output))return'';const record=output as Record<string,unknown>;const content=Array.isArray(record.content)?record.content as Array<unknown>:[];return content.flatMap(item=>item&&typeof item==='object'&&!Array.isArray(item)&&typeof(item as Record<string,unknown>).text==='string'?[(item as Record<string,unknown>).text as string]:[]).join('').trim()}
