...
Deno.serve(async(req)=>{try{
  ...
  if(CHEMISTRY_STAGE_CALLERS.has(caller)){
...
}catch(error){const message=error instanceof Error?error.message:String(error);return json({error:message},message.includes('IDENTITY')?401:message.includes('MISMATCH')||message.includes('DENIED')||message.includes('APPROVAL')||message.includes('LEASE')||message.includes('STAGE')?403:409)}})
