...
Deno.serve(async(req)=>{try{
  ...
  const sourceRaw=body.sourceAuthority&&typeof body.sourceAuthority==='object'&&!Array.isArray(body.sourceAuthority)?body.sourceAuthority as Record<string,unknown>:null
  ...
  if(WORKER_MODEL_CALLERS.has(caller)){if(suppliedKind!=='worker_model_invocation'||!suppliedRef)throw new Error('CYBORG_WORKER_SOURCE_AUTHORITY_REQUIRED');if(suppliedToken)throw new Error('CYBORG_WORKER_MODEL_TOKEN_DENIED')}
  else if(CHEMISTRY_STAGE_CALLERS.has(caller)){if(suppliedKind!=='chemistry_stage_attempt'||!suppliedRef||!suppliedToken)throw new Error('CYBORG_WORKER_STAGE_AUTHORITY_REQUIRED')}
  ...
  const sourceAuthorityKind=WORKER_MODEL_CALLERS.has(caller)?'worker_model_invocation':CHEMISTRY_STAGE_CALLERS.has(caller)?'chemistry_stage_attempt':'service'
  const sourceAuthorityRef=sourceAuthorityKind==='service'?invocationId:suppliedRef
  const sourceAuthorityToken=sourceAuthorityKind==='chemistry_stage_attempt'?suppliedToken:null
...
}catch(error){const message=error instanceof Error?error.message:String(error);return json({error:message},message.includes('IDENTITY')?401:message.includes('MISMATCH')||message.includes('DENIED')||message.includes('APPROVAL')||message.includes('REQUIRED')?403:409)}})
