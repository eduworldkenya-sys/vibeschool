...
export type QuickCheckOutcome =
  | { status: 'confident'; pathway: PathwayKey; runnerUp: PathwayKey; scores: QuickCheckScores; margin: number; evidence: number }
  | { status: 'uncertain'; reason: 'no_evidence' | 'tie' | 'low_signal' | 'narrow_margin'; leaders: PathwayKey[]; scores: QuickCheckScores; margin: number; evidence: number }

type QuickCheckChoice = { label: string; hint?: string; scores: Partial<QuickCheckScores> }
type QuickCheckQuestion = { id: string; prompt: string; choices: QuickCheckChoice[] }

export const QUICK_CHECK_RULE_VERSION = 'pathways-quick-v2'
export const QUICK_CHECK_STORAGE_KEY = 'vs_pathways_quick_check_v2'
export const QUICK_CHECK_MIN_SIGNAL = 4
export const QUICK_CHECK_MIN_MARGIN = 2
...
export function evaluateQuickCheck(scores: QuickCheckScores): QuickCheckOutcome {
  const ranking = rankQuickCheck(scores)
  const topScore = scores[ranking[0]]
  const secondScore = scores[ranking[1]]
  const evidence = scores.stem + scores.social + scores.arts
  const leaders = ranking.filter(key => scores[key] === topScore)
  const margin = topScore - secondScore

  if (evidence === 0 || topScore === 0) return { status: 'uncertain', reason: 'no_evidence', leaders: [], scores, margin: 0, evidence }
  if (leaders.length > 1) return { status: 'uncertain', reason: 'tie', leaders, scores, margin: 0, evidence }
  if (topScore < QUICK_CHECK_MIN_SIGNAL) return { status: 'uncertain', reason: 'low_signal', leaders, scores, margin, evidence }
  if (margin < QUICK_CHECK_MIN_MARGIN) return { status: 'uncertain', reason: 'narrow_margin', leaders: ranking.slice(0, 2), scores, margin, evidence }
  return { status: 'confident', pathway: ranking[0], runnerUp: ranking[1], scores, margin, evidence }
}
