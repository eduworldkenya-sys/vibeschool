...
const BLOCK_TYPES: readonly BlockType[] = ['paragraph','heading1','heading2','heading3','quote','bulletList','numberedList','image','divider','callout','code','activity','question']

function isRecord(value: unknown): value is Record<string, Json | undefined> {
  return typeof value === 'object' && value !== null && !Array.isArray(value)
}
function isStringArray(value: unknown): value is string[] { return Array.isArray(value) && value.every(item => typeof item === 'string') }
function isEnumValue<T extends string>(value: string | null, options: readonly T[]): value is T { return value !== null && options.includes(value as T) }
...
function contentBlockToJson(block: ContentBlock): Json { return { id: block.id, type: block.type, content: block.content, ...(block.meta ? { meta: blockMetaToJson(block.meta) } : {}) } }
function parseContentBlocks(value: Json): ContentBlock[] { if (!Array.isArray(value)) throw new Error('Chapter blocks must be a JSON array.'); return value.map(parseContentBlock) }
...
export function publicationRowToDraft(row: PublicationRow): VibePublication {
  return {
    ...
    format: requireEnumValue(row.format, PUBLICATION_FORMATS, 'publication format'),
    title: row.title,
    subtitle: row.subtitle,
    cover_url: row.cover_url,
    description: row.description,
    genre: requireEnumValue(row.genre ?? 'other', PUBLICATION_GENRES, 'publication genre'),
    tags: row.tags ?? [],
    language: row.language === 'sw' || row.language === 'mixed' ? row.language : 'en',
    status: requireEnumValue(row.status, PUBLICATION_STATUSES, 'publication status'),
    ...
    cbc_subject: optionalEnumValue(row.cbc_subject, CBC_SUBJECTS, 'subject'),
    cbc_grade: optionalEnumValue(row.cbc_grade, CBC_GRADES, 'grade'),
...
export function chapterRowToDraft(row: ChapterRow): VibeChapter {
  return {
    ...
    status: requireEnumValue(row.status, CHAPTER_STATUSES, 'chapter status'),
