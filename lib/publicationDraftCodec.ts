...
type PublicationRow =
  Database['public']['Tables']['vibe_publications']['Row']

type PublicationInsert =
  Database['public']['Tables']['vibe_publications']['Insert']

type ChapterRow =
  Database['public']['Tables']['vibe_chapters']['Row']
...
const PUBLICATION_GENRES: readonly PublicationGenre[] = [
  'fiction',
  'non_fiction',
  'romance',
  'thriller',
...
const CBC_SUBJECTS: readonly CBCSubject[] = [
  'mathematics',
  'english',
  'kiswahili',
  'science',
...
const CBC_GRADES: readonly CBCGrade[] = [
  'pp1',
  'pp2',
  'grade1',
  'grade2',
...
function requireEnumValue<T extends string>(
  ...
  if (!isEnumValue(value, options)) {
    throw new Error(
      `Invalid ${fieldName}: ${value ?? 'null'}`,
    )
  }
...
function parsePricing(value: Json | null): PricingModel {
  ...
  if (
    type === 'paid'
    && typeof value.priceKsh === 'number'
  ) {
    return {
      type: 'paid',
      priceKsh: value.priceKsh,
    }
  }

  if (
    type === 'freemium'
    && typeof value.freeChapters === 'number'
    && typeof value.priceKsh === 'number'
  ) {
    return {
      type: 'freemium',
      freeChapters: value.freeChapters,
      priceKsh: value.priceKsh,
    }
  }
  ...
  if (
    type === 'donation'
    && typeof value.suggestedKsh === 'number'
  ) {
    return {
      type: 'donation',
      suggestedKsh: value.suggestedKsh,
    }
  }

  if (
    type === 'school_license'
    && typeof value.perStudentKsh === 'number'
    && typeof value.schoolKsh === 'number'
  ) {
    return {
      type: 'school_license',
      perStudentKsh: value.perStudentKsh,
      schoolKsh: value.schoolKsh,
    }
  }
...
function pricingToJson(pricing: PricingModel): Json {
  switch (pricing.type) {
    ...
    case 'freemium':
      return {
        type: 'freemium',
        freeChapters: pricing.freeChapters,
        priceKsh: pricing.priceKsh,
      }
    ...
    case 'donation':
      return {
        type: 'donation',
        suggestedKsh: pricing.suggestedKsh,
      }

    case 'school_license':
      return {
        type: 'school_license',
        perStudentKsh: pricing.perStudentKsh,
        schoolKsh: pricing.schoolKsh,
      }
  }
}
...
function parseBlockMeta(value: Json | undefined): BlockMeta | undefined {
  ...
  if (!isRecord(value)) {
    throw new Error('Invalid content block metadata.')
  }
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    throw new Error(
      `Invalid metadata value for content block key "${key}".`,
    )
  }
...
function parseContentBlock(value: Json): ContentBlock {
  if (!isRecord(value)) {
    throw new Error('Invalid content block payload.')
  }
  ...
  if (
    ...
    throw new Error('Content block is missing required fields.')
  }

  if (!isEnumValue(value.type, BLOCK_TYPES)) {
    throw new Error(`Invalid content block type: ${value.type}`)
  }
...
function contentBlockToJson(block: ContentBlock): Json {
  return {
    ...
    ...(block.meta
      ? { meta: blockMetaToJson(block.meta) }
      : {}),
  }
}
...
function contentBlocksToJson(blocks: ContentBlock[]): Json {
  return blocks.map(contentBlockToJson)
}

export function publicationRowToDraft(
  row: PublicationRow,
): VibePublication {
  return {
    id: row.id,
    author_id: row.author_id,
    format: requireEnumValue(
      row.format,
      PUBLICATION_FORMATS,
      'publication format',
    ),
    ...
    tags: row.tags ?? [],
    language:
      row.language === 'sw' || row.language === 'mixed'
        ? row.language
        : 'en',
    ...
    total_vibes: row.total_vibes ?? 0,
    earnings_ksh: row.earnings_ksh ?? 0,
...
export function publicationDraftToInsert(
  publication: VibePublication,
  chapterCount: number = publication.chapter_count,
): PublicationInsert {
  return {
    id: publication.id,
    author_id: publication.author_id,
    format: publication.format,
    title: publication.title?.trim() || undefined,
    ...
    pricing: pricingToJson(publication.pricing),
    chapter_count: chapterCount,
    total_reads: publication.total_reads,
    total_vibes: publication.total_vibes,
    earnings_ksh: publication.earnings_ksh,
...
export function chapterRowToDraft(row: ChapterRow): VibeChapter {
  return {
    ...
    title: row.title,
    number: row.number,
    blocks: parseContentBlocks(row.blocks),
    status: requireEnumValue(
      row.status,
      CHAPTER_STATUSES,
      'chapter status',
...
export function chapterDraftToInsert(
  ...
  return {
    ...
    blocks: contentBlocksToJson(chapter.blocks),
