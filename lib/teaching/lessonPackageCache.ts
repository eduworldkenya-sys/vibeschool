...
export function buildLessonPackageCacheKey(
...
export async function buildLessonPackageSourceFingerprint(
  identity: LessonPackageSourceIdentity,
): Promise<string> {
...
function untypedClient(): SupabaseClient<any> {
  // Database types intentionally lag this additive migration on the branch.
  // Keep the escape hatch contained to this cache adapter.
  return supabase as unknown as SupabaseClient<any>
}

export async function loadExactLessonPackage(
  identity: LessonPackageSourceIdentity,
): Promise<{
  ...
  return {
    ...
    reuseScope: selected.reuse_scope,
  }
}
