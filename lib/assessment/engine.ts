...
function assertString(value: unknown, label: string): string {
  if (typeof value !== 'string' || value.length === 0) {
    throw new Error(`${label} was not returned by the Assessment Engine.`)
  }
  return value
}
...
function numberValue(value: unknown, label: string): number {
  ...
  if (!Number.isFinite(resolved)) {
    throw new Error(`${label} was not numeric.`)
  }
  return resolved
}

function rpcError(action: string, error: { message?: string } | null): Error {
  return new Error(error?.message || `Assessment Engine ${action} failed.`)
}

export async function createDraftAssessment(
  input: CreateDraftAssessmentInput,
): Promise<string> {
  const { data, error } = await rpc<string>('exq_create_draft_assessment', {
    ...
    p_lesson_plan_id: input.lessonPlanId ?? null,
    p_teaching_occurrence_id: input.teachingOccurrenceId ?? null,
    p_source_resource_id: input.sourceResourceId ?? null,
    p_generation_source: input.generationSource ?? 'teacher_authored',
...
export async function addDraftItem(input: AddDraftItemInput): Promise<string> {
  const { data, error } = await rpc<string>('exq_add_draft_item', {
    ...
    p_source_resource_id: input.sourceResourceId ?? null,
    p_source_exercise_ref: input.sourceExerciseRef ?? null,
    p_generated_by: input.generatedBy ?? 'teacher',
  })
...
export async function approveAssessment(
  ...
  const { data, error } = await rpc<Json>('exq_approve_assessment', {
  ...
  return {
    ok: true,
    assessmentId: assertString(payload.assessment_id, 'Approved assessment ID'),
    itemCount: numberValue(payload.item_count, 'Approved item count'),
    totalMarks: numberValue(payload.total_marks, 'Approved total marks'),
  }
}

export async function assignAssessment(
  input: AssignAssessmentInput,
): Promise<string> {
  const { data, error } = await rpc<string>('exq_assign_assessment', {
    ...
    p_max_attempts: input.maxAttempts ?? 1,
    p_randomize_items: input.randomizeItems ?? false,
    p_randomize_options: input.randomizeOptions ?? false,
...
function mapAttemptWorkspace(value: unknown): AttemptWorkspace {
  ...
  return {
    ...
    timeLimitMinutes:
      payload.time_limit_minutes === null || payload.time_limit_minutes === undefined
        ? null
        : numberValue(payload.time_limit_minutes, 'Time limit'),
    closesAt: typeof payload.closes_at === 'string' ? payload.closes_at : null,
    items: items.map((item) => {
      const record = assertRecord(item, 'Learner assessment item')
      return {
        id: assertString(record.id, 'Assessment item ID'),
        questionType: assertString(
          record.question_type,
          'Question type',
        ) as AttemptWorkspace['items'][number]['questionType'],
        prompt: assertString(record.prompt, 'Question prompt'),
        options: (record.options ?? []) as Json,
        marks: numberValue(record.marks, 'Question marks'),
        orderNum: numberValue(record.order_num, 'Question order'),
        media: (record.media ?? []) as Json,
    ...
    responses: responses.map((response) => {
      ...
      return {
        assessmentItemId: assertString(
          record.assessment_item_id,
          'Response item ID',
        ),
        responseValue: (record.response_value ?? null) as Json,
        responseText:
          typeof record.response_text === 'string' ? record.response_text : null,
...
export async function startOrResumeAttempt(
  ...
  const { data, error } = await rpc<Json>('exq_start_or_resume_attempt', {
...
export async function saveResponse(
  ...
  return {
    ok: true,
    attemptId: assertString(payload.attempt_id, 'Saved attempt ID'),
    assessmentItemId: assertString(
      payload.assessment_item_id,
      'Saved assessment item ID',
    ),
    savedAt: assertString(payload.saved_at, 'Response save time'),
  }
}
...
export async function submitAttempt(
  ...
  const { data, error } = await rpc<Json>('exq_submit_attempt', {
  ...
  if (status !== 'auto_marked' && status !== 'teacher_review') {
    throw new Error('Assessment Engine returned an invalid attempt status.')
  }
  if (resultStatus !== 'marked' && resultStatus !== 'partially_marked') {
    throw new Error('Assessment Engine returned an invalid result status.')
  }
  ...
  return {
    ...
    status,
    resultStatus,
    score: numberValue(payload.score, 'Attempt score'),
    maxScore: numberValue(payload.max_score, 'Attempt maximum score'),
    percentage: numberValue(payload.percentage, 'Attempt percentage'),
