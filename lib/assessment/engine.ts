...
const rpc = supabase.rpc.bind(supabase) as AssessmentRpc
...
function rpcError(
  ...
  return new Error(
    error?.message || `Assessment Engine ${action} failed.`,
  )
}
...
export async function createDraftAssessment(
  ...
  const { data, error } = await rpc<string>(
    'exq_create_draft_assessment',
    {
      ...
      p_teaching_occurrence_id:
        input.teachingOccurrenceId ?? null,
      p_source_resource_id:
        input.sourceResourceId ?? null,
      p_generation_source:
        input.generationSource ?? 'teacher_authored',
  ...
  if (error) {
    throw rpcError('draft creation', error)
  }
...
export async function addDraftItem(
  ...
  const { data, error } = await rpc<string>(
    'exq_add_draft_item',
    {
      ...
      p_source_resource_id:
        input.sourceResourceId ?? null,
      p_source_exercise_ref:
        input.sourceExerciseRef ?? null,
      p_generated_by: input.generatedBy ?? 'teacher',
    },
  )

  if (error) {
    throw rpcError('question creation', error)
  }
...
export async function approveAssessment(
  ...
  const { data, error } = await rpc<Json>(
    'exq_approve_assessment',
    { p_assessment_id: assessmentId },
  )

  if (error) {
    throw rpcError('approval', error)
  }
  ...
  return {
    ok: true,
    assessmentId: assertString(
      payload.assessment_id,
      'Approved assessment ID',
    ),
    itemCount: numberValue(
      payload.item_count,
      'Approved item count',
    ),
    totalMarks: numberValue(
      payload.total_marks,
      'Approved total marks',
    ),
  }
}
...
export async function assignAssessment(
  ...
  const { data, error } = await rpc<string>(
    'exq_assign_assessment',
    {
      ...
      p_randomize_items:
        input.randomizeItems ?? false,
      p_randomize_options:
        input.randomizeOptions ?? false,
      p_show_score_policy:
        input.showScorePolicy ?? 'after_review',
    },
  )

  if (error) {
    throw rpcError('assignment', error)
  }

  return assertString(data, 'Assessment assignment ID')
}

function mapAttemptWorkspace(
  value: unknown,
): AttemptWorkspace {
  ...
  return {
    ...
    closesAt:
      typeof payload.closes_at === 'string'
        ? payload.closes_at
        : null,
    items: items.map((item) => {
      const record = assertRecord(
        item,
        'Learner assessment item',
      )
    ...
    responses: responses.map((response) => {
      ...
      return {
        ...
        responseValue:
          (record.response_value ?? null) as Json,
        responseText:
          typeof record.response_text === 'string'
...
export async function startOrResumeAttempt(
  ...
  if (error) {
    throw rpcError('attempt start', error)
  }
...
export async function saveResponse(
  ...
  if (error) {
    throw rpcError('response save', error)
  }
...
export async function submitAttempt(
  ...
  if (error) {
    throw rpcError('attempt submission', error)
  }
  ...
  const resultStatus = assertString(
  ...
  return {
    ...
    manualItems: numberValue(
      payload.manual_items,
      'Manual-marking item count',
    ),
  }
}
