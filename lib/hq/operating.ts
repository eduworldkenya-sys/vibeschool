...
export type HQWorkStatus = "open"|"assigned"|"in_progress"|"blocked"|"waiting_evidence"|"waiting_approval"|"verifying"|"failed"|"delivered"|"resolved"|"cancelled"
export type HQWorkItem = { id:string; department_key:string; work_type:string; priority:"low"|"normal"|"high"|"critical"; status:HQWorkStatus; title:string; summary:string|null; source_type:string|null; source_id:string|null; route:string|null; approval_required:boolean; due_at:string|null; evidence:Record<string,unknown>; created_at:string; updated_at:string; resolved_at:string|null; owner_id:string|null; assignee_id:string|null; acted_at:string|null; affected_persona:string|null; tenant_id:string|null; desired_outcome:string|null; next_action:string|null; blocker:string|null; risk_class:number; authority_required:string; evidence_status:string; delivery_status:string; version:number; verification_status:string|null; verification_evidence:Record<string,unknown>|null; action_taken:Record<string,unknown>|null }
...
export type HQWorkroomActor = {profile_id:string;display_name:string|null;role:string;title:string|null;department:string|null;permissions:string[];can_coordinate:boolean;can_verify:boolean;can_authorize:boolean;can_cancel:boolean;is_founder:boolean}
export type HQWorkroomItem = { actor:HQWorkroomActor; item:HQWorkItem; updates:HQWorkItemUpdate[]; links:HQWorkItemLink[]; runs:HQWorkforceRunSummary[]; handoffs:HQWorkforceHandoffSummary[] }
export type HQWorkroomAction = "start"|"block"|"resume"|"submit_evidence"|"submit_for_approval"|"authorize"|"request_correction"|"accept_verified"|"mark_delivered"|"confirm_delivery"|"cancel"
export type HQWorkroomView = "attention"|"my_work"|"founder_decisions"|"journeys"|"active"|"closed"
export type HQWorkroomSnapshot = {actor:HQWorkroomActor;counts:{active:number;mine:number;blocked:number;waiting_approval:number;overdue:number;unassigned:number;journeys_at_risk:number};items:HQWorkItem[];generated_at:string}
...
export async function loadHQWorkroom(view:HQWorkroomView="attention",department:string|null=null):Promise<HQWorkroomSnapshot>{const{data,error}=await sb.rpc("hq_workroom_list",{p_view:view,p_department:department,p_limit:300});if(error)throw error;return data as HQWorkroomSnapshot}
...
export async function actOnHQWorkroomItem(id:string,action:HQWorkroomAction,reason:string,version:number):Promise<HQWorkroomItem>{const{data,error}=await sb.rpc("hq_workroom_act",{p_work_item_id:id,p_action:action,p_reason:reason,p_expected_version:version});if(error)throw error;return data as HQWorkroomItem}
