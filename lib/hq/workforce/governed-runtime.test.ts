...
import type { ClarificationPayload, DigitalWorkerDefinition, WorkEnvelope, WorkerExecutionResult } from "./types"
...
const worker: DigitalWorkerDefinition = {
  ...
  fallbackPolicy: { requireApprovalOnFallback: true, allowedFallbackModes: ["local_ai"], maxFallbackDepth: 1 },
  contextPolicy: { maximumClassification: "internal", forbiddenContextKeys: ["secret"] },
  authority: [
...
const envelope = (payload: Record<string, unknown> = {}): WorkEnvelope<Record<string, unknown>> => ({ id: "env-proof", type: "request", fromWorkerKey: "hq", toWorkerKey: worker.key, priority: "normal", createdAt: now, payload })
const persistence = (overrides: Partial<GovernedRuntimePersistence> = {}): GovernedRuntimePersistence => ({ claimTrigger: async () => true, requestApproval: async () => "approval-proof", persistClarification: async () => "clarification-proof", ...overrides })
const healthy: WatchdogSnapshot = { lastTelemetryAt: now, lastWorkerHeartbeatAt: now, maximumTelemetryAgeSeconds: 60, maximumHeartbeatAgeSeconds: 60 }
const execute = async (): Promise<WorkerExecutionResult> => ({ status: "completed", workerKey: worker.key, workflowKey: "proof", evidence: {} })

test("inactive lifecycle fails closed before execution", async () => {
  let called = false
  const result = await executeGovernedWorker({ worker: { ...worker, status: "probation" }, workflowKey: "proof", action: "read", envelope: envelope(), mode: "deterministic", watchdogSnapshot: healthy, persistence: persistence(), execute: async () => { called = true; return execute() } })
  assert.equal(result.status, "blocked")
  assert.equal(called, false)
})

test("unconfigured execution mode fails closed", async () => {
  let called = false
  const result = await executeGovernedWorker({ worker, workflowKey: "proof", action: "read", envelope: envelope(), mode: "cloud_ai", watchdogSnapshot: healthy, persistence: persistence(), execute: async () => { called = true; return execute() } })
  assert.equal(result.status, "blocked")
  assert.equal(called, false)
})
...
test("watchdog fail-closes before execution", async () => {
  let called = false
  const unhealthy: WatchdogSnapshot = { lastTelemetryAt: null, lastWorkerHeartbeatAt: null, maximumTelemetryAgeSeconds: 60, maximumHeartbeatAgeSeconds: 60 }
  const result = await executeGovernedWorker({ worker, workflowKey: "proof", action: "read", envelope: envelope(), mode: "deterministic", watchdogSnapshot: unhealthy, persistence: persistence(), execute: async () => { called = true; return execute() } })
  assert.equal(result.status, "blocked")
  assert.equal(called, false)
})

test("invalid envelope recipient fails closed", async () => {
  let called = false
  const bad = { ...envelope(), toWorkerKey: "wrong-worker" }
  const result = await executeGovernedWorker({ worker, workflowKey: "proof", action: "read", envelope: bad, mode: "deterministic", watchdogSnapshot: healthy, persistence: persistence(), execute: async () => { called = true; return execute() } })
  assert.equal(result.status, "blocked")
  assert.equal(called, false)
})
...
test("authority approval persists and stops", async () => {
  let approvals = 0; let called = false
  ...
  assert.equal(result.status, "approval_required"); assert.equal(approvals, 1); assert.equal(called, false)
})
...
test("fallback requires approval", async () => {
  ...
  assert.equal(result.status, "approval_required"); assert.equal(called, false)
})
...
test("context is sanitized before executor", async () => {
  ...
  assert.equal(seen?.secret, undefined); assert.equal(seen?.safe, "yes")
})
...
test("metric trigger delegates durable admission", async () => {
  ...
  assert.equal(accepted, true); assert.equal(claims, 1)
})
