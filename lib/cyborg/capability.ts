...
function validateClaimsShape(value: unknown): CyborgCapabilityClaims {
  ...
  const requiredStrings = ['missionId','missionRevision','chatId','invocationId','callerServiceId','provider','model','operation','riskClass','dataClassification','policyVersion','nonce'] as const;
...
export async function verifyCyborgCapability(token: string, secret: string, expected: CyborgCapabilityExpectation): Promise<CyborgCapabilityClaims> {
  ...
  let decoded: unknown;
  try { decoded = JSON.parse(new TextDecoder().decode(fromBase64Url(payload))); }
  catch { throw new Error(CYBORG_CAPABILITY_INVALID); }
