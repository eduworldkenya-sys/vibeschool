...
export interface CyborgBoundaryInvocationResult {
  missionId: string;
...
function boundaryUrls() {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL?.replace(/\/$/, '');
  const admission = process.env.CYBORG_ADMISSION_URL || (supabaseUrl ? `${supabaseUrl}/functions/v1/cyborg-admission` : '');
  const gateway = process.env.CYBORG_LLM_GATEWAY_URL || (supabaseUrl ? `${supabaseUrl}/functions/v1/cyborg-llm-gateway` : '');
  if (!admission || !gateway) throw new Error('CYBORG_BOUNDARY_URL_REQUIRED');
  return { admission, gateway };
}
...
export async function invokeCyborgBoundary(input: CyborgBoundaryInvocationInput): Promise<CyborgBoundaryInvocationResult> {
  ...
  if (!serviceRole) throw new Error('CYBORG_ADMISSION_SERVICE_IDENTITY_REQUIRED');
  const urls = boundaryUrls();
  const operation = input.operation ?? 'model.generate';
  const admissionResponse = await fetch(urls.admission, {
    method: 'POST',
    headers: {
  ...
  const gatewayResponse = await fetch(urls.gateway, {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
  ...
  const raw = await gatewayResponse.json().catch(() => ({})) as Record<string, unknown>;
  if (!gatewayResponse.ok) throw new Error(`CYBORG_GATEWAY_FAILED:${String(raw.error ?? gatewayResponse.status)}`);
  ...
  return {
    ...
    chatId,
    invocationId,
    provider: admitted.output.provider,
    model: admitted.output.model,
    output: admitted.output.output,
    usage: admitted.output.usage,
