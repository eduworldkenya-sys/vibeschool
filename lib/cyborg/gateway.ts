...
export async function invokeCyborgModel(adapter: CyborgModelAdapter, request: CyborgModelRequest, secret = process.env.CYBORG_CAPABILITY_SECRET): Promise<CyborgModelResponse> {
  ...
  const claims = verifyCyborgCapability({ secret, token: request.capabilityToken, missionId: request.missionId, provider: request.provider, model: request.model });
...
export class CyborgUniversalGateway {
  ...
  async invokeModel(request: CyborgModelRequest): Promise<CyborgModelResponse> {
    const adapter = this.adapters.get(request.provider); if (!adapter) throw new Error(`CYBORG_PROVIDER_NOT_REGISTERED:${request.provider}`);
    const capabilityToken = issueCyborgCapability({ secret: this.capabilitySecret, missionId: request.missionId, conversationId: request.conversationId, provider: request.provider, model: request.model });
    return invokeCyborgModel(adapter, { ...request, capabilityToken }, this.capabilitySecret);
  }
...
export async function runCyborgMission(gateway: CyborgUniversalGateway, input: { objective: string; conversationId?: string; requestedProvider?: LlmProvider }): Promise<CyborgMission> {
  const mission = await gateway.start(input); gateway.assertReturnAllowed(mission); return mission;
}
