...
export default function TeacherSignupPage() {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          // Display data only. Role authority is claimed server-side after authentication.
      ...
      if (!data.session) {
        setMessage('Check your email to confirm your account. After confirmation, VibeSchool will continue teacher setup.')
        return
      }

      const claim = await supabase.rpc('claim_my_initial_role', { p_role: 'teacher' })
      if (claim.error || claim.data !== 'teacher') {
      ...
      const { error: profileError } = await supabase
        .from('profiles')
        .update({ full_name: name.trim(), country_code: 'KE' })
        .eq('id', data.user.id)
      if (profileError) {
  ...
  return <main className="shell"><section className="card">
    ...
    <h1>Start teaching with VibeSchool.</h1>
    <p className="lead">Create your account now. We will ask for your school next.</p>
    ...
    <button className="primary" disabled={busy} onClick={()=>void submit()}>{busy?'Creating account…':'Create teacher account'}</button>
    ...
    <p className="legal">By continuing you agree to our <a href="/legal/terms">Terms</a> and <a href="/legal/privacy">Privacy Policy</a>.</p>
  </section><style jsx>{`
