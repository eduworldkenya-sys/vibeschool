...
export default function ParentSignupPage() {
  ...
  const signin=continuationForRole(requestedNext,'parent')?`/login/parent?next=${encodeURIComponent(requestedNext!)}`:'/login/parent'
  return <main className="shell"><section className="card">
    ...
    <button className="primary" disabled={busy} onClick={()=>void submit()}>{busy?'Creating account…':'Create parent account'}</button><div className="or"><span/>or<span/></div><button className="secondary" disabled={busy} onClick={()=>void google()}>Continue with Google</button>
