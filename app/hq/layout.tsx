...
export default function HQLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (pathname === "/hq/login") { setChecking(false); return }
    async function check() {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) { router.replace("/hq/login"); return }
      const { data: profile } = await supabase.from("profiles").select("role").eq("id", session.user.id).single()
      if (profile?.role !== "admin") { await supabase.auth.signOut(); router.replace("/hq/login"); return }
      setChecking(false)
    }
    check()
  }, [router, pathname])

  if (checking && pathname !== "/hq/login") {
    return (
