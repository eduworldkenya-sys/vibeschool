...
export default function HQStudioEditorPage(){
 ...
 useEffect(()=>{void supabase.auth.getUser().then(({data})=>{if(!data.user){router.replace("/hq/login");return}setUserId(data.user.id)})},[router])
 if(!userId)return <main style={{minHeight:"100dvh",background:"#090D16",color:"rgba(255,255,255,.5)",display:"grid",placeItems:"center"}}>Loading owner editor…</main>
