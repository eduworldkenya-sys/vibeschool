...
type Format="Vertical short"|"Revision clip"|"Lesson visual"|"Campaign creative"
type Brief={id:string;title:string;hook:string;message:string;cta:string;format:Format;duration:string}
type Persisted={artifactId:string;versionId:string}

const formats:Format[]=["Vertical short","Revision clip","Lesson visual","Campaign creative"]
...
const presetMap:Record<string,{format:Format;duration:string}>={"vertical-short":{format:"Vertical short",duration:"45 sec"},"revision-clip":{format:"Revision clip",duration:"60 sec"},"lesson-visual":{format:"Lesson visual",duration:"3 min"},"campaign-creative":{format:"Campaign creative",duration:"Static"}}
...
function makeBriefs(insight:string):Brief[]{
 ...
 return [
  ...
  {id:"school",title:"School-gate conversion",hook:"Before your next lesson, test yourself on this.",message:`Create a practical school-facing asset inspired by: ${signal}`,cta:"Scan to continue on VibeSchool",format:"Campaign creative",duration:"Static"},
 ]
}

export default function MediaFactoryEditorPage(){
 const search=useSearchParams();const preset=presetMap[search.get("format")??""]??presetMap["vertical-short"]
 ...
 function loadBrief(b:Brief){setTitle(b.title);setHook(b.hook);setMessage(b.message);setCta(b.cta);setFormat(b.format);setDuration(b.duration);touch();setTab("brief")}
 function updateFormat(v:Format){setFormat(v);if(v==="Vertical short")setDuration("45 sec");else if(v==="Revision clip")setDuration("60 sec");else if(v==="Lesson visual")setDuration("3 min");else setDuration("Static");touch()}
 ...
 return <HQPage title="Media Factory Editor" description="Build media briefs and production-ready assets from Growth intelligence without bypassing governance." actions={<><Link href="/hq/studio/media-factory" style={{...hqButtonStyle,textDecoration:"none"}}>← Media Factory</Link><button style={hqButtonStyle} disabled={saving||submitted} onClick={()=>void saveDraft()}>{saving?"Saving…":dirty||!artifactId?"Save governed draft":savedAt?`Saved ${savedAt.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}`:"Saved"}</button></>}>
  ...
  <div className="notice"><b>Canonical persistence active:</b> media drafts use the existing HQ Company Library with owner authorization, RLS, immutable numbered versions and exact-version approval. If a producer saves a newer version after submission, the previous approval becomes stale and cannot be approved. <code>/hq/studio/editor</code> remains the publication/textbook editor. Cyborg model execution remains behind canonical mission intake; no direct provider call is introduced.</div>
