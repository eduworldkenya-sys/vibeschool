...
export default function PathwaysSchoolsPage() {
  ...
  async function search() {
    ...
    const {data,error:rpcError} = await supabase.rpc('pathways_search_public_schools',{
      p_query:query.trim()||null,p_county:county.trim()||null,p_pathway_slug:pathway||null,p_combination_slug:null,p_limit:40,
    })
  ...
  return <main style={S.root}><div style={S.shell}>
    ...
    <form onSubmit={e=>{e.preventDefault();void search()}} style={S.filters}>
    ...
    <div style={S.list}>{rows.map(row=><article key={`${row.school_id}:${row.pathway_slug??'school'}:${row.combination_slug??''}`} style={S.card}>
      ...
      <div style={S.chips}>{[row.school_category,row.gender_type,row.accommodation_type,row.cluster].filter(Boolean).map(value=><span key={value} style={S.chip}>{value}</span>)}</div>
