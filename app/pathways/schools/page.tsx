...
export default function PathwaysSchoolsPage(){
  ...
  async function searchSchools(searchQuery=query){
    setLoading(true);setError('');setSuggestOpen(false);++suggestRequest.current
  ...
  return <main style={S.root}><div style={S.shell}>
    ...
    <div style={S.notice}><strong>Evidence rule:</strong> a missing pathway claim means <em>not yet verified here</em>, not “No”. VibeSchool only uses “Unavailable” when authoritative evidence explicitly supports that conclusion.</div>
    ...
    {!loading&&!error&&rows.length===0&&<div style={S.state}><strong>No matching verified evidence found.</strong><span style={S.body}>Try removing a filter or searching a broader area. Missing evidence is kept unknown rather than guessed.</span></div>}
