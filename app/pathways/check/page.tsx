...
type VisualScores = Partial<Record<'stem'|'social'|'arts',number>>
...
function choiceIconName(scores:VisualScores) {
  const ranked = (['stem','social','arts'] as const).map(key=>({key,value:scores[key]??0})).sort((a,b)=>b.value-a.value)
  return ranked[0].value>ranked[1].value && ranked[0].value>0 ? ranked[0].key : 'compass'
}
...
export default function PathwayQuickCheckPage() {
  ...
  const signalRows = ranking.map((key,index) => ({
    key,
    name: QUICK_CHECK_PATHWAYS[key].name,
    score: scores[key],
    width: scores[key]===0 ? 0 : Math.max(6, Math.round((scores[key] / maxScore) * 100)),
    label: scores[key]===0 ? 'No signal' : index===0 ? (outcome.status==='confident'?'Strongest':'Strong') : scores[key] >= maxScore*.6 ? 'Strong' : 'Some',
  }))
  ...
  if (!started && !complete) return <main className="page"><div className="wrap intro">
    ...
    <h1>Six short prompts. One useful starting point.</h1>
    <p className="lead">This is guidance, not a test. The first five prompts compare interests across the three Senior School pathway families. The final prompt asks what you want to explore next and does not change your direction score.</p>
    <section className="how"><div><span>1</span><strong>Answer honestly</strong><small>No right or wrong answers.</small></div><div><span>2</span><strong>We compare signals</strong><small>Only direction choices add pathway evidence.</small></div><div><span>3</span><strong>We show uncertainty</strong><small>Ties and weak signals stay uncertain.</small></div><div><span>4</span><strong>You decide what to explore</strong><small>The result is guidance, not placement.</small></div></section>
    <button className="start" onClick={()=>setStarted(true)}>Start — about 2 minutes</button>
    <p className="privacy">No login required · Answers stay on this device unless you choose to save later.</p>
  </div><style jsx>{styles}</style></main>

  if (complete) {
    if (outcome.status === 'uncertain') {
      const closeSignals = outcome.reason==='tie'||outcome.reason==='narrow_margin'
      return <main className="page"><div className="wrap">
        ...
        <h1>{closeSignals?'You may fit more than one direction.':'No single pathway is clear yet.'}</h1>
        <p className="lead">{closeSignals?'Your strongest signals are close. VibeSchool will not break the tie just to produce one answer.':'Your answers do not provide enough direction evidence to responsibly name one pathway first. That is a valid result.'}</p>
        <section className="signalCard"><strong>Your current signals</strong>{signalRows.map(r=><div key={r.key} className="signalRow"><div className="signalLabel"><span><Icon name={r.key}/>{r.name}</span><b>{r.label}</b></div><div className="signalTrack"><span style={{width:`${r.width}%`}} /></div></div>)}<small>These bars compare internal evidence points. They are not percentages or probabilities.</small></section>
    ...
    const selectedLabels = QUICK_CHECK_QUESTIONS.flatMap(q => {
      ...
      if (!choice) return []
      const primaryPoints=choice.scores[outcome.pathway]??0
      const strongestOther=Math.max(...(['stem','social','arts'] as const).filter(key=>key!==outcome.pathway).map(key=>choice.scores[key]??0))
      if(primaryPoints<=strongestOther||primaryPoints===0)return []
      return [choice.label]
    }).slice(0,4)
    ...
    return <main className="page"><div className="wrap">
      ...
      <h1>Your answers currently point toward {primary.name}.</h1>
      <p className="lead">This result comes from the direction evidence in the answers you selected. It is VibeSchool guidance, not an official placement decision or a permanent label.</p>
      ...
      <section className="signalCard"><strong>How your signals compared</strong>{signalRows.map(r=><div key={r.key} className="signalRow"><div className="signalLabel"><span><Icon name={r.key}/>{r.name}</span><b>{r.label}</b></div><div className="signalTrack"><span style={{width:`${r.width}%`}} /></div></div>)}<small>Internal evidence comparison only—not probabilities. A direction appears first only after minimum signal and lead rules are met.</small></section>
      <section className="card"><strong>Why this direction appeared</strong>{selectedLabels.length ? <><p>These answers favored {primary.name} more than the other pathways:</p><ul>{selectedLabels.map(label=><li key={label}>{label}</li>)}</ul></> : <p>Your combined direction evidence and lead over the next pathway passed VibeSchool's threshold.</p>}<p>Neutral answers that added the same signal to several pathways are not shown here as reasons for the winner.</p></section>
      <section className="feedback"><strong>Does this feel like you?</strong><div><button onClick={()=>setFeedback('yes')} aria-pressed={feedback==='yes'}>Yes</button><button onClick={()=>setFeedback('somewhat')} aria-pressed={feedback==='somewhat'}>Somewhat</button><button onClick={()=>setFeedback('no')} aria-pressed={feedback==='no'}>Not really</button></div>{feedback==='no'&&<p>That is useful feedback. A short check cannot know everything about you. Compare another pathway, retake the check or explore careers before deciding.</p>}{feedback==='somewhat'&&<p>Use this as one direction to explore, not a conclusion.</p>}{feedback==='yes'&&<p>Use the result to explore subjects, careers and verified school information next.</p>}</section>
      <div className="actions"><Link className="primary" href="/pathways/continue">Save or continue</Link><Link href="/learn/careers">Explore careers</Link><Link href="/pathways/schools">Explore schools</Link><button onClick={reset}>Retake the check</button></div>
  ...
  const question = QUICK_CHECK_QUESTIONS[step]
  const finalPreferencePrompt=question.id==='choice'
  return <main className="page"><div className="wrap">
    <header className="top"><Link href="/pathways">← Pathways</Link><span>Quick Check</span></header>
    ...
    <div className="questionMeta"><p className="eyebrow">{finalPreferencePrompt?'NEXT STEP':'DIRECTION'} · {step + 1} OF {QUICK_CHECK_QUESTIONS.length}</p><span>{progress}% complete</span></div>
    <h1>{question.prompt}</h1>
    <p className="instruction">{finalPreferencePrompt?'This last choice helps VibeSchool understand what you want to do next. It does not change your pathway score.':'Choose the answer that feels closest to you. There is no right or wrong answer.'}</p>
    <div className="choices">{question.choices.map((choice, index) => <button key={choice.label} onClick={() => choose(index)}><span className="choiceMain"><span className="choiceIcon"><Icon name={choiceIconName(choice.scores)} /></span><span>{choice.label}</span></span><span aria-hidden="true">→</span></button>)}</div>
