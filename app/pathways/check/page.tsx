...
export default function PathwayQuickCheckPage() {
  ...
  useEffect(() => {
    try {
      const stored = window.localStorage.getItem(QUICK_CHECK_STORAGE_KEY)
      if (stored) {
        ...
        if (typeof parsed.step === 'number') setStep(Math.max(0, Math.min(QUICK_CHECK_QUESTIONS.length - 1, parsed.step)))
  ...
  useEffect(() => {
    if (!hydrated) return
    try {
      window.localStorage.setItem(QUICK_CHECK_STORAGE_KEY, JSON.stringify({ step, answers, complete }))
    } catch {
  ...
  function reset() {
    ...
    setStep(0)
    setComplete(false)
    try { window.localStorage.removeItem(QUICK_CHECK_STORAGE_KEY) } catch {}
  }
  ...
  if (complete) {
    ...
    return <main style={S.root}><div style={S.shell}>
      ...
      <section style={S.resultCard}>
        ...
        <p style={{ ...S.body, color: '#d8d8e6' }}>{primary.summary}</p>
      ...
      <section style={S.card}>
        ...
        <p style={S.body}>Your answers produced more interest signals for {primary.name}{close ? `, with ${secondary.name} close behind` : ''}. Adding real subject preferences, performance evidence and career goals later can strengthen or change the guidance.</p>
      </section>

      <section style={S.card}>
        <h2 style={S.cardTitle}>What should I do next?</h2>
        <div style={S.actions}>
          <Link href="/pathways/continue" style={S.primaryAction}>Save my pathway and continue</Link>
          <Link href="/learn/careers" style={S.secondaryAction}>Explore careers first</Link>
          <Link href={primary.href} style={S.secondaryAction}>Understand {primary.name}</Link>
          <button type="button" onClick={reset} style={S.textButton}>Retake the quick check</button>
        </div>
        <p style={{ ...S.small, marginTop: 12 }}>Saving is free. Your answers stay on this device unless you explicitly choose to save them to your learner account.</p>
      </section>

      <section style={S.trustCard}>
        <strong>Trust note</strong>
        <p style={S.small}>Kenya's official Grade 10 selection system identifies STEM, Social Sciences and Arts & Sports as the main pathway families. VibeSchool's result above is guidance based only on your answers. Detailed official combinations and school offerings are published only after source verification.</p>
      </section>
    </div></main>
  }
  ...
  return <main style={S.root}><div style={S.shell}>
    ...
    <div style={S.stepText}>Question {step + 1} of {QUICK_CHECK_QUESTIONS.length}</div>
    ...
    <div style={S.choiceGrid}>
      ...
      ><span style={S.choiceTitle}>{choice.label}</span>{choice.hint && <span style={S.small}>{choice.hint}</span>}</button>)}
    </div>
    <div style={S.footerRow}>
      ...
      <span style={S.small}>No login required · answers stay on this device</span>
    </div>
  </div></main>
}
