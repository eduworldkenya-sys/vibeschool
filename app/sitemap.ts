...
type PublishedPathwaySitemapRow = { slug: string; updated_at: string | null }
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const { data: publications } = await supabase.from('vibe_publications').select('id, format, published_at, updated_at').eq('status', 'published').order('published_at', { ascending: false }).limit(1000)
    ...
    const { data: pathways, error: pathwayError } = await pathwaysClient.from('pathways').select('slug, updated_at').eq('status', 'published').limit(100)
    const pathwayRoutes: MetadataRoute.Sitemap = pathwayError ? [] : ((pathways ?? []) as PublishedPathwaySitemapRow[]).map((pathway) => ({
