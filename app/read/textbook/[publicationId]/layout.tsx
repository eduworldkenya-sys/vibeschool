...
import { ReaderHumanFirstPolish } from "@/components/read/ReaderHumanFirstPolish";
...
async function getPublishedPublication(publicationId: string): Promise<PublicationMetadata | null> {
  try {
    ...
    const { data } = await supabase.from("vibe_publications").select("id,title,description,cover_url,cbc_subject,cbc_grade,status,published_at,updated_at").eq("id", publicationId).maybeSingle();
