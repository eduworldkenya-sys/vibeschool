...
async function getPublishedPublication(publicationId:string):Promise<PublicationMetadata|null>{
  try { const supabase=getSupabaseServerClient(); const {data}=await supabase.from("vibe_publications").select("id,title,description,cover_url,cbc_subject,cbc_grade,status,published_at,updated_at").eq("id",publicationId).maybeSingle(); const publication=data as PublicationMetadata|null; return publication?.status==="published"?publication:null; } catch { return null; }
}
...
export async function generateMetadata({params}:{params:{publicationId:string}}):Promise<Metadata>{
  ...
  return{title,description,alternates:{canonical},robots:{index:true,follow:true},openGraph:{type:"book",url:canonical,title,description,siteName:"VibeSchool",locale:"en_KE",...(publication.cover_url?{images:[{url:publication.cover_url,alt:title}]}:{})}};
}
...
export default async function TextbookReaderLayout({children,params}:{children:ReactNode;params:{publicationId:string}}){
  ...
  const bookSchema=publication?{"@context":"https://schema.org","@type":"Book","@id":`${canonical}#book`,name:title,description,url:canonical,inLanguage:"en-KE",publisher:{"@type":"EducationalOrganization",name:"VibeSchool",url:SITE_URL},...(publication.cover_url?{image:publication.cover_url}:{}),...(publication.published_at?{datePublished:publication.published_at}:{}),...(publication.updated_at?{dateModified:publication.updated_at}:{}),...(publication.cbc_grade?{educationalLevel:publication.cbc_grade}:{}),...(publication.cbc_subject?{about:publication.cbc_subject}:{}),mainEntityOfPage:{"@type":"WebPage","@id":canonical}}:null;
  return <div id="vibetextbook-reader-shell" data-reader-mode="read">
    {bookSchema?<script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(bookSchema)}}/>:null}
    <ReaderAccessibilityStyles/><ReadingAnalyticsTracker/><ReaderExcellenceShell/><ReaderCalmSurface/><ReaderListenContinuity publicationId={params.publicationId}/>
    <div id="vibetextbook-reading-content" tabIndex={-1}>
      <ReaderContinuityCoordinator/><ReaderStudyInteractions/><ReaderTermExplainer/><ReaderAnnotationManager/><ReaderSecondaryToolsDrawer/>{children}
    </div>
    <StudyCapturePanel publicationId={params.publicationId}/><ReaderPurchaseBar publicationId={params.publicationId}/><ReaderLearningLauncher/><ReaderAssessmentLauncher/><TeacherMaterialLauncher/><TeacherContentDeriveLauncher/>
  </div>;
}