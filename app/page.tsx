...
<section className={styles.audienceSection}><div className={styles.sectionHeading}><p className={styles.eyebrowDark}>ONE SYSTEM · THE RIGHT VIEW FOR EACH PERSON</p><h2>Learners, teachers, parents and schools stay connected without becoming the same user.</h2><p>Each person gets the tools and context that match their responsibility while the learning journey remains connected underneath.</p></div><div className={styles.audienceGrid}>{audiences.map(item=><article key={item.title} className={styles.audienceCard}><h3>{item.title}</h3><p>{item.body}</p><Link href={item.href}>{item.action} →</Link></article>)}</div></section>
<section className={styles.productProof}><div className={styles.proofIntro}><p className={styles.eyebrowLight}>THE VIBESCHOOL DIFFERENCE</p><h2>The lesson, the evidence and the next action should belong to the same story.</h2><p>VibeSchool is organised around educational continuity: curriculum informs planning, teaching creates learning activity, evidence improves understanding, and that understanding should shape what happens next.</p></div><div className={styles.proofGrid}>{proof.map(item=><div key={item.title} className={styles.proofCard}><span>{item.number}</span><h3>{item.title}</h3><p>{item.body}</p></div>)}</div><div className={styles.systemLine}><span>Curriculum</span><b>→</b><span>Scheme</span><b>→</b><span>Lesson</span><b>→</b><span>Learning</span><b>→</b><span>Evidence</span><b>→</b><span>Assessment</span><b>→</b><span>Understanding</span><b>→</b><span>Next action</span></div></section>
<SchoolReadinessAssessment/>
<section className={styles.demoSection}><div className={styles.sectionHeading}><p className={styles.eyebrowDark}>EXPLORE VIBESCHOOL</p><h2>See the educational model before deciding whether it belongs in your school or family.</h2><p>Start with the product map, public learning and Pathways experiences, then inspect how VibeSchool approaches institutions, evidence, privacy and responsibility.</p></div><div className={styles.demoGrid}><Link href="/product" className={styles.demoCard}><span className={styles.demoLabel}>PRODUCT</span><h3>See how the system connects across every role.</h3><p>Explore teacher, learner, parent and school capabilities together, and see how the learning chain connects curriculum to the next action.</p><strong>See the product →</strong></Link><TrackedLink href="/global" event="public_home_start_learning" className={styles.demoCard}><span className={styles.demoLabel}>LEARNING</span><h3>Learning that continues beyond a single resource.</h3><p>Explore curriculum-organised learning and how resources, practice and progress fit into the wider learner journey.</p><strong>Explore learning →</strong></TrackedLink><TrackedLink href="/pathways" event="public_home_pathways" className={styles.demoCard}><span className={styles.demoLabel}>SENIOR SCHOOL PATHWAYS</span><h3>Move from interests to subjects, careers and schools with clearer evidence.</h3><p>Explore pathways and subject choices while seeing where verified information begins, where guidance is used and where uncertainty remains.</p><strong>Explore Pathways →</strong></TrackedLink></div></section>
<section className={styles.trustSection}><div className={styles.sectionHeading}><p className={styles.eyebrowDark}>TRUST IS PART OF THE PRODUCT</p><h2>Education software should be useful enough to rely on and clear enough to question.</h2><p>VibeSchool treats evidence, access, child safety, uncertainty and human responsibility as product requirements — not footer decoration.</p></div><div className={styles.trustGrid}>{trust.map(([title,body])=><article key={title}><h3>{title}</h3><p>{body}</p></article>)}</div><div className={styles.trustLinks}><Link href="/trust">Trust Centre</Link><Link href="/about">How VibeSchool works</Link><Link href="/contact">Contact VibeSchool</Link></div></section>
