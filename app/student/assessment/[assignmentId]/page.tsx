...
type AnswerState = Record<string, { text: string; value: unknown; saving: boolean; saved: boolean }>
...
export default function LearnerAssessmentPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const savedMap = new Map(loaded.responses.map(response => [response.assessmentItemId, response]))
        setAnswers(Object.fromEntries(loaded.items.map(item => {
          const saved = savedMap.get(item.id)
          return [item.id, { text: saved?.responseText ?? '', value: saved?.responseValue ?? null, saving: false, saved: Boolean(saved) }]
        })))
      } catch (loadError) {
  ...
  }, [workspace?.expiresAt, result, submitting])
  ...
  function updateText(itemId: string, text: string) {
    setAnswers(current => ({ ...current, [itemId]: { ...(current[itemId] ?? { value: null, saving: false, saved: false }), text, saved: false } }))
  }
  function updateValue(itemId: string, value: unknown) {
    setAnswers(current => ({ ...current, [itemId]: { ...(current[itemId] ?? { text: '', saving: false, saved: false }), value, saved: false } }))
  }
  ...
  async function persist(item: LearnerAssessmentItem) {
    ...
    try {
      await saveResponse({ attemptId: workspace.attemptId, assessmentItemId: item.id, responseValue: current.value === undefined ? null : current.value, responseText: current.text || null })
      setAnswers(state => ({ ...state, [item.id]: { ...state[item.id], saving: false, saved: true } }))
    } catch (saveError) {
      console.error('[LearnerAssessment] save response', saveError)
      setAnswers(state => ({ ...state, [item.id]: { ...state[item.id], saving: false, saved: false } }))
  ...
  async function submit(expired = false) {
    ...
    try {
      ...
      setResult(await submitAttempt(workspace.attemptId))
    } catch (submitError) {
      ...
      <main style={shell}>
        <section style={{ ...card, borderColor: '#6ee7b7' }}>
          ...
          {result.scoreReleased && result.score !== null && result.maxScore !== null && result.percentage !== null ? (
            <><p style={{ fontSize: 30, fontWeight: 800, margin: '12px 0' }}>{result.score}/{result.maxScore}</p><p style={{ color: '#374151' }}>{result.percentage.toFixed(1)}%</p></>
          ) : (
            <p style={{ color: '#374151', lineHeight: 1.6 }}>Your answers were submitted. Your score will appear when your teacher releases it.</p>
          )}
          <p style={{ color: '#6b7280', lineHeight: 1.5 }}>
            {result.submittedDueToExpiry ? 'Time ended and the attempt was submitted automatically. ' : ''}
            {result.manualItems > 0 ? `${result.manualItems} written response${result.manualItems === 1 ? '' : 's'} will be reviewed by your teacher.` : 'All questions were processed.'}
          </p>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        <section style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: '#4338ca' }}>{workspace.assessmentType.replaceAll('_', ' ').toUpperCase()}</div>
        ...
        {workspace.items.map((item, index) => {
          ...
          return (
            <section key={item.id} style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><strong>Question {index + 1}</strong><span style={{ color: '#6b7280', fontSize: 12 }}>{item.marks} mark{item.marks === 1 ? '' : 's'}</span></div>
              <p style={{ lineHeight: 1.6 }}>{item.prompt}</p>
              {item.questionType === 'true_false' ? (
                <div style={{ display: 'flex', gap: 10 }}>{['true', 'false'].map(option => <button key={option} type="button" disabled={locked} onClick={() => updateText(item.id, option)} onBlur={() => void persist(item)} style={{ ...secondaryButton, flex: 1, background: answer?.text === option ? '#eef2ff' : '#fff', borderColor: answer?.text === option ? '#4338ca' : '#d1d5db' }}>{option === 'true' ? 'True' : 'False'}</button>)}</div>
              ) : item.questionType === 'multiple_choice' && Array.isArray(item.options) ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{item.options.map((option, optionIndex) => <button key={optionIndex} type="button" disabled={locked} onClick={() => updateValue(item.id, option)} onBlur={() => void persist(item)} style={{ ...secondaryButton, textAlign: 'left', background: JSON.stringify(answer?.value) === JSON.stringify(option) ? '#eef2ff' : '#fff' }}>{String(option)}</button>)}</div>
              ) : (
                <textarea disabled={locked} value={answer?.text ?? ''} onChange={event => updateText(item.id, event.target.value)} onBlur={() => void persist(item)} rows={item.questionType === 'structured' ? 5 : 3} placeholder="Type your answer" style={input} />
              )}
              <div style={{ marginTop: 8, fontSize: 11, color: '#6b7280' }}>{answer?.saving ? 'Saving…' : answer?.saved ? 'Saved ✓' : 'Not saved yet'}</div>
            </section>
          )
        })}

        {error && <div style={{ ...card, color: '#b91c1c', borderColor: '#fecaca' }}>{error}</div>}
        <button type="button" onClick={() => void submit(false)} disabled={locked} style={{ ...primaryButton, width: '100%', opacity: locked ? 0.6 : 1, cursor: locked ? 'not-allowed' : 'pointer' }}>{submitting ? 'Submitting…' : 'Submit Assessment'}</button>
      </div>
    </main>
  )
}
...
const primaryButton: React.CSSProperties = { border: 'none', borderRadius: 12, padding: '13px 16px', background: '#4338ca', color: '#fff', fontWeight: 800, fontSize: 13, fontFamily: 'inherit' }
