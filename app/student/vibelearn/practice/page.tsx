...
export default function VibeLearnPracticePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        if (hasReaderSource) {
          const grounded = await getGroundedChapterPractice({
        ...
        if (!cancelled) setQuestions((data ?? []).flatMap(value => {
          const parsed = asBankQuestion(value)
  ...
  async function checkAnswer() {
    ...
    try {
      if (question.kind === 'grounded') {
        const result = await recordGroundedPracticeAnswer({
          contentBlockId: question.contentBlockId,
  ...
  function restart() {
    ...
    setShowHint(false)
    startedAtRef.current = Date.now()
    sessionIdRef.current = typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : null
  }
    ...
    <main style={shell}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        {sourceContext && <section style={{ ...card, borderColor: '#c7d2fe', background: '#f5f3ff' }}>
          <div style={eyebrowDark}>Exact learning source</div>
          ...
          <p style={{ ...muted, marginTop: 8 }}>Questions come from authored assessable blocks in this exact unit. Answers are scored by the server, and mistakes retain a link back to their source section.</p>
        </section>}

        {error && <section style={{ ...card, color: '#b91c1c' }}>{error}</section>}

        {loading ? <section style={card}>Loading questions…</section>
          : questions.length === 0 ? <section style={card}><strong>No grounded questions available</strong><p style={muted}>{sourceContext ? 'This unit is readable, but it has no authored, markable question blocks yet. Review the unit or choose another practice set.' : 'Choose another subject or topic from VibeLearn.'}</p></section>
          : finished ? <section style={card}>
              <div style={eyebrowDark}>Session complete</div>
              <h2 style={{ fontSize: 28, margin: '8px 0' }}>{scoreLabel}</h2>
              <p style={muted}>Your answers and grounded mistakes are now part of your learning journey and revision evidence.</p>
              <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
                <button style={primaryButton} onClick={restart}>Try again</button>
          ...
          : question && <>
              <section style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
                  ...
                  <div style={pill}>{question.kind === 'grounded' ? 'From this unit' : (question.topic ?? question.subject)}</div>
                </div>
                ...
                {question.kind === 'grounded' ? <textarea
                  value={textAnswer}
                ...
                {!submitted && <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
                  {question.kind === 'bank' && <button style={secondaryButton} onClick={() => setShowHint(value => !value)}>{showHint ? 'Hide hint' : 'Need a hint?'}</button>}
                  <button style={{ ...primaryButton, opacity: (question.kind === 'grounded' ? !textAnswer.trim() : selected === null) || saving ? 0.5 : 1 }} disabled={(question.kind === 'grounded' ? !textAnswer.trim() : selected === null) || saving} onClick={() => void checkAnswer()}>{saving ? 'Checking…' : 'Check answer'}</button>
                </div>}
                ...
                {submitted && <div style={answerCorrect ? correctBox : incorrectBox}>
                  ...
                  {question.kind === 'grounded' ? <div style={{ marginTop: 6 }}>
                    {expectedAnswer && <div><strong>Expected answer:</strong> {expectedAnswer}</div>}
                    {!answerCorrect && <div style={{ marginTop: 6 }}>Review the exact source section, then retry this practice set.</div>}
                  </div> : <div style={{ marginTop: 6 }}>{question.explanation ?? 'Review the correct option and compare it with your choice before continuing.'}</div>}
                  <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                    {!answerCorrect && reviewUrl && <button style={secondaryButton} onClick={() => router.push(reviewUrl)}>Review this section</button>}
                    <button style={primaryButton} onClick={nextQuestion}>{index + 1 === questions.length ? 'Finish session' : 'Next question'}</button>
                  </div>
                </div>}
              </section>
              ...
              <section style={{ ...card, background: '#faf5ff', borderColor: '#ddd6fe' }}>
                ...
                <p style={{ ...muted, lineHeight: 1.6 }}>Scoring is deterministic and server-authoritative. VibeTwin does not answer questions or reveal solutions while scoring is active.</p>
              </section>
            </>}
      </div>
    </main>
...
const answerInput: React.CSSProperties = { width: '100%', boxSizing: 'border-box', border: '1px solid #cbd5e1', borderRadius: 12, padding: 13, font: 'inherit', color: '#0f172a', background: '#fff', resize: 'vertical' }
...
const primaryButton: React.CSSProperties = { border: 'none', background: '#4f46e5', color: '#fff', borderRadius: 10, padding: '10px 14px', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }
const secondaryButton: React.CSSProperties = { border: '1px solid #cbd5e1', background: '#fff', color: '#334155', borderRadius: 10, padding: '10px 14px', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }
const pill: React.CSSProperties = { fontSize: 11, fontWeight: 800, borderRadius: 999, background: '#eef2ff', color: '#4338ca', padding: '5px 9px' }
const hintBox: React.CSSProperties = { marginTop: 12, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: 12, color: '#92400e', fontSize: 13 }
const correctBox: React.CSSProperties = { marginTop: 14, background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: 12, padding: 13, color: '#065f46' }
const incorrectBox: React.CSSProperties = { marginTop: 14, background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 12, padding: 13, color: '#9a3412' }
