...
export default function LearnerReportCardsPage() {
  ...
  async function open(report: PublishedReportSummary) {
    setError('')
    try {
      const [published, longitudinal] = await Promise.all([
        getPublishedReportCard(report.reportCardId),
        getLongitudinalReportRecord(report.studentId),
      ])
      setDetail(published)
      setHistory(longitudinal.trends && typeof longitudinal.trends === 'object' && !Array.isArray(longitudinal.trends) ? longitudinal.trends as Record<string, unknown> : null)
  ...
  return <main style={shell}><div style={{ maxWidth: 920, margin: '0 auto' }}>
    <section style={card}><div style={eyebrow}>My Learning Journey</div><h1 style={{ margin: '6px 0' }}>My Report Cards</h1><p style={muted}>See your published reports and how your learning has changed over time.</p></section>
    {error && <section style={{ ...card, color: '#b91c1c' }}>{error}</section>}
    {loading ? <section style={card}>Loading reports…</section> : reports.length === 0 ? <section style={card}>No published report cards are available yet.</section> : reports.map(report => <section key={report.reportCardId} style={card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><div><div style={eyebrow}>{report.termName} · {report.academicYear}</div><h2 style={{ margin: '5px 0', fontSize: 18 }}>{report.className}</h2><div style={muted}>Revision {report.revision}</div></div><strong style={{ textTransform: 'capitalize' }}>{report.status}</strong></div>
      <button onClick={() => void open(report)} style={{ ...button, marginTop: 12 }}>View report and growth</button>
    </section>)}

    {detail && <section style={card}>
      <div style={eyebrow}>Published Learning Record</div>
      <h2 style={{ margin: '6px 0' }}>Progress summary</h2>
      ...
      {history && <div style={{ marginTop: 14 }}><div style={label}>Growth over time</div><div style={grid}><Metric label="Reports" value={String(history.report_count ?? '—')} /><Metric label="Latest assessment" value={percent(history.latest_assessment_average)} /><Metric label="Latest mastery" value={percent(history.latest_mastery_average)} /><Metric label="Starting mastery" value={percent(history.first_mastery_average)} /></div></div>}
