      ...
      {filtered.length === 0 ? <section style={{ ...card, textAlign: 'center', padding: '42px 20px' }}><div style={{ fontSize: 30, marginBottom: 8 }}>✓</div><strong>No tasks in this section</strong><p style={{ margin: '6px 0 0', color: 'var(--vs-muted)', fontSize: 12 }}>Your next assigned task will appear here.</p></section> : <div style={{ display: 'grid', gap: 10 }}>
        {filtered.map(task => {
          ...
          return <article key={task.taskId} style={{ ...card, borderLeft: `4px solid ${tone.accent}` }}>
            ...
            <button type="button" disabled={task.status === 'awaiting_marking' || task.status === 'closed' || task.status === 'upcoming'} onClick={() => router.push(task.actionUrl)} style={{ ...taskButton, background: tone.accent, opacity: ['awaiting_marking', 'closed', 'upcoming'].includes(task.status) ? .55 : 1 }}>{task.actionLabel}</button>
          </article>
        })}
      </div>}
...
function Stat({ label, value, emphasis = false }: { label: string; value: number; emphasis?: boolean }) {
  return <div style={{ ...stat, background: emphasis ? '#fff7ed' : 'var(--vs-card)' }}><strong style={{ fontSize: 18, color: emphasis ? '#c2410c' : 'var(--vs-text)' }}>{value}</strong><span style={{ fontSize: 9, color: 'var(--vs-muted)', fontWeight: 700 }}>{label}</span></div>
}
