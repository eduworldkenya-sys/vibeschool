...
async function fetchLocationOptions(county:string,subCounty:string){
  const {data,error}=await supabase.rpc('schools_location_options_public_v1',{
...
export default function SchoolsDirectoryPage(){
  ...
  async function load(){
    ...
    const [canonicalResult,pendingResult]=await Promise.all([
      supabase.rpc('schools_search_public_v2',{
        p_query:query.trim()||null,p_county:county||null,p_sub_county:subCounty||null,p_ward:ward||null,
        p_school_category:schoolCategory,p_ownership_type:null,p_gender_type:null,p_accommodation_type:null,p_limit:100,
      }),
      supabase.rpc('schools_search_community_pending_v2',{
        p_query:query.trim()||null,p_county:county||null,p_sub_county:subCounty||null,p_ward:ward||null,p_level:level||null,p_limit:25,
      }),
    ])
  ...
  useEffect(()=>{
    void fetchLocationOptions('','').then(options=>setCounties(options.filter(o=>o.option_type==='county').map(o=>o.value)))
    void load()
  },[])
  ...
  function openMissing(){
    setMissing({...emptyMissing,name:query,county,subCounty,ward,level})
    setMissingOtherSubCounty('');setMissingOtherWard('');setFeedback('');setShowMissing(true)
  }
  ...
  async function submitMissing(e:FormEvent){
    ...
    const {data,error}=await supabase.rpc('submit_school_discovery_request',{
      p_name:missing.name.trim(),p_county:missing.county||null,p_sub_county:finalSubCounty||null,p_ward:finalWard||null,
      p_level:missing.level||null,p_school_code:missing.code.trim()||null,p_lat:null,p_lng:null,p_alternative_name:null,
      p_notes:missing.notes.trim()||null,p_contact_name:null,p_contact_phone:null,
    })
    setSaving(false)
  ...
  return <div className={styles.page}>
    ...
    <main id="main-content" className={styles.main}>
      <section className={styles.hero}>
        ...
        <p className={styles.lead}>Search by school name, county, sub-county and ward. Location choices narrow as you go, so users do not need to know the exact spelling.</p>
      </section>
      ...
      <section className={styles.panel} aria-label="Find a school">
        ...
        <div className={styles.notice}><strong>Location guide:</strong> counties are available nationally. Sub-counties and wards appear as verified reference data is added; Nakuru is seeded first, including Gilgil and its wards.</div>
      </section>
      ...
      {!loading&&!error&&<>
        ...
        <div className={styles.cards}>{rows.map(s=><Link className={styles.card} href={`/schools/${s.school_id}`} key={s.school_id}>
          <div><h3 className={styles.schoolName}>{s.school_name}</h3><p className={styles.meta}>{[s.ward,s.sub_county,s.county].filter(Boolean).join(' · ')||'Location not yet verified'}</p></div>
        ...
        {pendingRows.length>0&&<section className={styles.section} aria-label="Community submitted schools"><div className={styles.resultsHeader}><div><span className={styles.eyebrow}>COMMUNITY SUBMITTED</span><h2>Verification pending</h2></div><span className={styles.small}>Not yet canonical</span></div><div className={styles.cards}>{pendingRows.map(s=><article className={styles.card} key={s.request_id}>
          <div><h3 className={styles.schoolName}>{s.school_name}</h3><p className={styles.meta}>{[s.ward,s.sub_county,s.county].filter(Boolean).join(' · ')||'Approximate location not supplied'}</p></div>
      ...
      {showMissing&&<section className={`${styles.panel} ${styles.section}`} aria-label="Add a missing school">
        <h2>Add a missing school</h2><p className={styles.meta}>Choose the location where available. If a sub-county or ward has not been indexed yet, select “Not listed” and type it once.</p>
        <form onSubmit={submitMissing}>
          <div className={styles.searchGrid}>
            ...
            {missing.subCounty==='__OTHER__'&&<label className={styles.field}><span>Sub-county name</span><input className={styles.input} value={missingOtherSubCounty} onChange={e=>setMissingOtherSubCounty(e.target.value)} placeholder="Type sub-county"/></label>}
            ...
            {missing.ward==='__OTHER__'&&<label className={styles.field}><span>Ward name</span><input className={styles.input} value={missingOtherWard} onChange={e=>setMissingOtherWard(e.target.value)} placeholder="Type ward"/></label>}
