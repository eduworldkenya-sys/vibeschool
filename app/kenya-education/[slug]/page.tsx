...
export default function KnowledgeArticlePage({params}:{params:{slug:string}}){
  ...
  const schema={'@context':'https://schema.org','@type':'Article','@id':`${canonical}#article`,headline:article.title,description:article.description,url:canonical,mainEntityOfPage:{'@type':'WebPage','@id':canonical},author:{'@type':'Organization','@id':`${SITE_URL}/#organization`,name:'VibeSchool Editorial'},publisher:{'@type':'EducationalOrganization','@id':`${SITE_URL}/#organization`,name:'VibeSchool',url:SITE_URL},inLanguage:'en-KE',dateModified:article.updated_on,isAccessibleForFree:true,citation:sources.map(source=>source.url),about:article.audience.map(name=>({'@type':'Thing',name}))}
