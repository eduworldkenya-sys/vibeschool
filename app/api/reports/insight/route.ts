...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!reportType || !data) return NextResponse.json({ error: 'Missing reportType or data' }, { status: 400 })
    ...
    const response = await invokeGovernedCyborgModel(adapter, { provider: 'anthropic', model: 'claude-sonnet-4-20250514', missionId, metadata: { maxTokens: 150, feature: 'report-insight' }, messages: [{ role: 'user', content: `You are an AI analyst for a Kenyan school management system (CBC curriculum).\nAnalyze this ${reportType} report data and return ONE concise insight sentence (max 25 words).\nFocus on the most actionable or concerning pattern. No preamble. Just the insight.\nData: ${summary}` }] })
    ...
    return NextResponse.json({ insight, missionId, cyborgCapability: response.capabilityJti })
  } catch (err) { console.error('AI insight error:', err); return NextResponse.json({ insight: 'Unable to generate insight at this time.' }, { status: 200 }) }
}
