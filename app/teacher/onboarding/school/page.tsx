...
export default function SchoolOnboardingPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('county', county)
      .then(({ data }) => {
        const unique = [...new Set((data ?? []).map((r: {sub_county:string}) => r.sub_county).filter(Boolean))].sort()
        setSubCounties(unique)
        setLoadingSubs(false)
      })
  }, [county])

  // Load schools when sub-county changes
  useEffect(() => {
  ...
  async function handleSelectSchool() {
    ...
    // Check if school already exists in schools table
    ...
    if (existing) {
      ...
      // Auto-create school from directory
      const subdomain = selected.name.toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
      ...
      const { data: created, error: createErr } = await supabase
        .from('schools')
        .insert({ name: selected.name, subdomain, timezone: 'Africa/Nairobi', country_code: 'KE', status: 'active', created_by: user.id, requires_dual_approval: false })
        .select('id')
        .single()
  ...
  async function handleManual() {
    ...
    const subdomain = manualName.trim().toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '')
      .slice(0, 40) + '-' + Date.now().toString().slice(-4)
    ...
    const { data: school, error: schoolErr } = await supabase
      .from('schools')
      .insert({ name: manualName.trim(), subdomain, timezone: 'Africa/Nairobi', country_code: 'KE', status: 'active', created_by: user.id, requires_dual_approval: false })
      .select('id')
      .single()
  ...
  const lbl: React.CSSProperties = { fontSize: 11, fontWeight: 700, color: '#6b7280', letterSpacing: 1, textTransform: 'uppercase' }
      ...
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 20, padding: 28, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        {mode === 'choose' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button onClick={() => setMode('join')} style={btn('transparent', accent)}>🔗 Join with a Code</button>
