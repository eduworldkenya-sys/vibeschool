...
function score(items: ContentRow[]): number {
  ...
  const totalViews  = items.reduce((a, c) => a + c.view_count, 0);
  const tagged      = items.filter(c => c.tags?.length > 0).length;
  const described   = items.filter(c => c.description?.trim().length > 20).length;
  const hasUrl      = items.filter(c => c.url?.trim()).length;
  const liveScore   = Math.min((live / Math.max(items.length, 1)) * 30, 30);
  const viewScore   = Math.min(totalViews * 2, 30);
  const tagScore    = Math.min((tagged / items.length) * 20, 20);
  const descScore   = Math.min((described / items.length) * 10, 10);
  const urlScore    = Math.min((hasUrl / items.length) * 10, 10);
...
export default function IndexerPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [contentRes, statsRes] = await Promise.all([
        supabase.from("vibelearn_content").select("id,title,view_count,earnings_ksh,tags,description,url,status,created_at").eq("submitted_by", user.id),
        supabase.from("vibelearn_teacher_stats").select("teacher_rank").eq("teacher_id", user.id).maybeSingle(),
      ]);
  ...
  const tips: { icon: string; title: string; body: string; done: boolean }[] = [
    ...
    { icon: "🔄", title: "Publish at least weekly", body: "Freshness boosts your index score over time.", done: (() => { if (items.length === 0) return false; const latest = new Date(items.sort((a,b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].created_at); return Date.now() - latest.getTime() < 7 * 86400000; })() },
  ];
  ...
  if (loading) return (
    <div style={{ padding: 0 }}>
      {[1,2,3].map(i => <div key={i} style={{ ...card, height: 80 }} />)}
    </div>
  );
    ...
    <div style={{ animation: "slideIn 0.22s ease" }}>
      ...
      <div style={card}>
        ...
        {[
          ...
          { label: "Total Views",     value: totalViews,   max: 50,                         suffix: " views",             color: "#0284c7" },
      ...
      <div style={card}>
        ...
        {tips.map(tip => (
          <div key={tip.title} style={{ display: "flex", gap: 12, alignItems: "flex-start", paddingBottom: 14, marginBottom: 14, borderBottom: `1px solid ${C.border}` }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, background: tip.done ? "#d1fae5" : "#f3f4f6" }}>
              {tip.done ? "✓" : tip.icon}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: tip.done ? C.accent : C.textPrimary, textDecoration: tip.done ? "line-through" : "none" }}>{tip.title}</div>
