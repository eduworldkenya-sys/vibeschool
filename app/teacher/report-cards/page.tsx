    ...
    {loading ? <section style={card}>Loading report cards…</section> : items.length === 0 ? <section style={card}>No report cards yet.</section> : items.map(item => {
      ...
      return <section key={item.id} style={card}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
          ...
          {editable && item.completenessStatus !== 'not_generated' && <button disabled={busyId === item.id} onClick={() => void generateNarratives(item)} style={secondaryButton}>Generate narratives</button>}
          {editable && <button disabled={busyId === item.id} onClick={() => void validate(item)} style={secondaryButton}>Validate report</button>}
          ...
          {editable && <button disabled={busyId === item.id || !canSubmit} onClick={() => void submit(item)} style={primaryButton}>Submit for review</button>}
        </div>
