            ...
            {SECTIONS.map((section, i) => (
              <button
                ...
                style={{
                  ...
                  fontWeight: activeSection === i ? 600 : 400,
                  color: activeSection === i ? "#00E5A0" : "rgba(255,255,255,0.4)",
                  background: activeSection === i ? "rgba(0,229,160,0.08)" : "transparent",
                  border: activeSection === i ? "1px solid rgba(0,229,160,0.2)" : "1px solid transparent",
                  cursor: "pointer",
  ...
  const pdEntries = [
    {
      ...
      takeaway: "Aligned my continuous assessment with CBC competency framework",
    },
    {
          ...
          {[
            ...
            <div key={step.label} className="flex items-center gap-4 p-3 rounded-xl bg-white/4 border border-white/8">
              ...
              <div className="flex-1">
                <p className={`text-sm font-medium ${step.status === "pending" ? "text-[#FFB800]" : step.status === "done" ? "text-[#00E5A0]" : "text-white/40"}`}>
                  {step.label}
                </p>
          ...
          return (
            <div key={doc.name} className="flex items-center justify-between p-3.5 rounded-xl bg-white/3 border border-white/8 hover:border-white/15 transition-colors">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button className="text-xs text-[#00B8FF] hover:underline">{doc.uploaded ? "Replace" : "Upload"}</button>
              </div>
            </div>
          );
        })}
  ...
  return (
    <div>
      ...
      <div className="mt-4 p-4 rounded-xl bg-white/4 border border-white/8 text-center">
        ...
        <a href="/finance/payslips" className="text-[#00B8FF] text-sm hover:underline mt-1 block">View payslip history →</a>
      </div>
    </div>
  );
}