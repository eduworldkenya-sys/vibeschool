...
const FORMAT_OPTIONS: Array<{ format: StudioFormat; title: string; description: string; icon: string }> = [
  { format: 'vibetextbook', title: 'Interactive textbook', description: 'Curriculum-linked units, illustrations, activities, questions, media and revision history.', icon: '🎓' },
  { format: 'ebook', title: 'eBook', description: 'Chapter-based publication using the same rich editor and content blocks.', icon: '📘' },
]
...
export default function TeacherContentStudioPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await sb.from('vibe_publications').select('*').eq('author_id', user.id).in('format', ['vibetextbook', 'ebook']).order('updated_at', { ascending: false })
      if (cancelled) return
  ...
  return <div style={{ minHeight: '100dvh', background: BG, color: TEXT, padding: '24px 16px 110px', fontFamily: 'system-ui,-apple-system,sans-serif' }}><div style={{ maxWidth: 760, margin: '0 auto' }}>
    ...
    <section><div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.1em', color: MUTED }}>YOUR CONTENT</div><div style={{ fontSize: 13, color: MUTED, marginTop: 4 }}>Drafts and published resources use the same creation system.</div></div>
      {publications.length === 0 ? <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28, textAlign: 'center', color: MUTED }}>No studio publications yet.</div> : <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>{publications.map(item => <button key={item.id} onClick={() => router.push(`/teacher/studio/editor?format=${item.format === 'ebook' ? 'ebook' : 'vibetextbook'}&publication=${item.id}`)} style={{ width: '100%', textAlign: 'left', background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 15, color: TEXT, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div style={{ minWidth: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><strong style={{ fontSize: 14 }}>{item.title || 'Untitled publication'}</strong><span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.08em', color: item.status === 'published' ? ACCENT : MUTED, border: `1px solid ${item.status === 'published' ? 'rgba(204,255,0,.35)' : BORDER}`, borderRadius: 999, padding: '3px 7px' }}>{item.status.toUpperCase()}</span></div><div style={{ fontSize: 11, color: MUTED, marginTop: 5 }}>{item.format === 'vibetextbook' ? 'Interactive textbook' : 'eBook'} · {item.chapter_count} {item.chapter_count === 1 ? 'unit' : 'units'} · Updated {dateLabel(item.updated_at)}</div></div><span style={{ color: ACCENT, fontSize: 20 }}>›</span></button>)}</div>}
    </section>
  </div></div>
}
