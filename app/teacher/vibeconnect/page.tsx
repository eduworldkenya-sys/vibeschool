...
function Skeleton({ h = 56, r = 12 }: { h?: number; r?: number }) {
  return (
    <div style={{ height: h, borderRadius: r, background: 'linear-gradient(90deg,#e2e8f0 25%,#f1f5f9 50%,#e2e8f0 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />
  )
}
...
function ContextTag({ tag }: { tag: string }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', fontSize: '11px', fontWeight: 600, padding: '2px 7px', borderRadius: '99px', background: t.bg, color: t.color, whiteSpace: 'nowrap', flexShrink: 0 }}>{t.icon} {t.label}</span>
  )
}
...
async function ensureVCId(userId: string, fullName: string) {
  ...
  const vcId = `@${slug}.${Math.floor(1000 + Math.random() * 9000)}`
...
const COMPOSE_TYPES = [
  { type: 'general',   icon: '💬', label: 'Message to parent',  desc: "Reach out to a student's parent" },
  { type: 'colleague', icon: '👥', label: 'Message to colleague', desc: 'Message another staff member' },
  { type: 'concern',   icon: '🚨', label: 'Report concern',      desc: 'Flag a safeguarding or welfare concern' },
]

export default function TeacherVibeConnectPage() {
  ...
  async function loadUser() {
    try {
      ...
      if (!user) { router.replace('/teacher/pulse'); return }
      ...
      if (!p || p.role !== 'teacher') { router.replace('/teacher/pulse'); return }
    ...
    } catch { router.replace('/teacher/pulse') } finally { setLoading(false) }
  }
  ...
  async function loadAll(uid: string, sid: string) {
    ...
    if (circIds.length > 0) {
      ...
      const normalized: CircularUI[] = (circs ?? []).map(c => {
  ...
  async function searchPeople(q: string) {
    ...
    setSearching(true)
    const roles = pendingTag === 'colleague' ? ['teacher', 'admin'] : ['parent', 'teacher', 'admin']
    const { data } = await supabase.from('profiles').select('id, full_name, role').eq('school_id', schoolId).neq('id', userId).in('role', roles).ilike('full_name', `%${q}%`).limit(10)
    setSearchResults(
      (data ?? []).map(row => ({
        id: row.id,
        ...
        <>
          {threads.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 20px', background: C.surface, borderRadius: '16px', border: `1px solid ${C.border}` }}>
              ...
              <p style={{ fontSize: '13px', color: C.muted, marginTop: '4px' }}>Tap + to message a parent or colleague</p>
            </div>
          ) : (
        ...
        <>
          {circulars.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 20px', background: C.surface, borderRadius: '16px', border: `1px solid ${C.border}` }}>
              ...
              <p style={{ fontSize: '13px', color: C.muted, marginTop: '4px' }}>School circulars will appear here</p>
            </div>
          ) : (
                ...
                <div key={c.id} style={{ background: C.surface, border: `1px solid ${c.requires_ack && !c.acked ? C.warning : C.border}`, borderRadius: '14px', padding: '16px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                    ...
                    {c.requires_ack && (c.acked
                      ...
                      : <button onClick={() => acknowledgeCircular(c.recipientId, c.id)} disabled={acking === c.id} style={{ padding: '6px 14px', borderRadius: '10px', border: 'none', background: C.warning, color: '#fff', fontWeight: '700', fontSize: '12px', cursor: 'pointer', opacity: acking === c.id ? 0.6 : 1 }}>{acking === c.id ? 'Saving...' : 'Acknowledge'}</button>
                    )}
                  </div>
                </div>
              ))}
