...
export default function ParentChildPage() {
  ...
  const load = useCallback(async () => {
    ...
    // This is deliberately the first child-scoped read. Parent RLS on students
    // is the deep-link authority gate: a guessed or revoked learner ID returns no row.
    ...
    if (todayRes.error || termRes.error) {
      setError("Attendance is temporarily unavailable. No other learner's data has been substituted.")
    }
  ...
  return (
    <main className="mx-auto min-h-screen max-w-xl bg-slate-50 px-4 pb-28 pt-4 text-slate-950">
      <button onClick={() => router.push("/parent")} className="mb-3 min-h-11 text-sm font-semibold text-slate-600">
        ← Parent Home
      </button>
      ...
      <section className="mt-4 grid grid-cols-2 gap-3" aria-label={`${firstName}'s family actions`}>
        <button onClick={() => router.push(`/parent/child/${child.id}/homework`)} className="min-h-24 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm">
          <span className="block text-lg" aria-hidden="true">📚</span>
          <span className="mt-2 block text-sm font-bold">Homework</span>
          <span className="mt-1 block text-xs leading-5 text-slate-500">Tasks, due dates and submission status</span>
        </button>
        <button onClick={() => router.push(`/parent/assessments?studentId=${child.id}`)} className="min-h-24 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm">
          <span className="block text-lg" aria-hidden="true">📊</span>
          <span className="mt-2 block text-sm font-bold">Results</span>
          <span className="mt-1 block text-xs leading-5 text-slate-500">Released assessment results only</span>
        </button>
        <button onClick={() => router.push(`/parent/child/${child.id}/growth`)} className="min-h-24 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm">
          <span className="block text-lg" aria-hidden="true">🌱</span>
          <span className="mt-2 block text-sm font-bold">Learning & progress</span>
          <span className="mt-1 block text-xs leading-5 text-slate-500">Understand progress and areas to support</span>
        </button>
        <button onClick={() => router.push(`/parent/child/${child.id}/messages`)} className="min-h-24 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm">
          <span className="block text-lg" aria-hidden="true">💬</span>
          <span className="mt-2 block text-sm font-bold">Messages</span>
          <span className="mt-1 block text-xs leading-5 text-slate-500">Child-scoped school communication</span>
        </button>
      </section>
            ...
            {todayAttendance.length === 0 ? (
              ...
              <div className="mt-2 space-y-2">
                {todayAttendance.map(row => (
                  <p key={row.id} className="text-sm font-semibold text-slate-800">{statusCopy(row.status)}</p>
                ))}
              </div>
            )}
          </div>
      ...
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button onClick={() => router.push(`/parent/child/${child.id}/profile`)} className="min-h-11 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold">Profile</button>
          <button onClick={() => router.push(`/parent/child/${child.id}/finance`)} className="min-h-11 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold">Finance</button>
        </div>
      </section>
    </main>
  )
