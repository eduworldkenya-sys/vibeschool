...
export default function ParentHelpPage() {
  ...
  return <div style={{ paddingBottom: 24 }}>
    <section style={hero}><div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,.62)" }}>PARENT GUIDE</div><h1 style={{ fontSize: 23, margin: "6px 0" }}>How VibeSchool works</h1><p style={{ fontSize: 13, lineHeight: 1.55, color: "rgba(255,255,255,.72)", margin: 0 }}>Use VibeSchool to understand your child's school journey, notice what needs attention and stay connected with the people supporting them.</p></section>
    <section style={card}><h2 style={heading}>The simple way to use VibeSchool</h2><div style={{ display: "grid", gap: 10, marginTop: 12 }}>{[["1", "Orient", "Start at Home and see what changed."],["2", "Understand", "Open the evidence behind important updates."],["3", "Act", "Take the suggested next step when one is needed."],["4", "Confirm", "Check that your message, request or action was completed."]].map(([n,t,d]) => <div key={n} style={step}><div style={stepNumber}>{n}</div><div><div style={{ fontSize: 13, fontWeight: 850 }}>{t}</div><div style={muted}>{d}</div></div></div>)}</div></section>
    <div style={{ margin: "18px 2px 9px", fontSize: 15, fontWeight: 850 }}>Common questions</div>
    <section style={card}>{topics.map((topic, i) => <div key={topic.q} style={{ borderBottom: i === topics.length - 1 ? "none" : "1px solid #e5e7eb" }}><button onClick={() => setOpen(open === i ? null : i)} aria-expanded={open === i} style={question}>{topic.q}<span aria-hidden="true">{open === i ? "−" : "+"}</span></button>{open === i && <p style={{ ...muted, padding: "0 0 13px" }}>{topic.a}</p>}</div>)}</section>
    <section style={{ ...card, marginTop: 12, background: "#f8fafc" }}><div style={{ fontSize: 14, fontWeight: 850 }}>Need more help?</div><p style={{ ...muted, marginTop: 4 }}>If information is incorrect, access is wrong, or something does not work as expected, use Messages to contact the school or your school's support channel. Keep the child and the issue clear so the right person can help.</p><button onClick={() => router.push("/parent/messages")} style={primary}>Open Messages</button></section>
  </div>;
}
