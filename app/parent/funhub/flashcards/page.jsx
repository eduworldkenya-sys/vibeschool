  ...
  return (
    <div style={{ minHeight: '100dvh', background: BG, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 900, color: TEAL, margin: 0, letterSpacing: '-0.5px' }}>Flashcards</h1>
...
function FlipCard({ card, onGotIt, onStillLearning }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, width: '100%' }}>
      ...
      <div
        ...
        onPointerUp={() => { setPressing(false); setFlipped(f => !f); }}
        onPointerLeave={() => setPressing(false)}
        style={{ transform: pressing ? 'scale(0.97)' : 'scale(1)', transition: 'transform 0.12s' }}
      >
      ...
      {flipped ? (
        <div style={{ display: 'flex', gap: 12, width: '100%', maxWidth: 360 }}>
          <button
            ...
            style={{ flex: 1, padding: '14px 0', borderRadius: 16, border: '2px solid #fca5a5', background: '#fff', color: '#dc2626', fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s' }}
          >❌ Still learning</button>
          <button
            onClick={onGotIt}
            style={{ flex: 1, padding: '14px 0', borderRadius: 16, border: 'none', background: `linear-gradient(135deg, ${GREEN}, #15803d)`, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s', boxShadow: '0 4px 12px rgba(22,163,74,0.3)' }}
          >✅ Got it!</button>
        </div>
      ) : (
        <div style={{ height: 52 }} />
...
function GameScreen({ cards, subject, grade, onFinish }) {
    ...
    <div style={{ minHeight: '100dvh', background: BG, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}>
      <div style={{ width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${TEAL}, ${TEAL_DARK})`, borderRadius: 20, padding: '20px 0', textAlign: 'center', boxShadow: '0 6px 24px rgba(8,145,178,0.3)' }}>
          <p style={{ margin: 0, fontSize: 13, color: '#a5f3fc', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Cards Mastered</p>
  ...
  async function handleStart(sub, gr) {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        setErrorMsg(`No cards found for ${sub} Grade ${gr}. Try a different subject or grade!`);
        setScreen('error');
        return;
      }
