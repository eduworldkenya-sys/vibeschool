...
export default function AcademySignUp() {
  ...
  async function handleSubmit() {
    ...
    if (!/^\d{6}$/.test(code.trim())) { setError('Invitation code must be exactly 6 digits.'); return }

    setLoading(true)

    // Step 1 — check invitation code (does not consume it)
    ...
    if (!checkRes.ok || checkData.result !== 'accepted') {
      ...
      if (checkData.result === 'invalid')   { setError('Invalid invitation code.'); return }
      ...
      setError(checkData.error || 'Invalid invitation code.')
      return
    }
    ...
    // Step 4 — commit invitation (consume one use)
    await fetch('/api/validate-invitation', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      <div id="academy-signup-root" className={styles.root}>
        ...
        <div className={styles.content} ref={contentRef}>
          ...
          <div className={styles.form}>
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor="code">INVITATION CODE</label>
              <input
                id="code"
                className={styles.input}
                type="text"
          ...
          <p className={styles.switch}>
            ...
            <span
              ...
              tabIndex={0}
              onClick={() => fadeOut('/academy/signin')}
              onKeyDown={e => { if (e.key === 'Enter') fadeOut('/academy/signin') }}
            >
              Sign in
            </span>
          </p>
