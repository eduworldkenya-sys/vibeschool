...
export function SimpleArticleEditor({authorId, publicationId}:{authorId:string;publicationId?:string}){
  ...
  async function publish(){
    ...
    const checks=publicationChecks(publication.title??"",publication.description??"",publication.tags??[],body)
    if(checks.some(check=>!check.pass)) return
    setPublishing(true)
    const saved=await draft.forceSave()
    const published=saved&&await draft.publishPublication()
    setPublishing(false)
    if(published) router.push(`/blog/${draft.publication?.id}`)
  }
  ...
  const checks=publicationChecks(pub.title??"",pub.description??"",pub.tags??[],body)
  const publishReady=checks.every(check=>check.pass)
  return <main className="simple-editor">
    <header><button onClick={()=>router.push("/hq/studio")}>← Studio</button><div><strong>VibeSchool Article Editor</strong><span>{draft.saving?"Saving…":draft.lastSaved?"Saved automatically":"Draft"}</span></div><button className="publish" disabled={publishing||!publishReady} onClick={publish}>{publishing?"Publishing…":"Publish"}</button></header>
    <div className="workspace">
      ...
      <section className="canvas">
        ...
        <div className="row"><label>Category<select value={pub.genre} onChange={event=>draft.updatePublication({genre:event.target.value as typeof pub.genre})}><option value="academic">Revision</option><option value="magazine">Education news</option><option value="self_help">Study skills</option><option value="non_fiction">Careers & guidance</option></select></label><label>Cover image URL<input value={pub.cover_url??""} onChange={event=>draft.updatePublication({cover_url:event.target.value||null})} placeholder="Optional — branded fallback provided"/></label></div>
        <label>Tags<input value={(pub.tags??[]).join(", ")} onChange={event=>draft.updatePublication({tags:event.target.value.split(",").map(tag=>tag.trim()).filter(Boolean)})} placeholder="KCSE, Chemistry, Revision"/></label>
        ...
        <section className="readiness" aria-label="Publishing readiness"><div><strong>Ready to publish?</strong><span>{wordCount} words</span></div>{checks.map(check=><p className={check.pass?"pass":"todo"} key={check.label}>{check.pass?"✓":"○"} {check.label}</p>)}</section>
