...
export function HQNavigation(){
  ...
  return <>
    <aside className="hq-sidebar" aria-label="HQ owner navigation">
      ...
      <div className="hq-side-search"><HQGlobalSearch /></div>
      <div className="hq-side-scroll">{HQ_NAV_GROUPS.map(group=><section key={group.label} className="hq-side-group"><div className="hq-side-group-label">{group.label}</div>{group.items.map(([label,href,icon])=>{const active=isActive(pathname,href);return <Link key={`${label}-${href}`} href={href} className={`hq-side-link${active?" is-active":""}`} aria-current={active?"page":undefined}><span className="hq-side-icon" aria-hidden>{icon}</span><span>{label}</span></Link>})}</section>)}</div>
    ...
    <header className="hq-mobile-topbar"><Link href="/hq" className="hq-mobile-brand"><span className="hq-side-logo">V</span><strong>VibeSchool HQ</strong><i>Owner</i></Link><div className="hq-mobile-tools"><HQGlobalSearch compact/><HQNotificationCenter /></div></header>
