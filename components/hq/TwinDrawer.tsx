...
import {
  loadHQBrain, resolveHQReply, buildHQContextString, buildHQOpeningBrief, HQBrainState,
} from "@/lib/twin/hq-brain";
...
async function saveToMemory(userId: string, userMsg: string, twinReply: string) {
  try {
    await supabase.from("twin_memory").insert([
      { user_id: userId, type: "hq_query", content: userMsg.slice(0, 300),   subject: "hq" },
      { user_id: userId, type: "hq_reply", content: twinReply.slice(0, 300), subject: "hq" },
    ]);
  } catch {}
}
...
export default function HQTwinDrawer({ open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      try {
        const brain = await loadHQBrain(data.user.id);
        brainRef.current = brain;
        setMessages([{ role: "twin", text: buildHQOpeningBrief(brain), source: "js" }]);
      } catch {
        setMessages([{ role: "twin", text: "Ready. Ask me about the platform.", source: "offline" }]);
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        setMessages(m => [...m, { role: "twin", text: "You are offline. Ask about schools, courses, or moderation — I can answer from cache.", source: "offline" }]);
        setThinking(false); return;
      }

      const context   = brain ? buildHQContextString(brain) : "No context.";
      const firstName = brain?.firstName ?? "Admin";
      const history   = [
      ...
      const response = await fetch(
        process.env.NEXT_PUBLIC_SUPABASE_URL + "/functions/v1/twin-chat",
        {
          ...
          body: JSON.stringify({ context, firstName, messages: history, role: "hq" }),
        }
      );
      ...
      setMessages(m => [...m, { role: "twin", text: "Something went wrong. Try again or check your connection.", source: "offline" }]);
  ...
  const headerBg  = health === "critical"
    ? "linear-gradient(135deg,#7f1d1d,#991b1b)"
    : health === "warning"
    ? "linear-gradient(135deg,#78350f,#92400e)"
    : "linear-gradient(135deg,#0f1d33,#1e1b4b)";
      ...
      <div style={{
        ...
        <div style={{ background: headerBg, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, transition: "background 0.4s" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#fff" }}>{firstName ? `${firstName}'s HQ Twin` : "HQ Twin"}</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>
                {loading ? "Reading platform data…" : offline ? "Offline · cached" : thinking ? "Thinking…" : health === "critical" ? "⚡ Platform needs attention" : "Knows the platform · Always watching"}
              </div>
            </div>
          </div>
          ...
          {!loading && messages.map((m, i) => (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start", gap: 6 }}>
              <div style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8, width: "100%" }}>
                {m.role === "twin" && (
                  <div style={{ width: 26, height: 26, borderRadius: "50%", background: m.source === "offline" ? "rgba(245,158,11,0.2)" : C.accentLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0, color: m.source === "offline" ? "#f59e0b" : C.accent }}>
                    {m.source === "offline" ? "○" : "✦"}
                  </div>
                )}
                <div style={{
                  ...
                  border: m.role === "twin" ? `1px solid ${C.border}` : "none",
                }}>
                  {m.text}
                  {m.source && m.source !== "ai" && m.source !== "offline" && (
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>instant · no AI used</div>
                  )}
                </div>
              </div>
              {m.role === "twin" && m.actions && m.actions.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, paddingLeft: 34 }}>
                  {m.actions.map((a, ai) => (
                    <button key={ai} onClick={() => handleChipTap(a)} style={{ padding: "6px 12px", borderRadius: 14, border: `1px solid ${C.accent}`, background: C.accentLight, color: C.accent, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                      {a.label}
                    </button>
                  ))}
                </div>
        ...
        <div style={{ padding: "10px 14px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8, flexShrink: 0, alignItems: "center", background: C.panel }}>
          <button
            onClick={toggleVoice}
            style={{ width: 36, height: 36, borderRadius: "50%", border: "none", flexShrink: 0, background: listening ? "#ef4444" : C.accentLight, color: listening ? "#fff" : C.accent, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 15, transition: "all 0.2s" }}
          ...
          <input
            ...
            placeholder={listening ? "Listening…" : "Ask about schools, courses, moderation…"}
            style={{ flex: 1, padding: "9px 13px", borderRadius: 10, border: `1.5px solid ${C.border}`, outline: "none", fontSize: 13, fontFamily: "inherit", background: C.surface, color: C.textPrimary }}
          />
          <button
            onClick={send}
            style={{ padding: "9px 16px", borderRadius: 10, border: "none", background: C.accent, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}
