...
export default function TwinLearningCanvas({
  ...
  return <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 14px 18px', display: 'grid', gap: 12, alignContent: 'start', WebkitOverflowScrolling: 'touch' }}>
    <section style={sectionCard}>
      <div style={eyebrow}>What matters now</div>
      ...
      <div style={body}>{now?.reason || (weakest ? `Twin can strengthen ${weakest.outcomeText} next.` : 'Twin is waiting for enough verified evidence to choose a stronger next step.')}</div>
      <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
        {now?.actionUrl && <button onClick={() => onContinueTask(now.actionUrl!)} style={primaryButton}>{now.actionLabel || 'Continue current task'}</button>}
        <button onClick={onStartPractice} disabled={practiceLoading} style={now?.actionUrl ? secondaryButton : primaryButton}>{practiceLoading ? 'Preparing…' : practiceQuestion ? 'New question' : 'Practice weakest skill'}</button>
    ...
    <section style={sectionCard}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
        {stages.map(item => {
          ...
          return <div key={item.id} style={{ textAlign: 'center', padding: '7px 4px', borderRadius: 10, border: `1px solid ${active ? T.accentBdr : T.border}`, background: active ? T.accentBg : 'transparent' }}><div style={{ fontSize: 9, fontWeight: 900, color: active ? T.text : T.muted }}>{item.label}</div></div>
        })}
      </div>

      {sessionSummary ? <div style={{ display: 'grid', gap: 8, padding: '4px 2px' }}>
        <div style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Session wrapped up</div>
        <div style={body}>{sessionSummary}</div>
        <div style={{ fontSize: 10.5, lineHeight: 1.5, color: T.muted }}>Twin keeps only the evidence already recorded. Ending the workspace does not manufacture mastery.</div>
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}><button onClick={onStartPractice} style={secondaryButton}>Keep learning</button><button onClick={onResumeCompanion} style={secondaryButton}>Continue later</button>{now?.actionUrl && <button onClick={() => onContinueTask(now.actionUrl!)} style={primaryButton}>Do current task</button>}</div>
      </div> : !practiceQuestion ? <div style={{ padding: '8px 2px 2px', display: 'grid', gap: 8 }}>
        <div style={{ fontSize: 15, fontWeight: 900, color: T.text }}>Ready when you are, {userName}.</div>
        <div style={body}>You do not need to know what prompt to type. Continue the recommended task, practice your weakest verified skill, or ask Twin anything about what you are learning.</div>
      </div> : <>
        <div><div style={eyebrow}>Adaptive practice · {practiceQuestion.difficulty}</div><div style={{ marginTop: 4, fontSize: 11, color: T.muted }}>{practiceQuestion.outcomeCode ?? 'Curriculum outcome'} · {practiceQuestion.outcomeText}</div></div>
        <div style={{ fontSize: 16, lineHeight: 1.5, color: T.text, fontWeight: 850 }}>{practiceQuestion.prompt}</div>
        {coachTurn && <div style={{ padding: 11, borderRadius: 12, background: T.accentBg, border: `1px solid ${T.accentBdr}` }}><div style={eyebrow}>{coachTurn.mode.replaceAll('_', ' ')}</div><div style={{ marginTop: 4, fontSize: 12, lineHeight: 1.55, color: T.text }}>{coachTurn.prompt}</div></div>}
        <div style={{ display: 'grid', gap: 7 }}>{practiceQuestion.options.map((option, index) => <button key={`${practiceQuestion.id}-${index}`} onClick={() => onAnswer(index)} disabled={practiceLoading} style={optionButton}><strong>{String.fromCharCode(65 + index)}.</strong> {option}</button>)}</div>
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
        ...
        {hintIndex > 0 && <div style={body}>{practiceQuestion.hints.slice(0, hintIndex).join(' ')}</div>}
    ...
    <section style={sectionCard}>
      ...
      {messages.length === 0 ? <div style={body}>Ask a question whenever you need to. Conversation supports the learning session; it does not replace it.</div> : messages.slice(-6).map(message => <div key={message.id} style={{ justifySelf: message.role === 'user' ? 'end' : 'start', maxWidth: '88%', padding: '9px 11px', borderRadius: 12, background: message.role === 'user' ? T.accentMsg : T.bg, border: `1px solid ${message.role === 'user' ? T.accentMsgBdr : T.border}`, fontSize: 11.5, lineHeight: 1.5, color: T.text }}>{message.text}</div>)}
