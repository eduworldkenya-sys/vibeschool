...
export function ClaimCodeCard({ studentName, code, expiresAt, loading = false, onGenerate, onRegenerate }: ClaimCodeCardProps) {
  ...
  const expiryLabel = expiry && Number.isFinite(expiry.getTime())
    ? `Expires ${expiry.toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "numeric" })}`
  ...
  return (
    <section aria-label="Claim code" style={{ borderRadius: 20, overflow: "hidden", border: "1px solid #dfe7e3", background: "#fff", boxShadow: "0 8px 28px rgba(15,23,42,.06)" }}>
      <div style={{ padding: "18px 18px 14px", background: "linear-gradient(135deg,#111827 0%,#1e1b4b 72%,#064e3b 100%)", color: "#fff" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
          <div>
            ...
            <p style={{ margin: 0, maxWidth: 430, fontSize: 11, lineHeight: 1.5, color: "rgba(255,255,255,.72)" }}>One code can connect {studentName} to both the parent and learner account. One person claiming it does not use it up for the other.</p>
          </div>
          <span style={{ flexShrink: 0, borderRadius: 99, padding: "5px 8px", background: "rgba(16,185,129,.16)", color: "#6ee7b7", fontSize: 9, fontWeight: 850 }}>SHARED</span>
        </div>
        <div style={{ marginTop: 16, borderRadius: 16, background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.1)", padding: "14px 12px" }}>
          {code ? <>
            <div style={{ fontFamily: "ui-monospace,SFMono-Regular,Menlo,monospace", fontSize: 30, lineHeight: 1, fontWeight: 900, letterSpacing: 6, textAlign: "center" }} aria-label={`Claim code ${code}`}>{code}</div>
            <div style={{ marginTop: 8, textAlign: "center", fontSize: 9, color: "rgba(255,255,255,.55)" }}>{expiryLabel}</div>
          </> : <div style={{ textAlign: "center", padding: "7px 0", fontSize: 12, color: "rgba(255,255,255,.68)" }}>No claim code yet</div>}
        </div>
      </div>
      <div style={{ padding: 12 }}>
        {code ? <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        ...
        </div> : <button type="button" onClick={() => void run(onGenerate)} disabled={busy} style={{ width: "100%", minHeight: 44, borderRadius: 11, border: "none", background: busy ? "#9ca3af" : "#111827", color: "#fff", fontWeight: 850, fontSize: 12, cursor: busy ? "wait" : "pointer" }}>{busy ? "Creating code…" : "Generate claim code"}</button>}
        {code && onRegenerate && <div style={{ marginTop: 10 }}>
