...
function storageKey(publicationId: string, chapterId: string): string {
  return `${STORAGE_PREFIX}:${publicationId}:${chapterId}`;
}
...
export function ReaderListenContinuity({ publicationId }: { publicationId: string }) {
  ...
  useEffect(() => {
    function onChapter(raw: Event) {
      ...
      setSaved(chapterId ? readPosition(publicationId, chapterId) : null);
    }
    ...
    const observer = root ? new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        ...
        const position: SavedListenPosition = {
          publicationId,
          chapterId,
          blockId,
        ...
        try {
          window.localStorage.setItem(storageKey(publicationId, chapterId), JSON.stringify(position));
        } catch {
          // Listening remains available even if local storage is blocked.
        }
        setSaved(position);
      }
    }) : null;
  ...
  function resume() {
    ...
    window.setTimeout(() => delete block.dataset.readerListenResume, 1500);

    // The existing listener intentionally starts from the viewport. Open its
    // controls after positioning so one explicit learner action remains before
    // speech begins (important for browser autoplay/user-gesture rules).
    const listenButton = document.querySelector<HTMLButtonElement>(".reader-excellence-bar .reader-excellence-action:first-child");
    if (listenButton && !document.querySelector("[aria-label='Listen to this reading']")) listenButton.click();
  }
  ...
  return <>
    <style jsx global>{`
      .reader-listen-resume{position:fixed;left:12px;bottom:max(66px,env(safe-area-inset-bottom));z-index:93;max-width:min(320px,calc(100vw - 24px));border:1px solid var(--reader-border,rgba(0,0,0,.14));border-radius:12px;background:color-mix(in srgb,var(--reader-surface,#fffaf0) 96%,transparent);color:var(--reader-text,#27231f);padding:8px 11px;font-size:12px;font-weight:850;box-shadow:0 8px 28px rgba(0,0,0,.16);cursor:pointer}
      #reader-active-unit [data-reader-listen-resume="true"]{outline:3px solid color-mix(in srgb,var(--reader-accent,#466400) 65%,transparent);outline-offset:5px;border-radius:7px}
    `}</style>
