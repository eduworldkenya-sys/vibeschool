...
export function LearningCheckpoint({ publicationId, chapterId, blockId, spec }: Props) {
  ...
  if (spec.type !== "multiple_choice" || !spec.choices?.length) {
    return (
      <section style={card} aria-label="Learning checkpoint">
        ...
        <p style={muted}>This checkpoint type is defined in the canonical contract and will use a dedicated renderer.</p>
      </section>
    );
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!auth.user) return;
      await supabase.rpc("record_learning_checkpoint_attempt", {
      ...
      <div style={{ display: "grid", gap: 9, marginTop: 14 }}>
        {spec.choices.map(choice => (
          <button
            key={choice.id}
            type="button"
            onClick={() => { setSelected(choice.id); setFeedback(null); setMisconception(null); setRemediation(null); }}
...
const card: React.CSSProperties = { margin: "20px 0 26px", padding: 16, borderRadius: 14, border: "1px solid rgba(207,255,0,.30)", background: "rgba(207,255,0,.045)" };
const eyebrow: React.CSSProperties = { color: "#cfff00", fontSize: 11, fontWeight: 900, letterSpacing: ".12em", textTransform: "uppercase" };
const muted: React.CSSProperties = { margin: "6px 0 0", opacity: .72, fontSize: 13 };
const option: React.CSSProperties = { width: "100%", textAlign: "left", borderRadius: 10, border: "1px solid rgba(255,255,255,.14)", background: "rgba(255,255,255,.025)", color: "inherit", padding: "11px 12px", cursor: "pointer", fontSize: 14 };
const selectedOption: React.CSSProperties = { borderColor: "rgba(207,255,0,.6)", background: "rgba(207,255,0,.09)" };
const submitButton: React.CSSProperties = { marginTop: 12, minHeight: 42, border: 0, borderRadius: 10, background: "#cfff00", color: "#0b0d12", padding: "9px 14px", fontWeight: 900, cursor: "pointer" };
const link: React.CSSProperties = { display: "inline-block", marginTop: 9, color: "#cfff00", fontWeight: 800, textDecoration: "none", fontSize: 13 };
