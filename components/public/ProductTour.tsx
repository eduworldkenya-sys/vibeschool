...
export function ProductTour(){
  ...
  return <section className={styles.section} aria-labelledby="product-tour-title">
    <div className={styles.wrap}>
      <div className={styles.heading}><p>SEE VIBESCHOOL WORK</p><h2 id="product-tour-title">See one lesson become learning.</h2><span>Follow one connected journey from curriculum planning to learner progress and support, then run it yourself in the live sandbox.</span></div>
      ...
      <div className={styles.stage} id="product-tour-panel" role="tabpanel" aria-labelledby={`product-tour-tab-${active}`}>
        ...
        <div className={styles.frame} aria-label={`${step.role} product state`}><div className={styles.frameBar}><span></span><span></span><span></span><strong>VibeSchool</strong></div><div className={styles.frameBody}><div className={styles.side}><i></i><i></i><i></i><i></i></div><div className={styles.screen}><small>CONNECTED PRODUCT STATE · {String(active+1).padStart(2,'0')}</small><h4>{step.role}</h4><div className={styles.stateCard}><span>{step.label}</span><strong>{step.state}</strong></div><div className={styles.lines}><i></i><i></i><i></i></div><p>{step.detail}</p><Link href="/sandbox">Open live sandbox →</Link></div></div></div>
      </div>
      ...
      <p className={styles.disclosure}>This orientation explains the connected learning chain. The public sandbox uses fabricated browser-only demonstration state and never exposes production learner data.</p>
    </div>
  </section>
}
