...
export default function ProductRuntimeGate({ product, children }: { product: HQProductKey; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function handshake() {
      try {
        const { data, error } = await (supabase as any).rpc("hq_product_runtime_handshake", { p_product_key: product, p_route: pathname || null });
        if (error) throw error;
        if (alive) setState(data?.enabled === true ? "enabled" : "disabled");
      } catch {
  ...
  if (state === "checking") return <div style={{ minHeight: "100dvh", display: "grid", placeItems: "center", background: "var(--vs-bg, #0f172a)", color: "var(--vs-text, #fff)", fontFamily: "system-ui, sans-serif" }}><div style={{ fontSize: 13, opacity: .7 }}>Checking VibeSchool service status…</div></div>;
