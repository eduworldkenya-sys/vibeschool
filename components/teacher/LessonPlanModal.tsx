...
export default function LessonPlanModal({ slot, onClose }: Props) {
  ...
  const [status,     setStatus]     = useState<'draft'|'published'|'shared_to_parents'>('draft')
  ...
  const [ctx,        setCtx]        = useState<Context>({
    teacherName: '', schoolName: '', schoolId: '', studentCount: 0, previousTopics: [], students: [],
  })
  ...
  useEffect(() => {
    async function load() {
      ...
      const [schoolRes, studentRes, prevRes] = await Promise.all([
        ...
        supabase.from('students').select('id, name').eq('class_id', slot.class_id),
        supabase.from('lesson_plans')
          .select('topic')
          .eq('teacher_id', user.id)
          .eq('class_id',   slot.class_id)
      ...
      if (existingRes.data?.body) {
        ...
        if (existingRes.data.status) setStatus(existingRes.data.status as 'draft'|'published'|'shared_to_parents')
  ...
  async function handlePublish() {
    if (!planId) return
    setPublishing(true)
    await supabase.from('lesson_plans').update({ status: 'published' }).eq('id', planId)
    const { data: { user } } = await supabase.auth.getUser()
    if (user && ctx.students.length > 0) {
  ...
  async function handleShareToParents() {
    ...
    await supabase.from('lesson_plans').update({ status: 'shared_to_parents' }).eq('id', planId)
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 16, borderTop: '1px solid ' + C.border, marginTop: 8 }}>
                {status === 'draft' && (
                  <button onClick={handlePublish} disabled={publishing} style={{
                    ...
                    cursor: publishing ? 'not-allowed' : 'pointer', opacity: publishing ? 0.7 : 1, fontFamily: 'inherit',
                  }}>
                    {publishing ? 'Publishing…' : '📤 Publish to Students'}
                  </button>
                )}
                {status !== 'shared_to_parents' && (
                  <button onClick={handleShareToParents} disabled={sharing} style={{
                    width: '100%', padding: '13px', borderRadius: 12,
                    border: '1.5px solid #1e40af', background: '#eff6ff', color: '#1e40af',
                    fontSize: 13, fontWeight: 800, cursor: sharing ? 'not-allowed' : 'pointer',
                    opacity: sharing ? 0.7 : 1, fontFamily: 'inherit',
                  }}>
                    {sharing ? 'Sharing…' : '👨‍👩‍👧 Share to Parents'}
                  </button>
                )}
                {status === 'shared_to_parents' && (
                  <div style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 10, paddingTop: 8, borderTop: '1px solid ' + C.border }}>
                ...
                <button onClick={saveEdit} disabled={saving} style={{
                  ...
                  cursor: 'pointer', opacity: saving ? 0.7 : 1, fontFamily: 'inherit',
                }}>
