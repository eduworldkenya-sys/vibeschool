...
- Do not link a non-main workflow to production Supabase or run `db push`, migration repair, function deployment, or secrets mutation from a non-main ref.
...
Supabase Preview Branching must remain disabled unless a written exception identifies the exact PR and expiry. Database validation on non-main refs must use the isolated local stack only; production is read-only until the verified main release.
