...
| Worker Engine deterministic-first model authorization | PARTIAL PROOF | `hq_workforce_authorize_model_call`, durable invocation/budget ledger | production authorization observed; provider transport still needs Cyborg capability enforcement |
...
Current `scripts/validate-cyborg-llm-gateway.mjs` scans `app/api`, `lib`, `components`, and `scripts`, but not `supabase/functions`. Its forbidden URL set also omits Groq. Repository search shows direct Groq execution in several server/Edge Function surfaces, including model-backed content workers. A separate chat-session validator covers chat entrypoints and requires mission identity before provider calls, but `twin-chat` still performs direct Groq HTTP execution rather than routing through the canonical `CyborgUniversalGateway`.

Therefore mission-tagged chat admission must not be represented as the stronger invariant "no signed Cyborg capability = no LLM." That stronger capability-enforced provider boundary remains a separate implementation/proof programme.

## Worker Engine boundary

Fresh production inspection on 2026-08-23 supersedes one stale statement in the older mutation-topology document. `hq_workforce_tool_gateway_execute(task_id)` is now a compatibility wrapper into `hq_workforce_consequential_execution_gateway(task_id)`, not an independent legacy mutator. The legacy safe queue and older externally callable authority paths have also been revoked/fail-closed, active capability-authority grants are zero, and runtime/heartbeat/Factory/Shadow remain off with autonomy/risk zero and Global Stop on.

The remaining Worker Engine ↔ Cyborg gap is model transport. Worker Engine can authorize and account for a model call through `hq_workforce_authorize_model_call`, but model-backed Edge Functions still call Groq directly. Full integration requires the Worker authorization to be exchanged for a short-lived signed Cyborg capability and consumed by a sole provider gateway, with no provider credentials usable by worker runtimes.
...
Full autonomous production certification is a higher-level programme and remains blocked until every applicable production/runtime row has fresh evidence, including persistence and execution adapters, restart/recovery, bounded real missions, applied-production database proof, provider-bypass prevention, Worker Engine -> Cyborg provider enforcement, broader consequential-resource proof, independent assurance, and production/repository parity appropriate to that release.
