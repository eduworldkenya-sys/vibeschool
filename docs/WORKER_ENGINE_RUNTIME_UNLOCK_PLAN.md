...
Before L3, workers may:

- detect work that needs doing;
- create candidate jobs;
- select a proposed worker and certified skill/procedure;
- calculate priority and SLA;
- produce proposed actions;
- record evidence and reasoning;
- predict expected outcomes;
- identify missing workers/skills;
- escalate uncertain or high-risk decisions to the Decision Inbox.
...
Every consequential skill/procedure must be versioned and declare at least:

- purpose;
- required inputs;
- allowed tables/functions/tools;
- preconditions;
- authority level;
...
Before any capability is promoted beyond shadow/recommendation mode, the measurement program must target:
